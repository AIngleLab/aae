Win32 C++ builds of AIngle-C
*****************************

April 2, 2012

These instructions describe some of the changes required to allow
AIngle-C to compile under the Microsoft Visual C++ 2008 compiler, as
well as some limitations of the Windows build.

Status of the Windows Build:
****************************

The Windows build of AIngle-C compiles under Microsoft Visual C++
2008. You can use C-Make to create the solution file (AIngleC.sln) for
...
1. The Windows build of AIngle-C is compiled using Microsoft's C++
   ...
   statements were removed, that AIngle-C would compile under
   Microsoft's C compiler also. I have not tried this.

2. The shared library, i.e. DLL, for aingle has not been built. There
   are instructions on how to build DLLs using CMake at
   https://www.cmake.org/Wiki/BuildingWinDLL

3. Currently ainglepipe.c and ainglemod.c do not compile under Windows. In
...
4. Windows cannot run version.sh to generate the version
   number. Currently, LIBAINGLE_VERSION is hard coded to "22:0:0" for
   the Windows build, in the top level CMakeLists.txt.

5. Found two bugs related to improper return values under error
   conditions in AIngle. These bugs were marked with #warnings in the
   code.
...
1. Instructions to check name mangling in Visual C++:
    ...
    Now go to the aingle_c\build_win32\src\Debug\ directory.
    Run the command

    dumpbin /ALL aingle.lib > tmp.txt
...
6. MS Visual C++ 2008 does not properly expand variadic preprocessor
   ...
   This method is used in the macro expand_args(...) in test_aingle_values.c.
...
8. Sometimes it is necessary to declare a struct at the top of a file
   ...
   AINGLE_DATUM_VALUE_CLASS in src/datum_value.c. A C++ compiler will
...
14. Do not include the headers inttypes.h or unistd.h or
    stdint.h. Instead include aingle/platform.h in your C files.

15. Do not include dirent.h in your tests. When _WIN32 is defined
    include msdirent.h. See example in test_aingle_schema.c.
