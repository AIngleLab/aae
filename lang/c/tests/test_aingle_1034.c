...
/* Test code for JIRA Issue AINGLE-1034.
 *
 * AINGLE-1034: Resolved reader does not initialize children of arrays,
 ...
 *
 * The test creates and populates a nested array aingle value, and
 ...
 *
 * An identical nested array aingle value is then created. A
 ...
 *   gcc -g aingle-1034-test-2.c -o test2 -I../../build/ainglelib/include -L../../build/ainglelib/lib/static -laingle
 *
 */
...
int populate_array( aingle_value_t *p_array, int32_t elements )
{
  int32_t idx;
  ...
  for ( idx = 0; idx < elements; idx ++ )
  {
    ...
    try ( aingle_value_append( p_array, &subarray, &index ),
               ...
               int use_resolving_writer )
{
  ...
  if ( use_resolving_writer )
  {
    ...
    aingle_value_iface_t *resolved_reader_class;
    aingle_value_iface_t *writer_class;
    aingle_value_t resolved_record;

    // Note - we will read values from the reader of "schema to write
    // to file" and we will copy them into a writer of the same
    // schema.
    resolved_reader_class = aingle_resolved_reader_new( schema_old,// schema populated above
                                                      schema_new // schema_to_write_to_file
                                                    );
    if ( resolved_reader_class == NULL )
    {
      printf( "Failed aingle_resolved_reader_new()\n");
      exit( EXIT_FAILURE );
    }

    try ( aingle_resolved_reader_new_value( resolved_reader_class, &resolved_record ),
          "Failed aingle_resolved_reader_new_value" );

    // Map the resolved reader to the record you want to get data from
    aingle_resolved_reader_set_source( &resolved_record, &nested );
    ...
    writer_class = aingle_generic_class_from_schema( schema_new );
    if ( writer_class == NULL )
    {
      printf( "Failed aingle_generic_class_from_schema()\n");
      exit( EXIT_FAILURE );
    }

    try ( aingle_value_write( writer, &resolved_record ),
          "Unable to write record into memory using writer_record" );

    print_array_fields( &resolved_record );

    aingle_value_reset( &nested );

    // Question: Is it permissible to call aingle_value_reset() on a
    // resolved_record? Set the #if 1 to #if 0 to disable the
    // aingle_value_reset(), to prevent the segmentation fault.
   #if 1
    aingle_value_reset( &resolved_record );
   #endif
    ...
    aingle_value_decref( &resolved_record );
    aingle_value_iface_decref( writer_class );
    aingle_value_iface_decref( resolved_reader_class );
  }
  else
  {
 ...
 * AINGLE-1034. See detailed documentation at the top of this file.
 */
int main(void)
{
