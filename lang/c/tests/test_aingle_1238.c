...
int main(void)
{
	...
	check_exit(aingle_file_reader("aingle-1238-good.aingle", &reader));
	...
	expect_eof(aingle_file_reader_read_value(reader, &actual));
	check_exit(aingle_file_reader_close(reader));


	/* Then read from the truncated file. */

	check_exit(aingle_file_reader("aingle-1238-truncated.aingle", &reader));
