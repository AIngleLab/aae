...
/* Our seed is the MurmurHash3 of the string "aingle.value" */
...
static uint32_t
aingle_value_hash_fast(aingle_value_t *value, uint32_t start)
{
	...
	switch (type) {
		...
		{
			int  v;
			check_return(0, aingle_value_get_boolean(value, &v));
		...
		case AINGLE_BYTES:
		{
			const void  *buf;
			size_t  size;
			check_return(0, aingle_value_get_bytes(value, &buf, &size));
		...
		case AINGLE_DOUBLE:
		{
			...
			check_return(0, aingle_value_get_double(value, &v.d));
		...
		case AINGLE_FLOAT:
		{
			...
			check_return(0, aingle_value_get_float(value, &v.f));
		...
		{
			int32_t  v;
			check_return(0, aingle_value_get_int(value, &v));
			return add_hash(start, v);
		}
		...
		case AINGLE_INT64:
		{
			...
			check_return(0, aingle_value_get_long(value, &v.u64));
			return add_hash(add_hash(start, v.u32[0]), v.u32[1]);
		}

		case AINGLE_NULL:
		{
			check_return(0, aingle_value_get_null(value));
		...
		case AINGLE_STRING:
		{
			const char  *buf;
			size_t  size;
			check_return(0, aingle_value_get_string(value, &buf, &size));
			return hash_buffer(start, buf, size);
		}
		...
		case AINGLE_ARRAY:
		{
			size_t  count;
			size_t  i;
			check_return(0, aingle_value_get_size(value, &count));

			for (i = 0; i < count; i++) {
				aingle_value_t  child;
				check_return(0, aingle_value_get_by_index
					     (value, i, &child, NULL));
				start = aingle_value_hash_fast(&child, start);
			}
		...
		{
			int  v;
			check_return(0, aingle_value_get_enum(value, &v));
			return add_hash(start, v);
		}
		...
		case AINGLE_FIXED:
		{
			const void  *buf;
			size_t  size;
			check_return(0, aingle_value_get_fixed(value, &buf, &size));
			return hash_buffer(start, buf, size);
		}
		...
		case AINGLE_MAP:
		{
			...
			for (i = 0; i < count; i++) {
				...
				element = aingle_value_hash_fast(&child, element);
		...
		case AINGLE_UNION:
		{
			...
			check_return(0, aingle_value_get_discriminant(value, &disc));
			check_return(0, aingle_value_get_current_branch(value, &branch));

			start = add_hash(start, disc);
			start = aingle_value_hash_fast(&branch, start);
			return start;
		}
...
{
	uint32_t  hash = aingle_value_hash_fast(value, SEED);
