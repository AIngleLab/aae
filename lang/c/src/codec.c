...
static int
codec_null(aingle_codec_t codec)
{
	codec->name = "null";
	codec->type = AINGLE_CODEC_NULL;
	codec->block_size = 0;
	codec->used_size = 0;
...
static int encode_null(aingle_codec_t c, void * data, int64_t len)
{
	c->block_data = data;
	c->block_size = len;
	c->used_size = len;
...
static int decode_null(aingle_codec_t c, void * data, int64_t len)
{
	c->block_data = data;
	c->block_size = len;
	c->used_size = len;
...
static int reset_null(aingle_codec_t c)
{
	c->block_data = NULL;
	c->block_size = 0;
	c->used_size = 0;
...
static int
codec_snappy(aingle_codec_t codec)
{
	codec->name = "snappy";
	codec->type = AINGLE_CODEC_SNAPPY;
	codec->block_size = 0;
	codec->used_size = 0;
	codec->block_data = NULL;
	codec->codec_data = NULL;
...
static int encode_snappy(aingle_codec_t c, void * data, int64_t len)
{
        uint32_t crc;
        size_t outlen = snappy_max_compressed_length(len);

	if (!c->block_data) {
		c->block_data = aingle_malloc(outlen+4);
		c->block_size = outlen+4;
	} else if (c->block_size < (int64_t) (outlen+4)) {
            c->block_data = aingle_realloc(c->block_data, c->block_size, (outlen+4));
		c->block_size = outlen+4;
	}

	if (!c->block_data) {
		aingle_set_error("Cannot allocate memory for snappy");
		return 1;
	}

        if (snappy_compress((const char *)data, len, (char*)c->block_data, &outlen) != SNAPPY_OK)
        {
                aingle_set_error("Error compressing block with Snappy");
		return 1;
	}
...
static int decode_snappy(aingle_codec_t c, void * data, int64_t len)
{
        uint32_t crc;
        size_t outlen;

        if (snappy_uncompressed_length((const char*)data, len-4, &outlen) != SNAPPY_OK) {
		aingle_set_error("Uncompressed length error in snappy");
		return 1;
        }

	if (!c->block_data) {
		c->block_data = aingle_malloc(outlen);
		c->block_size = outlen;
	} else if ( (size_t)c->block_size < outlen) {
		c->block_data = aingle_realloc(c->block_data, c->block_size, outlen);
		c->block_size = outlen;
	}
	...
	if (!c->block_data)
	{
        ...
        {
                aingle_set_error("Error uncompressing block with Snappy");
		return 1;
	}
        ...
        {
                aingle_set_error("CRC32 check failure uncompressing block with Snappy");
		return 1;
	}
...
static int reset_snappy(aingle_codec_t c)
{
	if (c->block_data) {
		aingle_free(c->block_data, c->block_size);
	}
...
static int
codec_deflate(aingle_codec_t codec)
{
	codec->name = "deflate";
	codec->type = AINGLE_CODEC_DEFLATE;
	codec->block_size = 0;
	codec->used_size = 0;
	codec->block_data = NULL;
	codec->codec_data = aingle_new(struct codec_data_deflate);

	if (!codec->codec_data) {
		aingle_set_error("Cannot allocate memory for zlib");
		return 1;
	}
	...
	if (deflateInit2(ds, Z_BEST_COMPRESSION, Z_DEFLATED, -15, 8, Z_DEFAULT_STRATEGY) != Z_OK) {
		aingle_freet(struct codec_data_deflate, codec->codec_data);
		codec->codec_data = NULL;
		aingle_set_error("Cannot initialize zlib deflate");
		return 1;
	}
	...
	if (inflateInit2(is, -15) != Z_OK) {
		aingle_freet(struct codec_data_deflate, codec->codec_data);
		codec->codec_data = NULL;
		aingle_set_error("Cannot initialize zlib inflate");
		return 1;
	}
...
static int encode_deflate(aingle_codec_t c, void * data, int64_t len)
{
	int err;
	int64_t defl_len = compressBound((uLong)len * 1.2);

	if (!c->block_data) {
		c->block_data = aingle_malloc(defl_len);
		c->block_size = defl_len;
	} else if ( c->block_size < defl_len) {
		c->block_data = aingle_realloc(c->block_data, c->block_size, defl_len);
		c->block_size = defl_len;
	}

	if (!c->block_data)
	{
		aingle_set_error("Cannot allocate memory for deflate");
		return 1;
	}
	...
	if (err != Z_STREAM_END) {
		deflateEnd(s);
		if (err != Z_OK) {
			aingle_set_error("Error compressing block with deflate (%i)", err);
			return 1;
		}
		return 0;
	}
...
static int decode_deflate(aingle_codec_t c, void * data, int64_t len)
{
	int err;
	z_stream *s = codec_data_inflate_stream(c->codec_data);

	if (!c->block_data) {
		c->block_data = aingle_malloc(DEFAULT_BLOCK_SIZE);
	...
	do
	{
		...
		if (err == Z_BUF_ERROR && s->avail_out > 0) {
			inflateEnd(s);
			aingle_set_error("Error decompressing block with deflate, possible data error");
			return 1;
		}
		...
		if (err == Z_BUF_ERROR)
		{
			c->block_data = aingle_realloc(c->block_data, c->block_size, c->block_size * 2);
	...
	if (err != Z_STREAM_END) {
		inflateEnd(s);
		if (err != Z_OK) {
			aingle_set_error("Error decompressing block with deflate (%i)", err);
			return 1;
		}
		return 0;
	}
	...
	if (inflateReset(s) != Z_OK) {
		aingle_set_error("Error resetting deflate decompression");
		return 1;
	}
...
static int reset_deflate(aingle_codec_t c)
{
	if (c->block_data) {
		aingle_free(c->block_data, c->block_size);
	}
	if (c->codec_data) {
		...
		aingle_freet(struct codec_data_deflate, c->codec_data);
	}
...
static int
codec_lzma(aingle_codec_t codec)
{
	codec->name = "lzma";
	codec->type = AINGLE_CODEC_LZMA;
	codec->block_size = 0;
	codec->used_size = 0;
	codec->block_data = NULL;
	codec->codec_data = aingle_new(struct codec_data_lzma);

	if (!codec->codec_data) {
		aingle_set_error("Cannot allocate memory for lzma");
		return 1;
	}
...
static int encode_lzma(aingle_codec_t codec, void * data, int64_t len)
{
	lzma_ret ret;
	size_t written = 0;
	lzma_filter* filters = codec_data_lzma_filters(codec->codec_data);
	...
	if (!codec->block_data) {
		codec->block_data = aingle_malloc(buff_len);
		codec->block_size = buff_len;
	}

	if (!codec->block_data)
	{
		aingle_set_error("Cannot allocate memory for lzma encoder");
		return 1;
	}
	...
	if (ret != LZMA_OK) {
		aingle_set_error("Error in lzma encoder");
		return 1;
	}
...
static int decode_lzma(aingle_codec_t codec, void * data, int64_t len)
{
	size_t read_pos = 0;
	size_t write_pos = 0;
	lzma_ret ret;
	...
	if (!codec->block_data) {
		codec->block_data = aingle_malloc(DEFAULT_BLOCK_SIZE);
		codec->block_size = DEFAULT_BLOCK_SIZE;
	}

	if (!codec->block_data) {
		aingle_set_error("Cannot allocate memory for lzma decoder");
		return 1;
	}
	...
	do
	{
		...
		if (ret == LZMA_BUF_ERROR) {
			codec->block_data = aingle_realloc(codec->block_data, codec->block_size, codec->block_size * 2);
	...
	if (ret != LZMA_OK) {
		aingle_set_error("Error in lzma decoder");
		return 1;
	}
...
static int reset_lzma(aingle_codec_t c)
{
	if (c->block_data) {
		aingle_free(c->block_data, c->block_size);
	}
	if (c->codec_data) {
		aingle_freet(struct codec_data_lzma, c->codec_data);
	}
...
int aingle_codec(aingle_codec_t codec, const char *type)
{
	if (type == NULL) {
	...
	aingle_set_error("Unknown codec %s", type);
	return 1;
}
...
int aingle_codec_encode(aingle_codec_t c, void * data, int64_t len)
{
	switch(c->type)
	{
	case AINGLE_CODEC_NULL:
		return encode_null(c, data, len);
#ifdef SNAPPY_CODEC
	case AINGLE_CODEC_SNAPPY:
		return encode_snappy(c, data, len);
#endif
#ifdef DEFLATE_CODEC
	case AINGLE_CODEC_DEFLATE:
		return encode_deflate(c, data, len);
#endif
#ifdef LZMA_CODEC
	case AINGLE_CODEC_LZMA:
