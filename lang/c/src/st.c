...
#define Calloc(n,s) (char*)aingle_calloc((n),(s))

#define free_bins(tbl)  \
	aingle_free(tbl->bins, tbl->num_bins * sizeof(st_table_entry *))
...
st_table *st_init_table_with_size(struct st_hash_type *type, int size)
{
	...
	tbl = (st_table *) aingle_new(st_table);
	tbl->type = type;
	tbl->num_entries = 0;
	tbl->num_bins = size;
...
void st_free_table(st_table *table)
{
	...
	for (i = 0; i < table->num_bins; i++) {
		...
		while (ptr != 0) {
			next = ptr->next;
			aingle_freet(st_table_entry, ptr);
			ptr = next;
		}
	}
	free_bins(table);
	aingle_freet(st_table, table);
}
...
do {\
    ...
    \
    entry = (st_table_entry *) aingle_new(st_table_entry);\
    \
    entry->hash = hash_val;\
    entry->key = key;\
    entry->record = value;\
...
st_table *st_copy(st_table *old_table)
{
	...
	new_table = (st_table *) aingle_new(st_table);
	if (new_table == 0) {
		return 0;
	}
	...
	*new_table = *old_table;
	...
	if (new_table->bins == 0) {
		aingle_freet(st_table, new_table);
		return 0;
	}
	...
	for (; ptr->next != 0; ptr = ptr->next) {
		if (EQUAL(table, ptr->next->key, *key)) {
			...
			if (value != 0)
				*value = tmp->record;
			*key = tmp->key;
			aingle_freet(st_table_entry, tmp);
			return 1;
		}
	}
