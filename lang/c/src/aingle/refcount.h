...
#ifndef AINGLE_REFCOUNT_H
#define AINGLE_REFCOUNT_H
...
static inline void
aingle_refcount_set(volatile int *refcount, int value);
...
static inline void
aingle_refcount_inc(volatile int *refcount);
...
static inline int
aingle_refcount_dec(volatile int *refcount);


/*-----------------------------------------------------------------------
 * Non-Atomic Reference Count
 */
#if defined(AINGLE_NON_ATOMIC_REFCOUNT)
static inline void
aingle_refcount_set(volatile int *refcount, int value)
{
	*refcount = value;
...
static inline int
aingle_refcount_LL_int(volatile int *ptr)
{
	int val;
...
static inline int
aingle_refcount_SC_int(volatile int *ptr, int val)
{
...
aingle_refcount_inc(volatile int *refcount)
{
	int prev;
	do {
		prev = aingle_refcount_LL_int(refcount);
		if (prev == (int) -1) {
			return;
		}
	} while (!aingle_refcount_SC_int(refcount, prev + 1));
}
