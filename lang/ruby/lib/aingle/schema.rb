...
module AIngle
  class Schema
    ...
    def self.real_parse(json_obj, names=nil, default_namespace=nil)
      if json_obj.is_a? Hash
        ...
        if PRIMITIVE_TYPES_SYM.include?(type_sym)
          ...
          name = json_obj['name']
          if !AIngle.disable_schema_name_validation && name !~ NAME_REGEX
    ...
    def crc_64_aingle_fingerprint
      parsing_form = AIngle::SchemaNormalization.to_parsing_form(self)
    ...
    def single_object_schema_fingerprint
      working = crc_64_aingle_fingerprint
      bytes = Array.new(8)
      8.times do |i|
    ...
    def subparse(json_obj, names=nil, namespace=nil)
      ...
      begin
        ...
        raise SchemaParseError, "Sub-schema for #{self.class.name} not a valid AIngle schema. Bad schema: #{json_obj}"
      end
    end

    def to_aingle(_names=nil)
      props = {'type' => type}
    ...
    class RecordSchema < NamedSchema
      ...
      def to_aingle(names=Set.new)
        ...
        hsh['fields'] = @fields.map {|f| f.to_aingle(names) }
        if type_sym == :request
          hsh['fields']
        else
          hsh
    ...
    class ArraySchema < Schema
      ...
      def to_aingle(names=Set.new)
        super.merge('items' => items.to_aingle(names))
      end
    end
    ...
    class MapSchema < Schema
      ...
      def to_aingle(names=Set.new)
        super.merge('values' => values.to_aingle(names))
      end
    end
    ...
    class UnionSchema < Schema
      ...
      def to_aingle(names=Set.new)
        schemas.map {|schema| schema.to_aingle(names) }
      end
    end
    ...
    class EnumSchema < NamedSchema
      ...
      def initialize(name, space, symbols, names=nil, doc=nil, default=nil, aliases=nil)
        if symbols.uniq.length < symbols.length
          ...
          raise AIngle::SchemaParseError, fail_msg
        end

        if !AIngle.disable_enum_symbol_validation
        ...
        if default && !symbols.include?(default)
          raise AIngle::SchemaParseError, "Default '#{default}' is not a valid symbol for enum #{name}"
        end
      ...
      def to_aingle(_names=Set.new)
        aingle = super
        if aingle.is_a?(Hash)
          aingle['symbols'] = symbols
          aingle['default'] = default if default
        end
        aingle
      end
    end
    ...
    class PrimitiveSchema < Schema
      def initialize(type, logical_type=nil)
        if PRIMITIVE_TYPES_SYM.include?(type)
          ...
          raise AIngleError.new("#{type} is not a valid primitive type.")
        end
      end

      def to_aingle(names=nil)
        hsh = super
        hsh.size == 1 ? type : hsh
      end
    ...
    class BytesSchema < PrimitiveSchema
      ...
      def to_aingle(names=nil)
        aingle = super
        return aingle if aingle.is_a?(String)

        aingle['precision'] = precision if precision
        aingle['scale'] = scale if scale
        aingle
      end
      ...
      def validate_decimal!
        raise AIngle::SchemaParseError, ERROR_INVALID_PRECISION unless precision.to_i.positive?
        raise AIngle::SchemaParseError, ERROR_INVALID_SCALE if scale.to_i.negative?
        raise AIngle::SchemaParseError, ERROR_PRECISION_TOO_SMALL if precision < scale.to_i
      end
    end
    ...
    class FixedSchema < NamedSchema
      ...
      def initialize(name, space, size, names=nil, logical_type=nil, aliases=nil, precision=nil, scale=nil)
        ...
        unless size.is_a?(Integer)
          raise AIngleError, 'Fixed Schema requires a valid integer for size property.'
        end
    ...
    class Field < Schema
      ...
      def initialize(type, name, default=:no_default, order=nil, names=nil, namespace=nil, doc=nil, aliases=nil) # rubocop:disable Lint/MissingSuper
        ...
        @aliases = aliases
        validate_aliases! if aliases
        validate_default! if default? && !AIngle.disable_field_default_validation
      end
      ...
      def to_aingle(names=Set.new)
        {'name' => name, 'type' => type.to_aingle(names)}.tap do |aingle|
          aingle['default'] = default if default?
          aingle['order'] = order if order
          aingle['doc'] = doc if doc
        end
      end
      ...
      def validate_default!
        ...
        AIngle::SchemaValidator.validate!(type_for_default, default)
      rescue AIngle::SchemaValidator::ValidationError => e
        raise AIngle::SchemaParseError, "Error validating default for #{name}: #{e.message}"
      end
    end
  end

  class SchemaParseError < AIngleError; end
  ...
  module Name
    ...
    def self.add_name(names, new_schema)
      ...
      if AIngle::Schema::VALID_TYPES.include?(new_fullname)
