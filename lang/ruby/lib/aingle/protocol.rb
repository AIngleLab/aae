...
module AIngle
  class Protocol
    ...
    class ProtocolParseError < AIngle::AIngleError; end
    ...
    def to_aingle(names=Set.new)
      ...
      hsh['types'] = types.map{|t| t.to_aingle(names) } if types

      if messages
        hsh['messages'] = messages.inject({}) {|h, (k,t)| h[k] = t.to_aingle(names); h }
      end
    ...
    class Message
      ...
      def to_aingle(names=Set.new)
        {
          'request' => request.to_aingle(names),
          'response' => response.to_aingle(names)
        }.tap do |hash|
          hash['errors'] = errors.to_aingle(names) if errors
          hash['doc'] = @doc if @doc
        end
      end
