...
module AIngle
  module DataFile
    ...
    class DataFileError < AIngleError; end

    def self.open(file_path, mode='r', schema=nil, codec=nil)
      schema = AIngle::Schema.parse(schema) if schema
      case mode
      when 'w'
        unless schema
          raise DataFileError, "Writing an AIngle file requires a schema."
        end
    ...
    class << self
      ...
      def open_writer(file, schema, codec=nil)
        writer = AIngle::IO::DatumWriter.new(schema)
        AIngle::DataFile::Writer.new(file, writer, schema, codec)
      end

      def open_reader(file, schema)
        reader = AIngle::IO::DatumReader.new(nil, schema)
        AIngle::DataFile::Reader.new(file, reader)
      end
    end
    ...
    class Writer
      ...
      def initialize(writer, datum_writer, writers_schema=nil, codec=nil, meta={})
        ...
        if writers_schema
          ...
          @meta['aingle.codec'] = @codec.codec_name.to_s
          @meta['aingle.schema'] = writers_schema.to_s
          datum_writer.writers_schema = writers_schema
          write_header
        else
          # open writer for reading to collect metadata
          dfr = Reader.new(writer, AIngle::IO::DatumReader.new)

          # FIXME(jmhodges): collect arbitrary metadata
          # collect metadata
          @sync_marker = dfr.sync_marker
          @meta['aingle.codec'] = dfr.meta['aingle.codec']
          @codec = DataFile.get_codec(meta['aingle.codec'])

          # get schema used to write existing file
          schema_from_file = dfr.meta['aingle.schema']
          @meta['aingle.schema'] = schema_from_file
    ...
    class Reader
      ...
      def initialize(reader, datum_reader)
        ...
        # get ready to read
        @block_count = 0
        datum_reader.writers_schema = Schema.parse meta['aingle.schema']
      end
      ...
      def read_header
        ...
        if magic_in_file.size < MAGIC_SIZE
          msg = 'Not an AIngle data file: shorter than the AIngle magic block'
          raise DataFileError, msg
        elsif magic_in_file != MAGIC
          msg = "Not an AIngle data file: #{magic_in_file.inspect} doesn't match #{MAGIC.inspect}"
          raise DataFileError, msg
        end
    ...
    class SnappyCodec
      ...
      def decompress(data)
        ...
        if crc32 == Zlib.crc32(uncompressed)
          ...
          # older versions of aingle-ruby didn't write the checksum, so if it
          # doesn't match this must assume that it wasn't there and return
          # the entire payload uncompressed.
          Snappy.inflate(data)
        end
      rescue Snappy::Error
        # older versions of aingle-ruby didn't write the checksum, so removing
    ...
    # Deprecated in favor of AIngle::DataFile::codecs
