...
module AIngle
  module IO
    ...
    class AIngleTypeError < AIngleError
    ...
    class SchemaMatchException < AIngleError
    ...
    class DatumReader
      def self.match_schemas(writers_schema, readers_schema)
        AIngle::SchemaCompatibility.match_schemas(writers_schema, readers_schema)
      end
      ...
      def read_data(writers_schema, readers_schema, decoder)
        ...
        when :record, :error, :request;  read_record(writers_schema, readers_schema, decoder)
          ...
          raise AIngleError, "Cannot read unknown schema type: #{writers_schema.type}"
        end
      ...
      def read_record(writers_schema, readers_schema, decoder)
        ...
        readers_fields_hash.each do |field_name, field|
          ...
          if field.default?
            ...
            raise AIngleError, "Missing data for #{field.type} with no default"
          end
        end
      ...
      def skip_data(writers_schema, decoder)
        ...
        when :record, :error, :request
          ...
          raise AIngleError, "Unknown schema type: #{writers_schema.type}"
        end
      end
    ...
    class DatumWriter
      ...
      def write_data(writers_schema, logical_datum, encoder)
        ...
        unless Schema.validate(writers_schema, datum, VALIDATION_OPTIONS)
          raise AIngleTypeError.new(writers_schema, datum)
        end
        ...
        when :record, :error, :request;  write_record(writers_schema, datum, encoder)
          ...
          raise AIngleError.new("Unknown type: #{writers_schema.type}")
        end
      end
      ...
      def write_array(writers_schema, datum, encoder)
        raise AIngleTypeError.new(writers_schema, datum) unless datum.is_a?(Array)
        if datum.size > 0
      ...
      def write_map(writers_schema, datum, encoder)
        raise AIngleTypeError.new(writers_schema, datum) unless datum.is_a?(Hash)
        if datum.size > 0
          encoder.write_long(datum.size)
          datum.each do |k,v|
