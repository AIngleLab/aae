...
module AIngle::IPC
  ...
  class AIngleRemoteError < AIngle::AIngleError; end

  HANDSHAKE_REQUEST_SCHEMA = AIngle::Schema.parse <<-JSON
  {
    "type": "record",
  ...
  HANDSHAKE_RESPONSE_SCHEMA = AIngle::Schema.parse <<-JSON
  {
    "type": "record",
  ...
  HANDSHAKE_REQUESTOR_WRITER = AIngle::IO::DatumWriter.new(HANDSHAKE_REQUEST_SCHEMA)
  HANDSHAKE_REQUESTOR_READER = AIngle::IO::DatumReader.new(HANDSHAKE_RESPONSE_SCHEMA)
  HANDSHAKE_RESPONDER_WRITER = AIngle::IO::DatumWriter.new(HANDSHAKE_RESPONSE_SCHEMA)
  HANDSHAKE_RESPONDER_READER = AIngle::IO::DatumReader.new(HANDSHAKE_REQUEST_SCHEMA)

  META_SCHEMA = AIngle::Schema.parse('{"type": "map", "values": "bytes"}')
  META_WRITER = AIngle::IO::DatumWriter.new(META_SCHEMA)
  META_READER = AIngle::IO::DatumReader.new(META_SCHEMA)

  SYSTEM_ERROR_SCHEMA = AIngle::Schema.parse('["string"]')
  ...
  class AIngleRemoteException < AIngle::AIngleError; end

  class ConnectionClosedException < AIngle::AIngleError; end
  ...
  class Requestor
    ...
    def request(message_name, request_datum)
      ...
      buffer_encoder = AIngle::IO::BinaryEncoder.new(buffer_writer)
      ...
      buffer_decoder = AIngle::IO::BinaryDecoder.new(StringIO.new(call_response))
    ...
    def write_call_request(message_name, request_datum, encoder)
      ...
      #   * the message name, an AIngle string, followed by
      ...
      unless message
        raise AIngleError, "Unknown message: #{message_name}"
      end
    ...
    def write_request(request_schema, request_datum, encoder)
      datum_writer = AIngle::IO::DatumWriter.new(request_schema)
    ...
    def read_handshake_response(decoder)
      ...
      when 'CLIENT'
        raise AIngleError.new('Handshake failure. match == CLIENT') if send_protocol
        self.remote_protocol = AIngle::Protocol.parse(handshake_response['serverProtocol'])
        self.remote_hash = handshake_response['serverHash']
        self.send_protocol = false
        we_have_matching_schema = true
      when 'NONE'
        raise AIngleError.new('Handshake failure. match == NONE') if send_protocol
        self.remote_protocol = AIngle::Protocol.parse(handshake_response['serverProtocol'])
        self.remote_hash = handshake_response['serverHash']
        self.send_protocol = true
      else
        raise AIngleError.new("Unexpected match: #{match}")
      end
    ...
    def read_call_response(message_name, decoder)
      ...
      raise AIngleError.new("Unknown remote message: #{message_name}") unless remote_message_schema

      # local response schema
      local_message_schema = local_protocol.messages[message_name]
      unless local_message_schema
        raise AIngleError.new("Unknown local message: #{message_name}")
      end
    ...
    def read_error(writers_schema, readers_schema, decoder)
      datum_reader = AIngle::IO::DatumReader.new(writers_schema, readers_schema)
      AIngleRemoteError.new(datum_reader.read(decoder))
    end
  end
  ...
  class Responder
    ...
    def respond(call_request, transport=nil)
      buffer_decoder = AIngle::IO::BinaryDecoder.new(StringIO.new(call_request))
      buffer_writer = StringIO.new(''.force_encoding('BINARY'))
      buffer_encoder = AIngle::IO::BinaryEncoder.new(buffer_writer)
      error = nil
      response_metadata = {}
      ...
      begin
        ...
        unless remote_message
          raise AIngleError.new("Unknown remote message: #{remote_message_name}")
        end
        local_message = local_protocol.messages[remote_message_name]
        unless local_message
          raise AIngleError.new("Unknown local message: #{remote_message_name}")
        end
        ...
        rescue AIngleRemoteError => e
          error = e
        rescue Exception => e # rubocop:disable Lint/RescueException
          error = AIngleRemoteError.new(e.to_s)
        end
      ...
      rescue AIngle::AIngleError => e
        error = AIngleRemoteException.new(e.to_s)
        # TODO does the stuff written here ever get used?
        buffer_encoder = AIngle::IO::BinaryEncoder.new(StringIO.new)
        META_WRITER.write(response_metadata, buffer_encoder)
    ...
    def process_handshake(decoder, encoder, connection=nil)
      ...
      if !remote_protocol && client_protocol
        remote_protocol = AIngle::Protocol.parse(client_protocol)
  ...
  class HTTPTransceiver
    ...
    def transceive(message)
      ...
      resp = @conn.post('/', writer.to_s, {'Content-Type' => 'aingle/binary'})
