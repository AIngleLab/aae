...
EOS
  ...
  def test_record_with_nil
    schema = AIngle::Schema.parse('{"type":"record", "name":"rec", "fields":[{"type":"int", "name":"i"}]}')
    assert_raise(AIngle::IO::AIngleTypeError) do
      write_datum(nil, schema)
    end
  end

  def test_array_with_nil
    schema = AIngle::Schema.parse('{"type":"array", "items":"int"}')
    assert_raise(AIngle::IO::AIngleTypeError) do
      write_datum(nil, schema)
    end
  end
  ...
  def test_map_with_nil
    schema = AIngle::Schema.parse('{"type":"map", "values":"long"}')
    assert_raise(AIngle::IO::AIngleTypeError) do
      write_datum(nil, schema)
    end
  end
  ...
  def test_enum_with_duplicate
    ...
    assert_raises(AIngle::SchemaParseError.new('Duplicate symbol: ["AA", "AA"]')) do
      AIngle::Schema.parse str
    end
  end
  ...
  def test_binary_int_encoding
    for value, hex_encoding in BINARY_INT_ENCODINGS
      ...
      buffer = StringIO.new
      encoder = AIngle::IO::BinaryEncoder.new(buffer)
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse('"int"'))
      datum_writer.write(value, encoder)

      buffer.seek(0)
      hex_val = aingle_hexlify(buffer)
  ...
  def test_binary_long_encoding
    for value, hex_encoding in BINARY_INT_ENCODINGS
      buffer = StringIO.new
      encoder = AIngle::IO::BinaryEncoder.new(buffer)
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse('"long"'))
      datum_writer.write(value, encoder)
  ...
  def test_utf8_string_encoding
    [
      ...
      encoder = AIngle::IO::BinaryEncoder.new(StringIO.new(output))
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse('"string"'))
      datum_writer.write(value, encoder)
  ...
  def test_bytes_encoding
    [
      ...
      output = String.new('', encoding: 'BINARY')
      encoder = AIngle::IO::BinaryEncoder.new(StringIO.new(output))
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse('"bytes"'))
      datum_writer.write(value, encoder)
  ...
  def test_fixed_encoding
    [
      ...
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse(schema))
      datum_writer.write(value, encoder)
  ...
  def test_skip_long
    for value_to_skip, _hex_encoding in BINARY_INT_ENCODINGS
      ...
      writer = StringIO.new
      encoder = AIngle::IO::BinaryEncoder.new(writer)
      datum_writer = AIngle::IO::DatumWriter.new(AIngle::Schema.parse('"long"'))
      ...
      decoder = AIngle::IO::BinaryDecoder.new(reader)
      decoder.skip_long()

      # read data from string buffer
      datum_reader = AIngle::IO::DatumReader.new(AIngle::Schema.parse('"long"'))
  ...
  def test_skip_int
    for value_to_skip, _hex_encoding in BINARY_INT_ENCODINGS
      ...
      datum_reader = AIngle::IO::DatumReader.new(AIngle::Schema.parse('"int"'))
      read_value = datum_reader.read(decoder)
  ...
  def test_skip_union
    ["hello", -1, 32, nil].each do |value_to_skip|
      ...
      schema = AIngle::Schema.parse('["int", "string", "null"]')
      writer = StringIO.new
      encoder = AIngle::IO::BinaryEncoder.new(writer)
      datum_writer = AIngle::IO::DatumWriter.new(schema)
      datum_writer.write(value_to_skip, encoder)
      datum_writer.write(value_to_read, encoder)

      reader = StringIO.new(writer.string)
      decoder = AIngle::IO::BinaryDecoder.new(reader)
      datum_reader = AIngle::IO::DatumReader.new(schema)
  ...
  def test_schema_promotion
    ...
    promotable_schemas.each_with_index do |ws, i|
      writers_schema = AIngle::Schema.parse(ws)
      datum_to_write = 219
      for rs in promotable_schemas[(i + 1)..-1]
        readers_schema = AIngle::Schema.parse(rs)
  ...
  def test_interchangeable_schemas
    ...
    interchangeable_schemas.each_with_index do |ws, i|
      writers_schema = AIngle::Schema.parse(ws)
      datum_to_write = 'foo'
      readers_schema = AIngle::Schema.parse(interchangeable_schemas[i == 0 ? 1 : 0])
  ...
  def test_unknown_enum_symbol
    writers_schema = AIngle::Schema.parse(<<-SCHEMA)
      {
        "type": "enum",
        "name": "test",
        "symbols": ["B", "C"]
    ...
    SCHEMA
    readers_schema = AIngle::Schema.parse(<<-SCHEMA)
      {
        "type": "enum",
        "name": "test",
        "symbols": ["A", "B"]
  ...
  def test_array_schema_promotion
    writers_schema = AIngle::Schema.parse('{"type":"array", "items":"int"}')
    readers_schema = AIngle::Schema.parse('{"type":"array", "items":"long"}')
    datum_to_write = [1, 2]
    writer, * = write_datum(datum_to_write, writers_schema)
    datum_read = read_datum(writer, writers_schema, readers_schema)
    assert_equal(datum_read, datum_to_write)
  ...
  def test_map_schema_promotion
    writers_schema = AIngle::Schema.parse('{"type":"map", "values":"int"}')
    readers_schema = AIngle::Schema.parse('{"type":"map", "values":"long"}')
  ...
  def test_snappy_backward_compat
    ...
    snappy = AIngle::DataFile::SnappyCodec.new
  ...
  def check_no_default(schema_json)
    ...
    actual = AIngle::Schema.parse(actual_schema)
...
EOS
    expected = AIngle::Schema.parse(expected_schema)

    reader = AIngle::IO::DatumReader.new(actual, expected)
    assert_raise AIngle::AIngleError do
      value = reader.read(AIngle::IO::BinaryDecoder.new(StringIO.new))
...
EOS
    ...
    reader = AIngle::IO::DatumReader.new(actual, expected)
    record = reader.read(AIngle::IO::BinaryDecoder.new(StringIO.new))
    assert_equal default_value, record["f"]
  end

  def check(str)
    # parse schema, then convert back to string
    schema = AIngle::Schema.parse str
    ...
    # test __eq__
    assert_equal(schema, AIngle::Schema.parse(str))
    ...
    # check that AIngleError is raised when there is no default
  ...
  def checkser(schm, randomdata)
    ...
    w = AIngle::IO::DatumWriter.new(schm)
    writer = StringIO.new(+"", "w")
    w.write(datum, AIngle::IO::BinaryEncoder.new(writer))
    r = datum_reader(schm)
    reader = StringIO.new(writer.string)
    ob = r.read(AIngle::IO::BinaryDecoder.new(reader))
  ...
  def check_datafile(schm)
    ...
    dw = AIngle::DataFile::Writer.new(f, datum_writer(schm), schm)
    ...
    dr = AIngle::DataFile::Reader.new(f, datum_reader(schm))
  ...
  def validate(schm, datum)
    AIngle::Schema.validate(schm, datum)
  end

  def datum_writer(schm)
    AIngle::IO::DatumWriter.new(schm)
  end

  def datum_reader(schm)
    AIngle::IO::DatumReader.new(schm)
  end

  def write_datum(datum, writers_schema)
    writer = StringIO.new
    encoder = AIngle::IO::BinaryEncoder.new(writer)
    datum_writer = AIngle::IO::DatumWriter.new(writers_schema)
  ...
  def read_datum(buffer, writers_schema, readers_schema=nil)
    ...
    decoder = AIngle::IO::BinaryDecoder.new(reader)
    datum_reader = AIngle::IO::DatumReader.new(writers_schema, readers_schema)
