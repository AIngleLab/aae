...
class TestLogicalTypes < Test::Unit::TestCase
  def test_int_date
    ...
    assert_preencoded AIngle::LogicalTypes::IntDate.encode(today), schema, today
  end

  def test_int_date_conversion
    type = AIngle::LogicalTypes::IntDate
  ...
  def test_timestamp_millis_long
    ...
    assert_preencoded AIngle::LogicalTypes::TimestampMillis.encode(time), schema, time.utc
  end

  def test_timestamp_millis_long_conversion
    type = AIngle::LogicalTypes::TimestampMillis
  ...
  def test_timestamp_micros_long
    ...
    assert_encode_and_decode time, schema
    assert_preencoded AIngle::LogicalTypes::TimestampMicros.encode(time), schema, time.utc
  end

  def test_timestamp_micros_long_conversion
    type = AIngle::LogicalTypes::TimestampMicros
  ...
  def test_bytes_decimal
    ...
    assert_raise ArgumentError do
      type = AIngle::LogicalTypes::BytesDecimal.new(schema)
      type.encode('1.23')
    end
  end
  ...
  def test_logical_type_with_schema
    exception = assert_raises(ArgumentError) do
      AIngle::LogicalTypes::LogicalTypeWithSchema.new(nil)
    end
    ...
    assert_nothing_raised do
      AIngle::LogicalTypes::LogicalTypeWithSchema.new(schema)
    end

    assert_raises NotImplementedError do
      AIngle::LogicalTypes::LogicalTypeWithSchema.new(schema).encode(BigDecimal('2'))
    end

    assert_raises NotImplementedError do
      AIngle::LogicalTypes::LogicalTypeWithSchema.new(schema).decode('foo')
    end
  end
  ...
  def decode(encoded, schema)
    ...
    decoder = AIngle::IO::BinaryDecoder.new(buffer)

    datum_reader = AIngle::IO::DatumReader.new(schema, schema)
