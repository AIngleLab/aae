...
class TestSchemaCompatibility < Test::Unit::TestCase
  ...
  def test_primitive_schema_compatibility
    AIngle::Schema::PRIMITIVE_TYPES.each do |schema_type|
  ...
  def test_missing_field
    reader_schema = AIngle::Schema.parse <<-SCHEMA
      {"type":"record", "name":"Record", "fields":[
        {"name":"oldfield1", "type":"int"}
      ]}
    SCHEMA
  ...
  def test_incompatible_record_field
    string_schema = AIngle::Schema.parse <<-SCHEMA
      {"type":"record", "name":"MyRecord", "namespace":"ns", "fields": [
        {"name":"field1", "type":"string"}
      ]}
    SCHEMA
    int_schema = AIngle::Schema.parse <<-SCHEMA2
      {"type":"record", "name":"MyRecord", "namespace":"ns", "fields": [
  ...
  def test_enum_symbols
    enum_schema1 = AIngle::Schema.parse <<-SCHEMA
      {"type":"enum", "name":"MyEnum", "symbols":["A","B"]}
    SCHEMA
    enum_schema2 = AIngle::Schema.parse <<-SCHEMA
  ...
  def test_crossed_aliases
    writer_schema = AIngle::Schema.parse(<<-SCHEMA)
      {"type":"record", "name":"Record", "fields":[
        {"name":"field1", "type": "int"},
  ...
  def test_bytes_decimal
    bytes_decimal_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":4, "scale":4}')
    bytes2_decimal_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":4, "scale":4}')
    bytes_decimal_different_precision_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":5, "scale":4}')
    bytes_decimal_no_scale_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":4}')
    bytes2_decimal_no_scale_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":4}')
    bytes_decimal_zero_scale_schema = AIngle::Schema.
      parse('{"type":"bytes", "logicalType":"decimal", "precision":4, "scale":0}')
    bytes_unknown_logical_type_schema = AIngle::Schema.
  ...
  def test_decimal_different_types
    fixed_decimal_schema = AIngle::Schema.
      parse('{"type":"fixed", "size":2, "name":"Fixed1", "logicalType":"decimal", "precision":4, "scale":2}')
    fixed_decimal_scale4_schema = AIngle::Schema.
  ...
  # Tests from lang/java/aingle/src/test/java/org/apache/aingle/io/parsing/TestResolvingGrammarGenerator2.java
  ...
  def can_read?(writer, reader)
    AIngle::SchemaCompatibility.can_read?(writer, reader)
  end

  def union_schema(*schemas)
    schemas ||= []
    AIngle::Schema.parse("[#{schemas.map(&:to_s).join(',')}]")
  end

  AIngle::Schema::PRIMITIVE_TYPES.each do |schema_type|
    define_method("#{schema_type}_schema") do
      AIngle::Schema.parse("\"#{schema_type}\"")
    end
  end

  def int_array_schema
    AIngle::Schema.parse('{"type":"array", "items":"int"}')
  end

  def long_array_schema
    AIngle::Schema.parse('{"type":"array", "items":"long"}')
  end

  def string_array_schema
    AIngle::Schema.parse('{"type":"array", "items":"string"}')
  end

  def int_map_schema
    AIngle::Schema.parse('{"type":"map", "values":"int"}')
  end

  def long_map_schema
    AIngle::Schema.parse('{"type":"map", "values":"long"}')
  end

  def string_map_schema
    AIngle::Schema.parse('{"type":"map", "values":"string"}')
  end

  def enum1_ab_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum1", "symbols":["A","B"]}')
  end

  def enum1_ab_default_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum1", "symbols":["A","B"], "default":"A"}')
  end

  def enum1_ab_aliased_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum2", "aliases":["Enum1"], "symbols":["A","B"]}')
  end

  def enum1_abc_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum1", "symbols":["A","B","C"]}')
  end

  def enum1_bc_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum1", "symbols":["B","C"]}')
  end

  def enum2_ab_schema
    AIngle::Schema.parse('{"type":"enum", "name":"Enum2", "symbols":["A","B"]}')
  end

  def fixed1_schema
    AIngle::Schema.parse('{"type":"fixed", "name":"Fixed1", "size": 2}')
  end

  def fixed1_aliased_schema
    AIngle::Schema.parse('{"type":"fixed", "name":"Fixed2", "aliases":["Fixed1"], "size": 2}')
  end

  def fixed2_schema
    AIngle::Schema.parse('{"type":"fixed", "name":"Fixed2", "size": 2}')
  end

  def fixed1_size3_schema
    AIngle::Schema.parse('{"type":"fixed", "name":"Fixed1", "size": 3}')
  end

  def empty_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1"}')
  end

  def empty_record1_aliased_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record2", "aliases":["Record1"]}')
  end

  def empty_record2_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record2"}')
  end

  def a_int_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"int"}]}')
  end

  def a_long_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"long"}]}')
  end

  def a_int_b_int_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"int"}, {"name":"b", "type":"int"}]}')
  end

  def a_dint_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"int", "default":0}]}')
  end

  def a_int_b_dint_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"int"}, {"name":"b", "type":"int", "default":0}]}')
  end

  def a_dint_b_dint_record1_schema
    AIngle::Schema.parse('{"type":"record", "name":"Record1", "fields":[{"name":"a", "type":"int", "default":0}, {"name":"b", "type":"int", "default":0}]}')
  end

  def nested_record
    AIngle::Schema.parse('{"type":"record","name":"parent","fields":[{"name":"attribute","type":{"type":"record","name":"child","fields":[{"name":"id","type":"string"}]}}]}')
  end

  def nested_optional_record
    AIngle::Schema.parse('{"type":"record","name":"parent","fields":[{"name":"attribute","type":["null",{"type":"record","name":"child","fields":[{"name":"id","type":"string"}]}],"default":null}]}')
  end
