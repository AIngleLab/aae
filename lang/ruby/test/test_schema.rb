...
class TestSchema < Test::Unit::TestCase
  def hash_to_schema(hash)
    AIngle::Schema.parse(hash.to_json)
  end

  def test_default_namespace
    schema = AIngle::Schema.parse <<-SCHEMA
  ...
  def test_to_aingle_includes_namespaces
    schema = AIngle::Schema.parse <<-SCHEMA
      {"type": "record", "name": "my.name.space.OuterRecord", "fields": [
        {"name": "definition", "type": {
  ...
  def test_to_aingle_includes_logical_type
    schema = AIngle::Schema.parse <<-SCHEMA
  ...
  def test_to_aingle_includes_aliases
    hash = {
      'type' => 'record',
      'name' => 'test_record',
    ...
    assert_equal(schema.to_aingle, hash)
  end

  def test_unknown_named_type
    error = assert_raise AIngle::UnknownSchemaError do
  ...
  def test_invalid_name
    error = assert_raise AIngle::SchemaParseError do
      AIngle::Schema.parse <<-SCHEMA
  ...
  def test_invalid_name_with_validation_disabled
    AIngle.disable_schema_name_validation = true
    ...
    AIngle.disable_schema_name_validation = false
  end

  def test_to_aingle_handles_falsey_defaults
    schema = AIngle::Schema.parse <<-SCHEMA
  ...
  def test_record_field_doc_attribute
    field_schema_json = AIngle::Schema.parse <<-SCHEMA
      {
        "type": "record",
        "name": "Record",
    ...
    assert_equal field_schema_hash, field_schema_json.to_aingle
  end

  def test_record_doc_attribute
    record_schema_json = AIngle::Schema.parse <<-SCHEMA
      {
        "type": "record",
        "name": "Record",
        "namespace": "my.name.space",
    ...
    assert_equal record_schema_hash, record_schema_json.to_aingle
  end

  def test_enum_doc_attribute
    enum_schema_json = AIngle::Schema.parse <<-SCHEMA
      {
        "type": "enum",
        "name": "Enum",
        "namespace": "my.name.space",
    ...
    assert_equal enum_schema_hash, enum_schema_json.to_aingle
  end

  def test_enum_default_attribute
    enum_schema = AIngle::Schema.parse <<-SCHEMA
      {
        "type": "enum",
        "name": "fruit",
        "default": "apples",
    ...
    assert_equal(enum_schema_hash, enum_schema.to_aingle)
  end

  def test_validate_enum_default
    exception = assert_raise(AIngle::SchemaParseError) do
      hash_to_schema(
        type: 'enum',
        name: 'fruit',
        default: 'bananas',
  ...
  def test_empty_record
    schema = AIngle::Schema.parse('{"type":"record", "name":"Empty"}')
  ...
  def test_read
    schema = AIngle::Schema.parse('"string"')
    writer_schema = AIngle::Schema.parse('"int"')
  ...
  def test_mutual_read
    schema = AIngle::Schema.parse('"string"')
    writer_schema = AIngle::Schema.parse('"int"')
    default1 = AIngle::Schema.parse('{"type":"record", "name":"Default", "fields":[{"name":"i", "type":"int", "default": 1}]}')
    default2 = AIngle::Schema.parse('{"type":"record", "name":"Default", "fields":[{"name:":"s", "type":"string", "default": ""}]}')
  ...
  def test_field_default_validation_disabled
    AIngle.disable_field_default_validation = true
    assert_nothing_raised do
      hash_to_schema(
        type: 'record',
        name: 'fruits',
    ...
    AIngle.disable_field_default_validation = false
  end

  def test_field_default_validation_disabled_via_env
    AIngle.disable_field_default_validation = false
    ENV['AINGLE_DISABLE_FIELD_DEFAULT_VALIDATION'] = "1"
    ...
    ENV.delete('AINGLE_DISABLE_FIELD_DEFAULT_VALIDATION')
    AIngle.disable_field_default_validation = false
  end

  def test_validate_record_valid_default
    assert_nothing_raised(AIngle::SchemaParseError) do
      hash_to_schema(
        type: 'record',
        name: 'with_subrecord',
        fields: [
  ...
  def test_fixed_decimal_to_include_precision_scale
    ...
    assert_equal schema_hash, schema.to_aingle
  end
  ...
  def test_bytes_schema
    ...
    schema_str = 'bytes'
    assert_equal schema_str, schema.to_aingle
  end
  ...
  def test_validate_enum_symbols
    ...
    assert_equal(
      "Invalid symbols for things: 8ad_symbol, also-bad-symbol, >=, $ don't match #{AIngle::Schema::EnumSchema::SYMBOL_REGEX.inspect}",
      exception.to_s
    )
  end

  def test_enum_symbol_validation_disabled_via_env
    AIngle.disable_enum_symbol_validation = nil
    ENV['AINGLE_DISABLE_ENUM_SYMBOL_VALIDATION'] = '1'
    ...
    ENV.delete('AINGLE_DISABLE_ENUM_SYMBOL_VALIDATION')
    AIngle.disable_enum_symbol_validation = nil
  end

  def test_enum_symbol_validation_disabled_via_class_method
    AIngle.disable_enum_symbol_validation = true
