...
JSON
    ...
    AIngle::DataFile.open('data.avr', 'w', writer_schema) do |dw|
      data.each{|h| dw << h }
    end

    # extract the username only from the aingle serialized file
    reader_schema = <<-JSON
{ "type": "record",
  "name": "User",
  "fields" : [
...
JSON
    ...
    AIngle::DataFile.open('data.avr', 'r', reader_schema) do |dr|
...
JSON
    ...
    %w[fixed enum record error array map union].each do |s|
      ...
      AIngle::DataFile.open('data.avr', 'r', MultiJson.dump(reader)) do |dr|
...
JSON
    ...
    AIngle::DataFile.open('data.avr', 'w', writer_schema) do |dw|
      while dw.writer.tell < AIngle::DataFile::SYNC_INTERVAL
        dw << data
      end
  ...
  def test_utf8
    datafile = AIngle::DataFile::open('data.avr', 'w', '"string"')
    datafile << "家"
    datafile.close

    datafile = AIngle::DataFile.open('data.avr')
    datafile.each do |s|
      assert_equal "家", s
    end
    datafile.close
  ...
  def test_deflate
    AIngle::DataFile.open('data.avr', 'w', '"string"', :deflate) do |writer|
      writer << 'a' * 10_000
    end
    assert(File.size('data.avr') < 500)

    records = []
    AIngle::DataFile.open('data.avr') do |reader|
  ...
  def test_snappy
    AIngle::DataFile.open('data.avr', 'w', '"string"', :snappy) do |writer|
      writer << 'a' * 10_000
    end
  ...
  def test_zstandard
    AIngle::DataFile.open('data.avr', 'w', '"string"', :zstandard) do |writer|
      writer << 'a' * 10_000
    end
    assert(File.size('data.avr') < 600)
  ...
  def test_append_to_deflated_file
    ...
    file = AIngle::DataFile::Writer.new(File.open('data.avr', 'wb'), writer, schema, :deflate)
    file << 'a' * 10_000
    file.close

    file = AIngle::DataFile::Writer.new(File.open('data.avr', 'a+b'), writer)
    file << 'b' * 10_000
    file.close
  ...
  def test_custom_meta
    ...
    schema = AIngle::Schema.parse('"string"')
    writer = AIngle::IO::DatumWriter.new(schema)
    file = AIngle::DataFile::Writer.new(File.open('data.avr', 'wb'), writer, schema, nil, meta)
    file.close
