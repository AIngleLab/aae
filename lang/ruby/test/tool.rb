...
class GenericResponder < AIngle::IPC::Responder
  def initialize(proto, msg, datum)
    ...
    super(AIngle::Protocol.parse(proto_json))
    @msg = msg
    @datum = datum
  end
...
class GenericHandler < WEBrick::HTTPServlet::AbstractServlet
  def do_POST(req, resp)
    call_request = AIngle::IPC::FramedReader.new(StringIO.new(req.body)).read_framed_message
    unframed_resp = $responder.respond(call_request)
    writer = AIngle::IPC::FramedWriter.new(StringIO.new)
...
def send_message(uri, proto, msg, datum)
  uri = URI.parse(uri)
  trans = AIngle::IPC::HTTPTransceiver.new(uri.host, uri.port)
  proto_json = File.open(proto).read
  requestor = AIngle::IPC::Requestor.new(AIngle::Protocol.parse(proto_json),
                                       trans)
...
def main
  ...
  when "dump"
    ...
    d = AIngle::DataFile.new(file_or_stdin(ARGV[1]), AIngle::IO::DatumReader.new)
  ...
  when "rpcreceive"
    ...
    if ARGV.size > 4
      case ARGV[4]
      when "-file"
        AIngle::DataFile.open(ARGV[5]) do |f|
          datum = f.first
        end
      when "-data"
  ...
  when "rpcsend"
    ...
    if ARGV.size > 4
      case ARGV[4]
      when "-file"
        AIngle::DataFile.open(ARGV[5]){ |f| datum = f.first }
      when "-data"
        puts "JSON Decoder not yet implemented"
        return 1
      else
