...
class TestSchemaValidator < Test::Unit::TestCase
  def validate!(schema, value, options = {})
    AIngle::SchemaValidator.validate!(schema, value, options)
  end

  def validate_simple!(schema, value)
    AIngle::SchemaValidator.validate!(schema, value, recursive: false)
  end
  ...
  def assert_failed_validation(messages)
    error = assert_raise(AIngle::SchemaValidator::ValidationError) { yield }
  ...
  def assert_valid_schema(schema, valid, invalid, simple = false)
    valid.each do |value|
      assert_nothing_raised { AIngle::SchemaValidator.validate!(schema, value) }
      assert_nothing_raised { AIngle::SchemaValidator.validate!(schema, value, recursive: false) } if simple
    end

    invalid.each do |value|
      assert_raise { AIngle::SchemaValidator.validate!(schema, value) }
      assert_raise { AIngle::SchemaValidator.validate!(schema, value, recursive: false) } if simple
      assert_nothing_raised { AIngle::SchemaValidator.validate!(schema, value, recursive: false) } unless simple
    end
  end
  ...
  def test_validate_int
    ...
    assert_valid_schema(
      schema,
      [AIngle::Schema::INT_MIN_VALUE, AIngle::Schema::INT_MAX_VALUE, 1],
      [AIngle::Schema::LONG_MIN_VALUE, AIngle::Schema::LONG_MAX_VALUE, 'string'],
      true
    )
    assert_failed_validation('at . out of bound value 9223372036854775807') do
      validate!(schema, AIngle::Schema::LONG_MAX_VALUE)
    end
    assert_failed_validation('at . out of bound value 9223372036854775807') do
      validate_simple!(schema, AIngle::Schema::LONG_MAX_VALUE)
    end
  end
  ...
  def test_validate_long
    ...
    assert_valid_schema(schema, [AIngle::Schema::LONG_MIN_VALUE, AIngle::Schema::LONG_MAX_VALUE, 1], [1.1, 'string'], true)
  end

  def test_validate_float
    schema = hash_to_schema(type: 'float', name: 'name')

    assert_valid_schema(schema, [1.1, 1, AIngle::Schema::LONG_MAX_VALUE], ['string'], true)
  end
  ...
  def test_validate_multiple_errors
    ...
    exception = assert_raise(AIngle::SchemaValidator::ValidationError) do
