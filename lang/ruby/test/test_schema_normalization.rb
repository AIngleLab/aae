...
class TestSchemaNormalization < Test::Unit::TestCase
  def test_primitives
    %w[null boolean string bytes int long float double].each do |type|
      schema = AIngle::Schema.parse(<<-JSON)
        { "type": "#{type}" }
      JSON

      canonical_form = AIngle::SchemaNormalization.to_parsing_form(schema)
  ...
  def test_shared_dataset
    CaseFinder.cases.each do |test_case|
      schema = AIngle::Schema.parse(test_case.input)
      assert_equal test_case.canonical, AIngle::SchemaNormalization.to_parsing_form(schema)
      assert_equal test_case.fingerprint, schema.crc_64_aingle_fingerprint
    end
  end
end
