...
import org.apache.aingle.grpc.test.TestError;
...
public class TestAIngleProtocolGrpc {
  ...
  private void setUpServerAndClient(TestService serviceImpl) throws IOException {
    ...
    server = ServerBuilder.forPort(0).addService(AIngleGrpcServer.createServiceDefinition(TestService.class, serviceImpl))
        .build();
    server.start();
    int port = server.getPort();
    channel = ManagedChannelBuilder.forAddress("localhost", port).usePlaintext().build();
    stub = AIngleGrpcClient.create(channel, TestService.class);
    callbackStub = AIngleGrpcClient.create(channel, TestService.Callback.class);
  }
  ...
  public void testConcurrentChannels() throws Exception {
    ManagedChannel otherChannel = ManagedChannelBuilder.forAddress("localhost", server.getPort()).usePlaintext()
        .build();
    TestService otherStub = AIngleGrpcClient.create(otherChannel, TestService.class);
    Future<Integer>[] adds = new Future[5];
