...
public abstract class Requestor {
   ...
   * @throws AIngleRemoteException  if an exception is thrown to client by server.
   * @throws IOException          if an I/O error occurs while sending the
   *                              message.
   * @throws AIngleRuntimeException for another undeclared error while sending the
   *                              message.
   */
  public <T> void request(String messageName, Object request, Callback<T> callback)
      throws AIngleRemoteException, IOException {
  ...
  <T> void request(Request request, Callback<T> callback) throws AIngleRemoteException, IOException {
          ...
          if (request.getMessage().isOneWay()) {
            ...
            if (error != null) {
              if (error instanceof AIngleRemoteException) {
                throw (AIngleRemoteException) error;
              } else if (error instanceof AIngleRuntimeException) {
                throw (AIngleRuntimeException) error;
  ...
  private boolean readHandshake(Decoder in) throws IOException {
    ...
    case NONE:
      ...
      break;
    default:
      throw new AIngleRuntimeException("Unexpected match: " + handshake.getMatch());
    }
  ...
  class Response {
    ...
    public Object getResponse() throws Exception {
      ...
      if (rm == null)
        throw new AIngleRuntimeException("Not a remote message: " + request.getMessageName());

      Transceiver t = getTransceiver();
      if ((lm.isOneWay() != rm.isOneWay()) && t.isConnected())
        throw new AIngleRuntimeException("Not both one-way messages: " + request.getMessageName());
