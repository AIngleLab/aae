...
/**
 * Read files written with {@link AIngleColumnWriter}. A subset of the schema used
 * for writing may be specified when reading. In this case only columns of the
 * subset schema are read.
 */
public class AIngleColumnReader<D> implements Iterator<D>, Iterable<D>, Closeable {
  ...
  /** Parameters for reading an AIngle column file. */
  ...
  public AIngleColumnReader(Params params) throws IOException {
    this.reader = new ColumnFileReader(params.input);
    this.model = params.model;
    this.fileSchema = new Schema.Parser().parse(reader.getMetaData().getString(AIngleColumnWriter.SCHEMA_KEY));
  ...
  void initialize() throws IOException {
    ...
    int i = 0;
    for (ColumnMetaData c : new AIngleColumnator(fileSchema).getColumns())
      fileColumnNumbers.put(c.getName(), i++);

    // create iterator for each column in readSchema
    AIngleColumnator readColumnator = new AIngleColumnator(readSchema);
