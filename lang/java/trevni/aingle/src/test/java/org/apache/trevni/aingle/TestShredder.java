...
public class TestShredder {
  ...
  private void check(Schema s, ColumnMetaData... expected) throws Exception {
    ColumnMetaData[] shredded = new AIngleColumnator(s).getColumns();
  ...
  private void checkRead(Schema schema) throws IOException {
    AIngleColumnReader<Object> reader = new AIngleColumnReader<>(new AIngleColumnReader.Params(FILE).setSchema(schema));
