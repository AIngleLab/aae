...
public class BlockingBinaryEncoder extends BufferedBinaryEncoder {
  ...
  public void writeArrayEnd() throws IOException {
    ...
    if (top.type != Schema.Type.ARRAY) {
      throw new AIngleTypeException("Called writeArrayEnd outside of an array.");
    }
    if (top.itemsLeftToWrite != 0) {
      throw new AIngleTypeException("Failed to write expected number of array elements.");
    }
    endBlockedValue();
  ...
  public void writeMapEnd() throws IOException {
    ...
    if (top.type != Schema.Type.MAP) {
      throw new AIngleTypeException("Called writeMapEnd outside of a map.");
    }
    if (top.itemsLeftToWrite != 0) {
      throw new AIngleTypeException("Failed to read write expected number of array elements.");
    }
    endBlockedValue();
