...
public class ReflectionUtil {
  ...
  static void resetFieldAccess() {
    ...
    try {
      if (null == System.getProperty("aingle.disable.unsafe")) {
        FieldAccess unsafeAccess = load("org.apache.aingle.reflect.FieldAccessUnsafe", FieldAccess.class);
    ...
    if (access == null) {
      try {
        FieldAccess reflectAccess = load("org.apache.aingle.reflect.FieldAccessReflect", FieldAccess.class);
        if (validate(reflectAccess)) {
          access = reflectAccess;
        }
      } catch (Throwable oops) {
        throw new AIngleRuntimeException("Unable to load a functional FieldAccess class!");
      }
    }
    fieldAccess = access;
  }
