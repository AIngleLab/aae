...
import org.apache.aingle.io.parsing.JsonGrammarGenerator;
...
/**
 * A {@link Decoder} for AIngle's JSON data encoding.
 * </p>
...
public class JsonDecoder extends ParsingDecoder implements Parser.ActionHandler {
  ...
  public void readFixed(byte[] bytes, int start, int len) throws IOException {
    ...
    if (in.getCurrentToken() == JsonToken.VALUE_STRING) {
      ...
      if (result.length != len) {
        throw new AIngleTypeException("Expected fixed length " + len + ", but got" + result.length);
      }
  ...
  private void doSkipFixed(int length) throws IOException {
    if (in.getCurrentToken() == JsonToken.VALUE_STRING) {
      ...
      if (result.length != length) {
        throw new AIngleTypeException("Expected fixed length " + length + ", but got" + result.length);
      }
    } else {
      throw error("fixed");
    }
  ...
  public int readEnum() throws IOException {
    ...
    if (in.getCurrentToken() == JsonToken.VALUE_STRING) {
      ...
      throw new AIngleTypeException("Unknown symbol in enum " + in.getText());
    } else {
      throw error("fixed");
    }
  }
  ...
  public int readIndex() throws IOException {
    ...
    if (n < 0)
      throw new AIngleTypeException("Unknown union branch " + label);
  ...
  public Symbol doAction(Symbol input, Symbol top) throws IOException {
    if (top instanceof Symbol.FieldAdjustAction) {
      ...
      if (in.getCurrentToken() == JsonToken.FIELD_NAME) {
        ...
        throw new AIngleTypeException("Expected field name not found: " + fa.fname);
      }
      ...
      // AINGLE-2034 advance to the end of our object
      ...
      // AINGLE-2034 advance beyond the end object for the next record.
      in.nextToken();

    } else {
      throw new AIngleTypeException("Unknown action symbol " + top);
    }
    return null;
  }

  private AIngleTypeException error(String type) {
    return new AIngleTypeException("Expected " + type + ". Got " + in.getCurrentToken());
  }
