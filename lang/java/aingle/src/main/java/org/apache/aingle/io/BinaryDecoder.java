...
import org.apache.aingle.InvalidNumberEncodingException;
 ...
 * <tt>org.apache.aingle.limits.string.maxLength</tt> and
 * <tt>org.apache.aingle.limits.bytes.maxLength</tt> before instantiating this
...
public class BinaryDecoder extends Decoder {
  ...
  private static final String MAX_BYTES_LENGTH_PROPERTY = "org.apache.aingle.limits.bytes.maxLength";
  ...
  public Utf8 readString(Utf8 old) throws IOException {
    ...
    if (length < 0L) {
      throw new AIngleRuntimeException("Malformed data. Length is negative: " + length);
    }
  ...
  public ByteBuffer readBytes(ByteBuffer old) throws IOException {
    ...
    if (length > maxBytesLength) {
      throw new AIngleRuntimeException("Bytes length " + length + " exceeds maximum allowed");
    }
    if (length < 0L) {
      throw new AIngleRuntimeException("Malformed data. Length is negative: " + length);
    }
   ...
   * AIngle data with other reads must access this InputStream to do so unless the
