...
import org.apache.aingle.util.internal.Accessor.JsonPropertiesAccessor;
import org.apache.aingle.reflect.MapEntry;
...
/**
 * Base class for objects that have JSON-valued properties. AIngle and JSON values
 ...
 * </table>
 *
 * @see org.apache.aingle.data.Json
 */
public abstract class JsonProperties {
  ...
  private void addProp(String name, JsonNode value) {
    if (reserved.contains(name))
      throw new AIngleRuntimeException("Can't set reserved property: " + name);

    if (value == null)
      throw new AIngleRuntimeException("Can't set a property to null: " + name);

    JsonNode old = props.putIfAbsent(name, value);
    if (old != null && !old.equals(value)) {
      throw new AIngleRuntimeException("Can't overwrite property: " + name);
    }
  }
