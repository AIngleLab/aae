...
public class GenericDatumWriter<D> implements DatumWriter<D> {
  ...
  protected <T> Object convert(Schema schema, LogicalType logicalType, Conversion<T> conversion, Object datum) {
    try {
      ...
      if (cause != null && cause.getClass() == ClassCastException.class) {
        ...
        // throw CCE. After being moved to Conversions, it throws AIngleRuntimeException
        ...
        // reading path, which throws AIngleRuntimeException
  ...
  /** Helper method for adding a message to an AIngle Type Exception . */
  protected AIngleTypeException addAIngleTypeMsg(AIngleTypeException e, String s) {
    AIngleTypeException result = new AIngleTypeException(e.getMessage() + s);
    result.initCause(e.getCause() == null ? e : e.getCause());
    return result;
  }
  ...
  protected void writeField(Object datum, Field f, Encoder out, Object state) throws IOException {
    ...
    } catch (AIngleTypeException ate) {
      throw addAIngleTypeMsg(ate, " in field " + f.name());
    }
  }
  ...
  protected void writeEnum(Schema schema, Object datum, Encoder out) throws IOException {
    if (!data.isEnum(datum))
      throw new AIngleTypeException("Not an enum: " + datum + " for schema: " + schema);
  ...
  private void error(Schema schema, Object datum) {
    throw new AIngleTypeException("Not a " + schema + ": " + datum);
  }
