...
public class TestAIngleMultipleOutputs {
  ...
  private static class GenericStatsReducer
    ...
    protected void reduce(Text line, Iterable<IntWritable> counts, Context context)
      ...
      amos.write("myaingle", mStats, NullWritable.get());
  ...
  private static class SpecificStatsReducer extends Reducer<Text, IntWritable, AIngleKey<TextStats>, NullWritable> {
    ...
    protected void reduce(Text line, Iterable<IntWritable> counts, Context context)
      ...
      mStats.datum(record);
      context.write(mStats, NullWritable.get());
      amos.write("myaingle3", mStats, NullWritable.get());
    }
  ...
  public void testAIngleGenericOutput() throws Exception {
    ...
    AIngleMultipleOutputs.addNamedOutput(job, "myaingle", AIngleKeyOutputFormat.class, STATS_SCHEMA, null);
    AIngleMultipleOutputs.addNamedOutput(job, "myaingle1", AIngleKeyOutputFormat.class, STATS_SCHEMA_2);
    job.setOutputFormatClass(AIngleKeyOutputFormat.class);

    Path outputPath = new Path(DIR.getRoot().getPath() + "/testAIngleGenericOutput");
    ...
    FileStatus[] outputFiles = fileSystem.globStatus(outputPath.suffix("/myaingle-r-00000.aingle"));
    ...
    outputFiles = fileSystem.globStatus(outputPath.suffix("/myaingle1-r-00000.aingle"));
    Assert.assertEquals(1, outputFiles.length);
    counts.clear();
    try (DataFileReader<GenericData.Record> reader = new DataFileReader<>(
    ...
    outputFiles = fileSystem.globStatus(outputPath.suffix("/testnewwrite-r-00000.aingle"));
    Assert.assertEquals(1, outputFiles.length);
    counts.clear();
    try (DataFileReader<GenericData.Record> reader = new DataFileReader<>(
        new FsInput(outputFiles[0].getPath(), job.getConfiguration()), new GenericDatumReader<>(STATS_SCHEMA))) {
    ...
    outputFiles = fileSystem.globStatus(outputPath.suffix("/testnewwrite2-r-00000.aingle"));
    Assert.assertEquals(1, outputFiles.length);
    counts.clear();
    try (DataFileReader<GenericData.Record> reader = new DataFileReader<>(
        new FsInput(outputFiles[0].getPath(), job.getConfiguration()), new GenericDatumReader<>(STATS_SCHEMA_2))) {
    ...
    outputFiles = fileSystem.globStatus(outputPath.suffix("/testwritenonschema-r-00000.aingle"));
    Assert.assertEquals(1, outputFiles.length);
    counts.clear();
    try (DataFileReader<GenericData.Record> reader = new DataFileReader<>(
        new FsInput(outputFiles[0].getPath(), job.getConfiguration()), new GenericDatumReader<>(STATS_SCHEMA))) {
  ...
  public void testAIngleSpecificOutput() throws Exception {
    ...
    job.setMapOutputKeyClass(Text.class);
    job.setMapOutputValueClass(IntWritable.class);
    AIngleMultipleOutputs.addNamedOutput(job, "myaingle3", AIngleKeyOutputFormat.class, TextStats.SCHEMA$, null);

    job.setReducerClass(SpecificStatsReducer.class);
    AIngleJob.setOutputKeySchema(job, TextStats.SCHEMA$);

    job.setOutputFormatClass(AIngleKeyOutputFormat.class);
    Path outputPath = new Path(DIR.getRoot().getPath() + "/testAIngleSpecificOutput");
    outputPath.getFileSystem(job.getConfiguration()).delete(outputPath, true);
    FileOutputFormat.setOutputPath(job, outputPath);

    Assert.assertTrue(job.waitForCompletion(true));
    FileSystem fileSystem = FileSystem.get(job.getConfiguration());
    FileStatus[] outputFiles = fileSystem.globStatus(outputPath.suffix("/myaingle3-*"));
    Assert.assertEquals(1, outputFiles.length);
    Map<String, Integer> counts = new HashMap<>();
  ...
  public void testAIngleInput() throws Exception {
    ...
    job.setOutputFormatClass(AIngleKeyOutputFormat.class);
    Path outputPath = new Path(DIR.getRoot().getPath() + "/testAIngleInput");
    FileOutputFormat.setOutputPath(job, outputPath);
  ...
  public void testAIngleMapOutput() throws Exception {
    ...
    job.setOutputFormatClass(AIngleKeyOutputFormat.class);
    Path outputPath = new Path(DIR.getRoot().getPath() + "/testAIngleMapOutput");
    FileOutputFormat.setOutputPath(job, outputPath);
