...
public class TestSortedKeyValueFile {
  ...
  public void testNamedCodecs() throws IOException {
    ...
    Schema recordSchema = AIngleKeyValue.getSchema(key, value);
    ...
    for (String codec : new String[] { "null", "deflate", "snappy", "bzip2" }) {
      ...
      assertEquals(codec, reader.getMetaString("aingle.codec"));
      reader.close();
    }
  }
  ...
  public void testDeflateClassCodec() throws IOException {
    ...
    assertEquals("deflate", reader.getMetaString("aingle.codec"));
    reader.close();
  }
  ...
  public void testWriter() throws IOException {
    ...
    DatumReader<GenericRecord> indexReader = new GenericDatumReader<>(
        AIngleKeyValue.getSchema(options.getKeySchema(), Schema.create(Schema.Type.LONG)));

    List<AIngleKeyValue<CharSequence, Long>> indexRecords = new ArrayList<>();
    try (FileReader<GenericRecord> indexFileReader = DataFileReader.openReader(indexFile, indexReader)) {
      for (GenericRecord indexRecord : indexFileReader) {
        indexRecords.add(new AIngleKeyValue<>(indexRecord));
      }
    }
    ...
    DatumReader<GenericRecord> dataReader = new GenericDatumReader<>(
        AIngleKeyValue.getSchema(options.getKeySchema(), options.getValueSchema()));
    ...
    try (DataFileReader<GenericRecord> dataFileReader = new DataFileReader<>(dataFile, dataReader)) {
      ...
      AIngleKeyValue<CharSequence, CharSequence> appleRecord = new AIngleKeyValue<>(dataFileReader.next());
      ...
      assertTrue(dataFileReader.hasNext());
      AIngleKeyValue<CharSequence, CharSequence> carrotRecord = new AIngleKeyValue<>(dataFileReader.next());
      assertEquals("carrot", carrotRecord.getKey().toString());
      assertEquals("Carrot", carrotRecord.getValue().toString());

      assertTrue(dataFileReader.hasNext());
      AIngleKeyValue<CharSequence, CharSequence> durianRecord = new AIngleKeyValue<>(dataFileReader.next());
