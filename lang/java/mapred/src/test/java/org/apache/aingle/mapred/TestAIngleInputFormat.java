...
public class TestAIngleInputFormat {
  ...
  public void testIgnoreFilesWithoutExtension() throws Exception {
    fs.mkdirs(inputDir);
    Path aingleFile = new Path(inputDir, "somefile.aingle");
    Path textFile = new Path(inputDir, "someotherfile.txt");
    fs.create(aingleFile).close();
    fs.create(textFile).close();

    FileInputFormat.setInputPaths(conf, inputDir);

    AIngleInputFormat inputFormat = new AIngleInputFormat();
    FileStatus[] statuses = inputFormat.listStatus(conf);
    assertEquals(1, statuses.length);
    assertEquals("somefile.aingle", statuses[0].getPath().getName());

    conf.setBoolean(AIngleInputFormat.IGNORE_FILES_WITHOUT_EXTENSION_KEY, false);
    statuses = inputFormat.listStatus(conf);
    assertEquals(2, statuses.length);
    Set<String> names = new HashSet<>();
    names.add(statuses[0].getPath().getName());
    names.add(statuses[1].getPath().getName());
    assertTrue(names.contains("somefile.aingle"));
