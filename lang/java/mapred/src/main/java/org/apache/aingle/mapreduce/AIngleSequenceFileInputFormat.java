...
/**
 * An input format for reading from AIngleSequenceFiles (sequence files that
 * support AIngle data).
 *
 * @param <K> The input key type.
 * @param <V> The input value type.
 */
public class AIngleSequenceFileInputFormat<K, V> extends SequenceFileInputFormat<K, V> {
  /** {@inheritDoc} */
  @Override
  public RecordReader<K, V> createRecordReader(InputSplit inputSplit, TaskAttemptContext context) throws IOException {
    return new AIngleSequenceFileRecordReader();
  }

  /**
   * Reads records from a SequenceFile that supports AIngle data.
   *
   * <p>
   * This class is based on Hadoop's SequenceFileRecordReader, modified to
   * construct an AIngleSequenceFile.Reader instead of a SequenceFile.Reader.
   * </p>
   */
  protected class AIngleSequenceFileRecordReader extends RecordReader<K, V> {
    ...
    public void initialize(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
      ...
      Schema keySchema = AIngleJob.getInputKeySchema(conf);
      if (null != keySchema) {
        options.withKeySchema(keySchema);
      }
      Schema valueSchema = AIngleJob.getInputValueSchema(conf);
      if (null != valueSchema) {
        options.withValueSchema(valueSchema);
      }

      mReader = new AIngleSequenceFile.Reader(options);
