...
import org.apache.aingle.hadoop.util.AIngleCharSequenceComparator;
...
/**
 * A SortedKeyValueFile is an indexed AIngle container file of KeyValue records
 * sorted by key.
 *
 * <p>
 * The SortedKeyValueFile is a directory with two files, named 'data' and
 * 'index'. The 'data' file is an ordinary AIngle container file with records.
 * Each record has exactly two fields, 'key' and 'value'. The keys are sorted
 * lexicographically. The 'index' file is a small AIngle container file mapping
 ...
 * file for each data block in the AIngle container file.
 * </p>
 *
 * <p>
 * SortedKeyValueFile is to AIngle container file as MapFile is to SequenceFile.
 * </p>
 */
public class SortedKeyValueFile {
   ...
   */
  public static class Reader<K, V> implements Closeable, Iterable<AIngleKeyValue<K, V>> {
    ...
    public Reader(Options options) throws IOException {
      ...
      Schema recordSchema = AIngleKeyValue.getSchema(mKeySchema, options.getValueSchema());
    ...
    public V get(K key) throws IOException {
      ...
      for (AIngleKeyValue<K, V> record : this) {
     ...
     */
    @Override
    public Iterator<AIngleKeyValue<K, V>> iterator() {
      return new AIngleKeyValue.Iterator<>(mDataFileReader.iterator());
    }
    ...
    private <K> NavigableMap<K, Long> loadIndexFile(Configuration conf, Path path, Schema keySchema)
      ...
      DatumReader<GenericRecord> datumReader = model
          .createDatumReader(AIngleKeyValue.getSchema(keySchema, Schema.create(Schema.Type.LONG)));
      ...
      try (DataFileReader<GenericRecord> fileReader = new DataFileReader<>(new FsInput(path, conf), datumReader)) {
        if (Schema.create(Schema.Type.STRING).equals(keySchema)) {
          // Because AIngle STRING types are mapped to the Java CharSequence class that does
          // not
          ...
          index = new TreeMap<>(new AIngleCharSequenceComparator<>());
        }
        for (GenericRecord genericRecord : fileReader) {
          AIngleKeyValue<K, Long> indexRecord = new AIngleKeyValue<>(genericRecord);
  ...
  public static class Writer<K, V> implements Closeable {
    ...
    public Writer(Options options) throws IOException {
      ...
      Path dataFilePath = new Path(options.getPath(), DATA_FILENAME);
      LOG.debug("Creating writer for aingle data file: {}", dataFilePath);
      mRecordSchema = AIngleKeyValue.getSchema(mKeySchema, mValueSchema);
      ...
      Path indexFilePath = new Path(options.getPath(), INDEX_FILENAME);
      LOG.debug("Creating writer for aingle index file: {}", indexFilePath);
      mIndexSchema = AIngleKeyValue.getSchema(mKeySchema, Schema.create(Schema.Type.LONG));
    ...
    public void append(K key, V value) throws IOException {
      ...
      AIngleKeyValue<K, V> dataRecord = new AIngleKeyValue<>(new GenericData.Record(mRecordSchema));
      ...
      if (0 == mRecordsWritten++ % mIndexInterval) {
        ...
        AIngleKeyValue<K, Long> indexRecord = new AIngleKeyValue<>(new GenericData.Record(mIndexSchema));
