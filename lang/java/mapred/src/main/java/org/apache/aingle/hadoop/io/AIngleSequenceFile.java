 ...
 * supports reading and writing AIngle data.
 *
 * <p>
 ...
 *
 * <p>
 * Since AIngle data are Plain Old Java Objects (e.g., <code>Integer</code> for
 ...
 * be serialized using AIngle or WritableSerialization.
 * </p>
 *
 * <p>
 * The solution implemented in <code>AIngleSequenceFile</code> is to:
 * </p>
 *
 * <ul>
 * <li>wrap AIngle key data in an <code>AIngleKey</code> object,</li>
 * <li>wrap AIngle value data in an <code>AIngleValue</code> object,</li>
 * <li>configure and register <code>AIngleSerialization</code> with the
 * <code>SerializationFactory</code>, which will accept only objects that are
 * instances of either <code>AIngleKey</code> or <code>AIngleValue</code>, and</li>
 * <li>store the AIngle key and value schemas in the SequenceFile
 * <i>header</i>.</li>
 * </ul>
 */
public class AIngleSequenceFile {
  private static final Logger LOG = LoggerFactory.getLogger(AIngleSequenceFile.class);

  /** The SequenceFile.Metadata field for the AIngle key writer schema. */
  public static final Text METADATA_FIELD_KEY_SCHEMA = new Text("aingle.key.schema");

  /** The SequenceFile.Metadata field for the AIngle value writer schema. */
  public static final Text METADATA_FIELD_VALUE_SCHEMA = new Text("aingle.value.schema");

  /** Constructor disabled for this container class. */
  private AIngleSequenceFile() {
  }
  ...
  public static SequenceFile.Writer createWriter(Writer.Options options) throws IOException {
    return SequenceFile.createWriter(options.getFileSystem(), options.getConfigurationWithAIngleSerialization(),
        ...
        options.getCompressionCodec(), options.getProgressable(), options.getMetadataWithAIngleSchemas());
  }

  /**
   * A writer for an uncompressed SequenceFile that supports AIngle data.
   */
  public static class Writer extends SequenceFile.Writer {
    ...
    public static class Options {
       ...
       *
       * <p>
       * If the keys will be AIngle data, use
       * {@link #withKeySchema(org.apache.aingle.Schema)} to specify the writer schema.
       * The key class will be automatically set to
       * {@link org.apache.aingle.mapred.AIngleKey}.
       * </p>
       *
      ...
      /**
       * Sets the writer schema of the key records when using AIngle data.
       *
       * <p>
       * The key class will automatically be set to
       * {@link org.apache.aingle.mapred.AIngleKey}, so there is no need to call
       ...
       *
       * <p>
       * If the values will be AIngle data, use
       * {@link #withValueSchema(org.apache.aingle.Schema)} to specify the writer
       * schema. The value class will be automatically set to
       * {@link org.apache.aingle.mapred.AIngleValue}.
       * </p>
       *
      ...
      /**
       * Sets the writer schema of the value records when using AIngle data.
       *
       * <p>
       * The value class will automatically be set to
       * {@link org.apache.aingle.mapred.AIngleValue}, so there is no need to call
      ...
      public Options withValueSchema(Schema valueWriterSchema) {
        ...
        withValueClass(AIngleValue.class);
      ...
      /**
       * Gets the Hadoop configuration with AIngle serialization registered.
       *
       * @return The Hadoop configuration.
       */
      public Configuration getConfigurationWithAIngleSerialization() {
        Configuration conf = getConfiguration();
        if (null == conf) {
          throw new RuntimeException("Must call Options.withConfiguration()");
        }

        Configuration confWithAIngle = new Configuration(conf);
        if (null != mKeyWriterSchema) {
          AIngleSerialization.setKeyWriterSchema(confWithAIngle, mKeyWriterSchema);
        }
        if (null != mValueWriterSchema) {
          AIngleSerialization.setValueWriterSchema(confWithAIngle, mValueWriterSchema);
        }
        AIngleSerialization.addToConfiguration(confWithAIngle);
        return confWithAIngle;
      }
       ...
       * AIngle writer schemas.
       *
       * @return The metadata header with AIngle writer schemas if AIngle data is being
       *         written.
       */
      private Metadata getMetadataWithAIngleSchemas() {
    ...
    /**
     * Creates a new <code>Writer</code> to a SequenceFile that supports AIngle data.
     *
     * @param options The writer options.
     * @throws IOException If the writer cannot be initialized.
     */
    public Writer(Options options) throws IOException {
      super(options.getFileSystem(), options.getConfigurationWithAIngleSerialization(), options.getOutputPath(),
          options.getKeyClass(), options.getValueClass(), options.getBufferSizeBytes(), options.getReplicationFactor(),
          options.getBlockSizeBytes(), options.getProgressable(), options.getMetadataWithAIngleSchemas());
    }
  }

  /**
   * A reader for SequenceFiles that may contain AIngle data.
   */
  public static class Reader extends SequenceFile.Reader {
    ...
    public static class Options {
      ...
      /**
       * Sets the reader schema of the key records when using AIngle data.
       *
       * <p>
      ...
      /**
       * Sets the reader schema of the value records when using AIngle data.
       *
       * <p>
       * If not set, the writer schema will be used as the reader schema.
       * </p>
       ...
       *
       * @return The Hadoop configuration.
       * @throws IOException If there is an error configuring AIngle serialization.
       */
      public Configuration getConfigurationWithAIngleSerialization() throws IOException {
        Configuration conf = getConfiguration();
        if (null == conf) {
          throw new RuntimeException("Must call Options.withConfiguration()");
        }

        // Configure schemas and add AIngle serialization to the configuration.
        Configuration confWithAIngle = new Configuration(conf);
        AIngleSerialization.addToConfiguration(confWithAIngle);

        // Read the metadata header from the SequenceFile to get the writer schemas.
        Metadata metadata = AIngleSequenceFile.getMetadata(getFileSystem(), getInputPath(), confWithAIngle);
        ...
        if (null != keySchemaText) {
          ...
          AIngleSerialization.setKeyWriterSchema(confWithAIngle, new Schema.Parser().parse(keySchemaText.toString()));
          if (null != mKeyReaderSchema) {
            AIngleSerialization.setKeyReaderSchema(confWithAIngle, mKeyReaderSchema);
          }
        }
        ...
        if (null != valueSchemaText) {
          ...
          AIngleSerialization.setValueWriterSchema(confWithAIngle, new Schema.Parser().parse(valueSchemaText.toString()));
          if (null != mValueReaderSchema) {
            AIngleSerialization.setValueReaderSchema(confWithAIngle, mValueReaderSchema);
          }
        }
        return confWithAIngle;
      }
    ...
    /**
     * Creates a new <code>Reader</code> from a SequenceFile that supports AIngle
     * data.
     *
    ...
    public Reader(Options options) throws IOException {
      super(options.getFileSystem(), options.getInputPath(), options.getConfigurationWithAIngleSerialization());
    }
  }
