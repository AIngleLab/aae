...
/** Setters to configure jobs for AIngle data. */
public class AIngleJob {
  private AIngleJob() {
  } // no public ctor

  static final String MAPPER = "aingle.mapper";
  static final String COMBINER = "aingle.combiner";
  static final String REDUCER = "aingle.reducer";

  /** The configuration key for a job's input schema. */
  public static final String INPUT_SCHEMA = "aingle.input.schema";
  /** The configuration key for a job's intermediate schema. */
  public static final String MAP_OUTPUT_SCHEMA = "aingle.map.output.schema";
  /** The configuration key for a job's output schema. */
  public static final String OUTPUT_SCHEMA = "aingle.output.schema";
  /**
   ...
   */
  public static final String OUTPUT_CODEC = "aingle.output.codec";
  /** The configuration key prefix for a text output metadata. */
  public static final String TEXT_PREFIX = "aingle.meta.text.";
  /** The configuration key prefix for a binary output metadata. */
  public static final String BINARY_PREFIX = "aingle.meta.binary.";
  /** The configuration key for reflection-based input representation. */
  public static final String INPUT_IS_REFLECT = "aingle.input.is.reflect";
  /** The configuration key for reflection-based map output representation. */
  public static final String MAP_OUTPUT_IS_REFLECT = "aingle.map.output.is.reflect";
  ...
  private static void configureAIngleInput(JobConf job) {
    if (job.get("mapred.input.format.class") == null)
      job.setInputFormat(AIngleInputFormat.class);
  ...
  private static void configureAIngleOutput(JobConf job) {
    ...
    job.setOutputKeyClass(AIngleWrapper.class);
    configureAIngleShuffle(job);
  }

  private static void configureAIngleShuffle(JobConf job) {
    job.setOutputKeyComparatorClass(AIngleKeyComparator.class);
    job.setMapOutputKeyClass(AIngleKey.class);
    job.setMapOutputValueClass(AIngleValue.class);

    // add AIngleSerialization to io.serializations
  ...
  public static void setMapperClass(JobConf job, Class<? extends AIngleMapper> c) {
    job.set(MAPPER, c.getName());
  }

  /** Configure a job's combiner implementation. */
  public static void setCombinerClass(JobConf job, Class<? extends AIngleReducer> c) {
    job.set(COMBINER, c.getName());
    job.setCombinerClass(HadoopCombiner.class);
  }

  /** Configure a job's reducer implementation. */
  public static void setReducerClass(JobConf job, Class<? extends AIngleReducer> c) {
