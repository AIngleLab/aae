 ...
 * job.setReducerClass(MyAIngleReducer.class);
 * ...
 *
 * Schema schema;
 * ...
 * // Defines additional single output 'aingle1' for the job
 * AIngleMultipleOutputs.addNamedOutput(job, "aingle1", AIngleKeyValueOutputFormat.class,
 ...
 * ...
 * amos = new AIngleMultipleOutputs(context);
 * }
 *
 * public void reduce(K, Iterator&lt;V&gt; values,Context context)
 * throws IOException {
 * ...
 * amos.write("aingle1",datum,NullWritable.get());
 * amos.write("aingle2",datum,NullWritable.get());
 * amos.getCollector("aingle3",datum); // here the value is taken as NullWritable
 * ...
 * }
 *
...
public class AIngleMultipleOutputs {
  ...
  private static final String MULTIPLE_OUTPUTS = "aingle.mapreduce.multipleoutputs";

  private static final String MO_PREFIX = "aingle.mapreduce.multipleoutputs.namedOutput.";

  private static final String FORMAT = ".format";
  private static final String COUNTERS_ENABLED = "aingle.mapreduce.multipleoutputs.counters";
   ...
   *                          AIngleKeyValueOutputFormat or null)
   */
  @SuppressWarnings("unchecked")
  public static void addNamedOutput(Job job, String namedOutput, Class<? extends OutputFormat> outputFormatClass,
   ...
   *
   * The counters group is the {@link AIngleMultipleOutputs} class name. The names
   ...
   */
  public AIngleMultipleOutputs(TaskInputOutputContext<?, ?, ?, ?> context) {
    this.context = context;
    namedOutputs = Collections.unmodifiableSet(new HashSet<>(AIngleMultipleOutputs.getNamedOutputsList(context)));
  ...
  private synchronized RecordWriter getRecordWriter(TaskAttemptContext taskContext, String baseFileName)
    ...
    if (writer == null) {
      ...
      taskContext.getConfiguration().set("aingle.mo.config.namedOutput", baseFileName);
      try {
  ...
  private void setSchema(Job job, Schema keySchema, Schema valSchema) {
    ...
    if (keySchema != null) {
      if (isMaponly)
        AIngleJob.setMapOutputKeySchema(job, keySchema);
      else
        AIngleJob.setOutputKeySchema(job, keySchema);
    }
    if (valSchema != null) {
      if (isMaponly)
        AIngleJob.setMapOutputValueSchema(job, valSchema);
      else
        AIngleJob.setOutputValueSchema(job, valSchema);
    }
