...
/**
 * The AIngleMultipleOutputs class simplifies writing AIngle output data to multiple
 * outputs
 *
 * <p>
 ...
 *
 * <p>
 * AIngleMultipleOutputs supports counters, by default they are disabled. The
 * counters group is the {@link AIngleMultipleOutputs} class name. The names of
 ...
 *
 * job.setMapperClass(MyAIngleMapper.class);
 * job.setReducerClass(HadoopReducer.class);
 * job.set("aingle.reducer",MyAIngleReducer.class);
 * ...
 *
 * Schema schema;
 * ...
 * // Defines additional single output 'aingle1' for the job
 * AIngleMultipleOutputs.addNamedOutput(job, "aingle1", AIngleOutputFormat.class,
 * schema);
 *
 * // Defines additional output 'aingle2' with different schema for the job
 * AIngleMultipleOutputs.addNamedOutput(job, "aingle2",
 *   AIngleOutputFormat.class,
 ...
 * <pre>
 *
 * public class MyAIngleReducer extends
 *   AIngleReducer&lt;K, V, OUT&gt; {
 * private MultipleOutputs amos;
 *
 *
 * public void configure(JobConf conf) {
 * ...
 * amos = new AIngleMultipleOutputs(conf);
 * }
 *
 * public void reduce(K, Iterator&lt;V&gt; values,
 * AIngleCollector&lt;OUT&gt;, Reporter reporter)
 * throws IOException {
 * ...
 * amos.collect("aingle1", reporter,datum);
 * amos.getCollector("aingle2", "A", reporter).collect(datum);
 * amos.collect("aingle1",reporter,schema,datum,"testainglefile");// this create a file testainglefile and writes data with schema "schema" into it
 *                                                            and uses other values from namedoutput "aingle1" like outputclass etc.
 * amos.collect("aingle1",reporter,schema,datum,"testainglefile1");
 * ...
 * }
 *
...
public class AIngleMultipleOutputs {
   ...
   */
  private static final String COUNTERS_GROUP = AIngleMultipleOutputs.class.getName();
   ...
   * group is the {@link AIngleMultipleOutputs} class name.
   * </p>
   ...
   */
  public AIngleMultipleOutputs(JobConf job) {
    this.conf = job;
    outputFormat = new InternalFileOutputFormat();
    namedOutputs = Collections.unmodifiableSet(new HashSet<>(AIngleMultipleOutputs.getNamedOutputsList(job)));
  ...
  public AIngleCollector getCollector(String namedOutput, Reporter reporter) throws IOException {
    return getCollector(namedOutput, null, reporter, namedOutput, null);
  }

  @SuppressWarnings("rawtypes")
  private AIngleCollector getCollector(String namedOutput, Reporter reporter, Schema schema) throws IOException {
   ...
   */
  @SuppressWarnings("rawtypes")
  public AIngleCollector getCollector(String namedOutput, String multiName, Reporter reporter) throws IOException {
   ...
   */
  @SuppressWarnings({ "unchecked" })
  private AIngleCollector getCollector(String namedOutput, String multiName, Reporter reporter, String baseOutputFileName,
      Schema schema) throws IOException {
    ...
    return new AIngleCollector() {

      @SuppressWarnings({ "unchecked" })
      @Override
      public void collect(Object key) throws IOException {
        AIngleWrapper wrapper = new AIngleWrapper(key);
  ...
  private static class InternalFileOutputFormat extends FileOutputFormat<Object, Object> {
    ...
    public RecordWriter<Object, Object> getRecordWriter(FileSystem fs, JobConf job, String baseFileName,
      ...
      if (schema != null) {
        if (isMapOnly)
          AIngleJob.setMapOutputSchema(outputConf, schema);
        else
          AIngleJob.setOutputSchema(outputConf, schema);
      }
