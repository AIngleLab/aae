...
public class TestDataFileTools {
  ...
  public static void writeSampleFile() throws IOException {
    sampleFile = new File(DIR.getRoot(), TestDataFileTools.class.getName() + ".aingle");
  ...
  public void testGetMeta() throws Exception {
    ...
    assertTrue(output, output.contains("aingle.schema\t" + schema.toString() + "\n"));
  ...
  public void testGetMetaForSingleKey() throws Exception {
    assertEquals(schema.toString() + "\n",
        run(new DataFileGetMetaTool(), sampleFile.getPath(), "--key", "aingle.schema"));
  }
  ...
  public void testWrite(String name, List<String> extra, String expectedCodec, String... extraArgs) throws Exception {
    File outFile = new File(DIR.getRoot(), TestDataFileTools.class + ".testWrite." + name + ".aingle");
    ...
    try (DataFileReader<Object> fileReader = new DataFileReader<>(outFile, reader)) {
      ...
      String codecStr = fileReader.getMetaString("aingle.codec");
      if (null == codecStr) {
        codecStr = "null";
      }
  ...
  public void testWritingZeroJsonValues() throws Exception {
    File outFile = writeToAIngleFile("zerojsonvalues", schema.toString(), "");
  ...
  public void testDifferentSeparatorsBetweenJsonRecords() throws Exception {
    File outFile = writeToAIngleFile("separators", "{ \"type\":\"array\", \"items\":\"int\" }",
        "[]    [] []\n[][3]     ");
    assertEquals(5, countRecords(outFile));
  }

  public File writeToAIngleFile(String testName, String schema, String json) throws Exception {
    File outFile = new File(DIR.getRoot(), TestDataFileTools.class + "." + testName + ".aingle");
    try (FileOutputStream fout = new FileOutputStream(outFile)) {
      try (PrintStream out = new PrintStream(fout)) {
