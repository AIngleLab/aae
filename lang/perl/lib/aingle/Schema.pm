...
sub parse {
    ...
    my $struct = try {
        ...
        throw AIngle::Schema::Error::Parse(
...
sub parse_struct {
    ...
    if (ref $struct eq 'HASH') {
        my $type = $struct->{type}
            or throw AIngle::Schema::Error::Parse("type is missing");
        if ( AIngle::Schema::Primitive->is_type_valid($type) ) {
            return AIngle::Schema::Primitive->new(type => $type);
        }
        ## XXX technically we shouldn't allow error type other than in
        ## a Protocol definition
        if ($type eq 'record' or $type eq 'error') {
            return AIngle::Schema::Record->new(
                struct => $struct,
                names => $names,
                namespace => $namespace,
            );
            ...
            return AIngle::Schema::Array->new(
                struct => $struct,
                names => $names,
                namespace => $namespace,
            );
            ...
            return AIngle::Schema::Fixed->new(
                struct => $struct,
                names => $names,
                namespace => $namespace,
            );
            ...
            throw AIngle::Schema::Error::Parse("unknown type: $type");
        }
    }
    ## 1.3.2 A JSON array, representing a union of embedded types.
    elsif (ref $struct eq 'ARRAY') {
        return AIngle::Schema::Union->new(
            struct => $struct,
            names => $names,
            namespace => $namespace,
        );
...
sub match {
    ...
    return $wtype if $wtype eq $rtype
             && AIngle::Schema::Primitive->is_type_valid($wtype);
...
package AIngle::Schema::Base;
our @ISA = qw/AIngle::Schema/;
use Carp;
...
sub to_string {
    ...
    return AIngle::Schema->to_string($schema->to_struct($known_names));
}

package AIngle::Schema::Primitive;
our @ISA = qw/AIngle::Schema::Base/;
use Carp;
use Config;
...
sub new {
    ...
    throw AIngle::Schema::Error::Parse("Not a primitive type $type")
...
sub new {
    ...
    unless (defined $name && length $name) {
        throw AIngle::Schema::Error::Parse( "Missing name for $class" );
    }
...
sub set_names {
    ...
    if (@parts > 1) {
        ...
        if (grep { ! length $_ } @parts) {
            throw AIngle::Schema::Error::Name(
...
sub add_name {
    ...
    if ( exists $names->{ $name } ) {
        throw AIngle::Schema::Error::Parse( "Name $name is already defined" );
    }
    $names->{$name} = $schema;
...
package AIngle::Schema::Record;
our @ISA = qw/AIngle::Schema::Named/;
use Scalar::Util;
...
sub new {
    ...
    my $fields = $param{struct}{fields}
        or throw AIngle::Schema::Error::Parse("Record must have Fields");

    throw AIngle::Schema::Error::Parse("Record.Fields must me an array")
    ...
    my @fields;
    for my $field (@$fields) {
        my $f = AIngle::Schema::Field->new($field, $names, $namespace);
        push @fields, $f;
    }
...
sub new {
    ...
    my $name = $struct->{name};
    throw AIngle::Schema::Error::Parse("Record.Field.name is required")
        unless defined $name && length $name;
    ...
    $type = AIngle::Schema->parse_struct($type, $names, $namespace);
...
sub new {
    ...
    my $schema = $class->SUPER::new(%param);
    my $struct = $param{struct}
        or throw AIngle::Schema::Error::Parse("Enum instantiation");
    my $symbols = $struct->{symbols} || [];

    unless (@$symbols) {
        throw AIngle::Schema::Error::Parse("Enum needs at least one symbol");
    }
    my %symbols;
    my $pos = 0;
    for (@$symbols) {
        ...
        throw AIngle::Schema::Error::Parse("Duplicate symbol in Enum")
            if exists $symbols{$_};
...
sub new {
    ...
    my $items = $struct->{items}
        or throw AIngle::Schema::Error::Parse("Array must declare 'items'");

    $items = AIngle::Schema->parse_struct($items, $param{names}, $param{namespace});
    $schema->{items} = $items;
    return $schema;
}
...
sub new {
    ...
    my $struct = $param{struct}
        or throw AIngle::Schema::Error::Parse("Map instantiation");

    my $values = $struct->{values};
    unless (defined $values && length $values) {
        throw AIngle::Schema::Error::Parse("Map must declare 'values'");
    }
    $values = AIngle::Schema->parse_struct($values, $param{names}, $param{namespace});
...
sub new {
    ...
    my $union = $param{struct}
        or throw AIngle::Schema::Error::Parse("Union.new needs a struct");
    ...
    my %seen_types;
    for my $struct (@$union) {
        my $sch = AIngle::Schema->parse_struct($struct, $names, $param{namespace});
        my $type = $sch->type;
        ...
        if (AIngle::Schema::Named->is_type_valid($type)) {
...
sub new {
    ...
    my $struct = $param{struct}
        or throw AIngle::Schema::Error::Parse("Fixed instantiation");

    my $size = $struct->{size};
    unless (defined $size && length $size) {
        throw AIngle::Schema::Error::Parse("Fixed must declare 'size'");
    }
    if (ref $size) {
        throw AIngle::Schema::Error::Parse(
...
package AIngle::Schema::Error::Parse;
use parent 'Error::Simple';

package AIngle::Schema::Error::Name;
use parent 'Error::Simple';

package AIngle::Schema::Error::Mismatch;
use parent 'Error::Simple';
