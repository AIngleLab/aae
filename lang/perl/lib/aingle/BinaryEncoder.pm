...
sub encode_int {
    ...
    my ($schema, $data, $cb) = @_;
    if ($data !~ /^$RE{num}{int}$/) {
        throw AIngle::BinaryEncoder::Error("cannot convert '$data' to integer");
    }
    if (abs($data) > 0x7fffffff) {
        throw AIngle::BinaryEncoder::Error("int ($data) should be <= 32bits");
    }
...
sub encode_long {
    ...
    my ($schema, $data, $cb) = @_;
    if ($data !~ /^$RE{num}{int}$/) {
        throw AIngle::BinaryEncoder::Error("cannot convert '$data' to long integer");
    }
    if (abs($data) > $max64) {
        throw AIngle::BinaryEncoder::Error("int ($data) should be <= 64bits");
    }
    my $enc = unsigned_varint(zigzag($data));
    $cb->(\$enc);
}
...
sub encode_enum {
    ...
    my $pos = $symbols->{ $data };
    throw AIngle::BinaryEncoder::Error("Cannot find enum $data")
        unless defined $pos;
...
sub encode_union {
    ...
    unless ($elected_schema) {
        throw AIngle::BinaryEncoder::Error("union cannot validate the data");
    }
...
sub encode_fixed {
    ...
    if (bytes::length $data != $schema->size) {
        ...
        my $s2 = $schema->size;
        throw AIngle::BinaryEncoder::Error("Fixed size doesn't match $s1!=$s2");
    }
    $cb->(\$data);
}
...
package AIngle::BinaryEncoder::Error;
