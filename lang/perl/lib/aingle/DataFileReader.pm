...
sub new {
    ...
    croak "schema is invalid"
        if $schema && ! eval { $schema->isa("AIngle::Schema") };
...
sub codec {
    my $datafile = shift;
    return $datafile->metadata->{'aingle.codec'};
}

sub writer_schema {
    my $datafile = shift;
    unless (exists $datafile->{_writer_schema}) {
        my $json_schema = $datafile->metadata->{'aingle.schema'};
        $datafile->{_writer_schema} = AIngle::Schema->parse($json_schema);
    }
...
sub read_file_header {
    ...
    my $data = AIngle::BinaryDecoder->decode(
        reader_schema => $AIngle::DataFile::HEADER_SCHEMA,
        writer_schema => $AIngle::DataFile::HEADER_SCHEMA,
        reader        => $datafile->{fh},
    );
    croak "Magic '$data->{magic}' doesn't match"
        unless $data->{magic} eq AIngle::DataFile->AINGLE_MAGIC;

    $datafile->{sync_marker} = $data->{sync}
        or croak "sync marker appears invalid";

    my $codec = $data->{meta}{'aingle.codec'} || "";

    throw AIngle::DataFile::Error::UnsupportedCodec($codec)
        unless AIngle::DataFile->is_codec_valid($codec);
...
sub read_within_block {
    ...
    while ($count-- > 0 && $datafile->{object_count} > 0) {
        push @objs, AIngle::BinaryDecoder->decode(
...
sub skip {
    ...
    if ($block_count <= $count) {
        ...
        while ($count--) {
            AIngle::BinaryDecoder->skip($writer_schema, $datafile->reader);
            $datafile->{object_count}--;
        }
    }
}
...
sub read_block_header {
    ...
    $datafile->{object_count} = AIngle::BinaryDecoder->decode_long(
        undef, undef, $fh,
    );
    $datafile->{block_size} = AIngle::BinaryDecoder->decode_long(
        undef, undef, $fh,
    );
...
package AIngle::DataFile::Error::UnsupportedCodec;
