...
sub decode {
    ...
    my $type = AIngle::Schema->match(
        writer => $writer_schema,
        reader => $reader_schema,
    ) or throw AIngle::Schema::Error::Mismatch;
...
sub decode_record {
    ...
    for my $name (keys %extra_fields) {
        ...
        unless (exists $extra_fields{$name}->{default}) {
            throw AIngle::Schema::Error::Mismatch(
...
sub decode_enum {
    ...
    throw AIngle::Schema::Error::Mismatch("enum unknown")
    ...
    while ($block_count) {
        ...
        for (1..$block_count) {
            ...
            unless (defined $key && length $key) {
                throw AIngle::Schema::Error::Parse("key of map is invalid");
            }
...
sub skip_union {
    ...
    my $union_schema = $schema->schemas->[$idx]
        or throw AIngle::Schema::Error::Parse("union union member");
