...
void TestSeek() {
    BOOST_TEST_MESSAGE("TestSeek");
    {
        ...
        aingle::OutputBuffer tmp1, tmp2, tmp3;
...
void server(boost::barrier &b) {
    ...
    aingle::InputBuffer::const_iterator iter = rbuf.begin();
    while (iter != rbuf.end()) {
...
void TestAsioBuffer() {
    ...
    BOOST_TEST_MESSAGE("TestAsioBuffer");
    {
        ...
        const aingle::InputBuffer rbuf(buf);
...
void TestCopy() {
    ...
    // Test1, small data, small buffer
    {
        ...
        BufferReader br(wb);
        br.seek(5);
        aingle::InputBuffer ib = br.copyData(msg.size() - 10);
    ...
    // Test2, small data, large buffer
    {
        ...
        aingle::OutputBuffer wb(bufsize);
    ...
    // Test3 Border case, buffer is exactly full
    {
        ...
        aingle::InputBuffer ib = br.copyData(20);
    ...
    // Test4, no data
    {
        ...
        try {
            BufferReader br(wb);
            aingle::InputBuffer it = br.copyData(10);
        } catch (std::exception &e) {
            cout << "Intentially triggered exception: " << e.what() << endl;
        }
...
void TestBug() {
    BOOST_TEST_MESSAGE("TestBug");
    {
        ...
        {
            aingle::InputBuffer ib(rxBuf.extractData());
            buf.append(ib);
        }
