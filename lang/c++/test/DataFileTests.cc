...
using aingle::GenericDatum;
using aingle::GenericRecord;
using aingle::NodePtr;
...
struct codec_traits<Complex<T>> {
    static void encode(Encoder &e, const Complex<T> &c) {
        aingle::encode(e, c.re);
        aingle::encode(e, c.im);
    }

    static void decode(Decoder &d, Complex<T> &c) {
        aingle::decode(d, c.re);
        aingle::decode(d, c.im);
    }
};
...
public:
    ...
    void testWrite() {
        testWriteWithCodec(aingle::NULL_CODEC);
    }

    void testWriteWithDeflateCodec() {
        testWriteWithCodec(aingle::DEFLATE_CODEC);
    }

#ifdef SNAPPY_CODEC_AVAILABLE
    void testWriteWithSnappyCodec() {
        testWriteWithCodec(aingle::SNAPPY_CODEC);
    }
#endif

    void testWriteWithCodec(aingle::Codec codec) {
        aingle::DataFileWriter<ComplexInteger> df(filename, writerSchema, 100);
        int64_t re = 3;
        int64_t im = 5;
    ...
    void testWriteGeneric() {
        aingle::DataFileWriter<Pair> df(filename, writerSchema, 100);
        int64_t re = 3;
        int64_t im = 5;
    ...
    void testWriteDouble() {
        aingle::DataFileWriter<ComplexDouble> df(filename, writerSchema, 100);
        double re = 3.0;
        double im = 5.0;
    ...
    void testReadFull() {
        aingle::DataFileReader<ComplexInteger> df(filename, writerSchema);
        int i = 0;
        ComplexInteger ci;
        int64_t re = 3;
        int64_t im = 5;
    ...
    void testReadProjection() {
        aingle::DataFileReader<Integer> df(filename, readerSchema);
        int i = 0;
        Integer integer;
        int64_t re = 3;
        int64_t im = 5;
    ...
    void testReaderGeneric() {
        aingle::DataFileReader<Pair> df(filename, writerSchema);
        int i = 0;
        Pair p(writerSchema, GenericDatum());
        int64_t re = 3;
        int64_t im = 5;
    ...
    void testReaderGenericProjection() {
        aingle::DataFileReader<Pair> df(filename, readerSchema);
        int i = 0;
    ...
    void testReaderSyncSeek() {
        ...
        for (int i = sync_points.size() - 2; i >= 0; --i) {
            ...
            ComplexInteger ci;
            // Subtract aingle::SyncSize here because sync and pastSync
            // expect a point *at or before* the sync marker, whereas seek
            // expects the point right *after* the sync marker.
            while (!df.pastSync(sync_points[i + 1] - aingle::SyncSize)) {
    ...
    // lang/java/aingle/src/test/java/org/apache/aingle/TestDataFile.java.
    ...
    void testReadDouble() {
        aingle::DataFileReader<ComplexDouble> df(filename, writerSchema);
        int i = 0;
        ComplexDouble ci;
        double re = 3.0;
        double im = 5.0;
    ...
    void testReadDoubleTwoStep() {
        unique_ptr<aingle::DataFileReaderBase>
            base(new aingle::DataFileReaderBase(filename));
        aingle::DataFileReader<ComplexDouble> df(std::move(base));
    ...
    void testReadDoubleTwoStepProject() {
        unique_ptr<aingle::DataFileReaderBase>
            base(new aingle::DataFileReaderBase(filename));
        aingle::DataFileReader<Double> df(std::move(base), readerSchema);
    ...
    void testZip() {
        ...
        ValidSchema dschema = aingle::compileJsonSchemaFromString(sch);
        {
            aingle::DataFileWriter<ComplexInteger> writer(
                filename, dschema, 16 * 1024, aingle::DEFLATE_CODEC);
        ...
        {
            aingle::DataFileReader<ComplexInteger> reader(filename, dschema);
    ...
    void testSnappy() {
        ...
        {
            aingle::DataFileWriter<ComplexInteger> writer(
                filename, dschema, 16 * 1024, aingle::SNAPPY_CODEC);
    ...
    void testSchemaReadWrite() {
        uint32_t a = 42;
        {
            aingle::DataFileWriter<uint32_t> df(filename, writerSchema);
            df.write(a);
        }

        {
            aingle::DataFileReader<uint32_t> df(filename);
            uint32_t b;
            df.read(b);
...
struct codec_traits<WriterObj> {
    static void encode(Encoder &e, const WriterObj &v) {
        aingle::encode(e, v.s1);
        aingle::encode(e, v.s2);
    }
};
...
struct codec_traits<ReaderObj> {
    static void decode(Decoder &d, ReaderObj &v) {
        if (auto *rd =
                dynamic_cast<aingle::ResolvingDecoder *>(&d)) {
            const std::vector<size_t> fo = rd->fieldOrder();
            for (unsigned long it : fo) {
                switch (it) {
                    case 0: {
                        aingle::decode(d, v.s2);
                        break;
                    }
                    default: {
                        break;
...
void testSkipString(aingle::Codec codec) {
    ...
    const char *readerSchemaStr = "{"
                                  ...
                                  "{\"name\": \"s2\", \"type\": \"string\"}"
                                  "]}";
    aingle::ValidSchema writerSchema =
        aingle::compileJsonSchemaFromString(writerSchemaStr);
    aingle::ValidSchema readerSchema =
        aingle::compileJsonSchemaFromString(readerSchemaStr);
    ...
    {
        aingle::DataFileWriter<WriterObj> df(filename,
    ...
    {
        aingle::DataFileReader<ReaderObj> df(filename, readerSchema);
        ReaderObj ro("");
...
void testSkipStringNullCodec() {
    ...
    testSkipString(aingle::NULL_CODEC);
}

void testSkipStringDeflateCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testSkipString(aingle::DEFLATE_CODEC);
}

#ifdef SNAPPY_CODEC_AVAILABLE
void testSkipStringSnappyCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testSkipString(aingle::SNAPPY_CODEC);
}
#endif
...
struct codec_traits<TestRecord> {
    static void encode(Encoder &e, const TestRecord &v) {
        aingle::encode(e, v.s1);
        aingle::encode(e, v.id);
    }

    static void decode(Decoder &d, TestRecord &v) {
        aingle::decode(d, v.s1);
        aingle::decode(d, v.id);
    }
};
} // namespace aingle

void testLastSync(aingle::Codec codec) {
    ...
    aingle::ValidSchema writerSchema =
        aingle::compileJsonSchemaFromString(schemaWithIdAndString);
    ...
    {
        aingle::DataFileWriter<TestRecord> df(filename,
    ...
    {
        aingle::DataFileReader<TestRecord> df(filename);
...
void testReadRecordEfficientlyUsingLastSync(aingle::Codec codec) {
    ...
    {
        std::unique_ptr<aingle::SeekableInputStream>
            seekableInputStream = aingle::fileSeekableInputStream(filename, 1000000);
        ...
        std::unique_ptr<aingle::InputStream>
            inputStream = aingle::memoryInputStream(stitchedData.data(), stitchedData.size());
        ...
        aingle::DataFileReader<TestRecord> df(std::move(inputStream));
        TestRecord readRecord("", 0);
...
void testLastSyncNullCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testLastSync(aingle::NULL_CODEC);
}

void testLastSyncDeflateCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testLastSync(aingle::DEFLATE_CODEC);
}

#ifdef SNAPPY_CODEC_AVAILABLE
void testLastSyncSnappyCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testLastSync(aingle::SNAPPY_CODEC);
}
#endif

void testReadRecordEfficientlyUsingLastSyncNullCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testReadRecordEfficientlyUsingLastSync(aingle::NULL_CODEC);
}

void testReadRecordEfficientlyUsingLastSyncDeflateCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testReadRecordEfficientlyUsingLastSync(aingle::DEFLATE_CODEC);
}

#ifdef SNAPPY_CODEC_AVAILABLE
void testReadRecordEfficientlyUsingLastSyncSnappyCodec() {
    BOOST_TEST_CHECKPOINT(__func__);
    testReadRecordEfficientlyUsingLastSync(aingle::SNAPPY_CODEC);
}
#endif
