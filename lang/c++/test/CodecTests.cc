 ...
 * The basic idea is to generate aingle data according to a schema and
 ...
 * In this case, aingle binary is generated in the standard way. It is read
...
static const TestData data[] = {
    ...
    {"{\"type\": \"record\", \"name\": \"Lisp\", \"fields\": ["
     ...
     "{\"name\":\"cdr\", \"type\":\"Lisp\"}]}]}]}",
     "U2U1S10U0N", 1},
    {"{" // https://issues.apache.org/jira/browse/AINGLE-1635
     "  \"name\": \"Container\","
     "  \"type\": \"record\","
     "  \"fields\": [{"
     "    \"name\": \"field\","
...
init_unit_test_suite(int argc, char *argv[]) {
    ...
    auto *ts = BOOST_TEST_SUITE("AIngle C++ unit tests for codecs");
    aingle::parsing::add_tests(*ts);
    ts->add(BOOST_TEST_CASE(aingle::testStreamLifetimes));
    ts->add(BOOST_TEST_CASE(aingle::testLimitsBinaryCodec));
    ts->add(BOOST_TEST_CASE(aingle::testLimitsJsonCodec));
    ts->add(BOOST_PARAM_TEST_CASE(&aingle::testJson, aingle::jsonData,
                                  ENDOF(aingle::jsonData)));
    ts->add(BOOST_TEST_CASE(aingle::testJsonCodecReinit));
    ts->add(BOOST_TEST_CASE(aingle::testByteCount));
