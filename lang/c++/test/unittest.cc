...
struct TestSchema {
    ...
    static void createExampleSchema() {
        ...
        aingle::RecordSchema myRecord("complex");

        // Now populate my record with fields (each field is another schema):
        myRecord.addField("real", aingle::DoubleSchema());
        myRecord.addField("imaginary", aingle::DoubleSchema());

        // The complex record is the same as used above, let's make a schema
        // for an array of these record

        aingle::ArraySchema complexArray(myRecord);

        aingle::ValidSchema validComplexArray(complexArray);
    ...
    void buildSchema() {
        ...
        IntSchema intSchema;
        aingle::MapSchema map = MapSchema(IntSchema());
...
struct TestBadStuff {
    void testBadFile() {
        ...
        std::string error;
        bool result = aingle::compileJsonSchema(in, schema, error);
...
init_unit_test_suite(int /*argc*/, char * /*argv*/[]) {
    ...
    auto *test = BOOST_TEST_SUITE("AIngle C++ unit test suite");
