...
struct TestCodeGenerator {
    ...
    void serializeToScreen() {
        std::cout << "Serialize:\n";
        aingle::Writer writer;

        aingle::serialize(writer, myRecord_);
    ...
    void serializeToScreenValid() {
        ...
        aingle::ValidatingWriter writer(schema_);
    ...
    void testParser() {
        ...
        aingle::serialize(s, myRecord_);

        testgen::RootRecord inRecord;
        aingle::Reader p(s.buffer());
        aingle::parse(p, inRecord);

        checkOk(myRecord_, inRecord);
    }

    void testParserValid() {
        aingle::ValidatingWriter s(schema_);

        aingle::serialize(s, myRecord_);

        testgen::RootRecord inRecord;
        aingle::ValidatingReader p(schema_, s.buffer());
        aingle::parse(p, inRecord);
    ...
    void testNameIndex() {
        const aingle::NodePtr &node = schema_.root();
        size_t index = 0;
        ...
        const aingle::NodePtr &enumNode = node->leafAt(index);
    ...
    TestCodeGenerator() {
        ...
        aingle::compileJsonSchema(in, schema_);
    }

    testgen::RootRecord myRecord_;
    aingle::ValidSchema schema_;
};
...
struct TestSchemaResolving {
    ...
    aingle::InputBuffer serializeWriteRecordToBuffer() {
        std::ostringstream ostring;
        aingle::Writer s;
        aingle::serialize(s, writeRecord_);
        return s.buffer();
    }

    void parseData(const aingle::InputBuffer &buf, aingle::ResolverSchema &xSchema) {
        aingle::ResolvingReader r(xSchema, buf);

        aingle::parse(r, readRecord_);
    }
    ...
    void test() {
        ...
        aingle::ResolverSchema xSchema(writerSchema_, readerSchema_, layout);

        printRecord(writeRecord_);

        aingle::InputBuffer buffer = serializeWriteRecordToBuffer();
    ...
    TestSchemaResolving() {
        ...
        aingle::compileJsonSchema(win, writerSchema_);

        std::ifstream rin(gReader.c_str());
        aingle::compileJsonSchema(rin, readerSchema_);
    }

    testgen::RootRecord writeRecord_;
    aingle::ValidSchema writerSchema_;

    testgen2::RootRecord readRecord_;
    aingle::ValidSchema readerSchema_;
};
...
init_unit_test_suite(int argc, char *argv[]) {
    ...
    test_suite *test = BOOST_TEST_SUITE("AIngle C++ unit test suite");
