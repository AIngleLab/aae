...
const char *basicSchemas[] = {
    ...
    "\"namespace\":\"org.apache.hadoop.aingle\",\"size\":1}",
...
const char *roundTripSchemas[] = {
    ...
    "[{\"name\":\"f1\",\"type\":\"long\"},"
    "{\"name\":\"f2\",\"type\":\"int\"}]}",
    /* AIngle-C++ cannot do a round-trip on error schemas.
 * "{\"type\":\"error\",\"name\":\"Test\",\"fields\":"
 *       "[{\"name\":\"f1\",\"type\":\"long\"},"
 *       "{\"name\":\"f2\",\"type\":\"int\"}]}"
 */
    ...
    // Fixed
    R"({"type":"fixed","name":"Test","size":1})",
    "{\"type\":\"fixed\",\"namespace\":\"org.apache.hadoop.aingle\","
    "\"name\":\"MyFixed\",\"size\":1}",
    R"({"type":"fixed","name":"Test","size":1})",
    R"({"type":"fixed","name":"Test","size":1})",
...
void testTypes() {
    BOOST_CHECK_EQUAL(isAIngleType(AINGLE_BOOL), true);
}
...
static void testRoundTrip(const char *schema) {
    BOOST_TEST_CHECKPOINT(schema);
    aingle::ValidSchema compiledSchema =
        compileJsonSchemaFromString(std::string(schema));
...
static void testLogicalTypes() {
    ...
    }";
    {
        ...
        BOOST_CHECK(schema1.root()->type() == AINGLE_BYTES);
        ...
        BOOST_CHECK(schema2.root()->type() == AINGLE_FIXED);
    ...
    }
    {
        ...
        BOOST_CHECK(schema.root()->type() == AINGLE_INT);
    ...
    }
    {
        ...
        BOOST_CHECK(schema.root()->type() == AINGLE_LONG);
        LogicalType logicalType = schema.root()->logicalType();
    ...
    }
    {
        ...
        BOOST_CHECK(schema.root()->type() == AINGLE_FIXED);
    ...
    }
    {
        ...
        BOOST_CHECK(schema.root()->type() == AINGLE_STRING);
        LogicalType logicalType = schema.root()->logicalType();
...
init_unit_test_suite(int /*argc*/, char * /*argv*/[]) {
    ...
    ts->add(BOOST_TEST_CASE(&aingle::schema::testTypes));
    ADD_PARAM_TEST(ts, aingle::schema::testBasic, aingle::schema::basicSchemas);
    ADD_PARAM_TEST(ts, aingle::schema::testBasic_fail,
                   aingle::schema::basicSchemaErrors);
    ADD_PARAM_TEST(ts, aingle::schema::testCompile, aingle::schema::basicSchemas);
    ADD_PARAM_TEST(ts, aingle::schema::testRoundTrip, aingle::schema::roundTripSchemas);
    ts->add(BOOST_TEST_CASE(&aingle::schema::testLogicalTypes));
    ADD_PARAM_TEST(ts, aingle::schema::testMalformedLogicalTypes,
                   aingle::schema::malformedLogicalTypes);
    ts->add(BOOST_TEST_CASE(&aingle::schema::testCompactSchemas));
    return ts;
}
