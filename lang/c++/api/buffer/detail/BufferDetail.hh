...
#ifndef aingle_BufferDetail_hh__
#define aingle_BufferDetail_hh__
...
public:
    ...
    /// Return the chunk aingle's begin iterator for reading.
    ChunkList::const_iterator beginRead() const {
        return readChunks_.begin();
    }

    /// Return the chunk aingle's end iterator for reading.
    ChunkList::const_iterator endRead() const {
        return readChunks_.end();
    }

    /// Return the chunk aingle's begin iterator for writing.
    ChunkList::const_iterator beginWrite() const {
        return writeChunks_.begin();
    }

    /// Return the chunk aingle's end iterator for writing.
