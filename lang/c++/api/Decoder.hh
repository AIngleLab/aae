...
#ifndef aingle_Decoder_hh__
#define aingle_Decoder_hh__
...
/// \file
///
/// Low level support for decoding aingle values.
 ...
 * of decoding AIngle data.
 */
class AINGLE_DECL Decoder {
public:
    ...
    /// records are stored consecutively in a stream (e.g. AIngle data file)
    ...
    /// if some non-aingle information is stored at the end of the stream,
    /// the consumers of such data would see the bytes left behind
    /// by the aingle decoder. Similar set of problems occur if the Decoder
 ...
 */
class AINGLE_DECL ResolvingDecoder : public Decoder {
public:
...
/**
 *  Returns an decoder that can decode binary AIngle standard.
 */
AINGLE_DECL DecoderPtr binaryDecoder();

/**
 *  Returns an decoder that validates sequence of calls to an underlying
 *  Decoder against the given schema.
 */
AINGLE_DECL DecoderPtr validatingDecoder(const ValidSchema &schema,
                                       const DecoderPtr &base);

/**
 *  Returns an decoder that can decode AIngle standard for JSON.
 */
AINGLE_DECL DecoderPtr jsonDecoder(const ValidSchema &schema);

/**
 *  Returns a decoder that decodes aingle data from base written according to
 ...
 */
AINGLE_DECL ResolvingDecoderPtr resolvingDecoder(const ValidSchema &writer,
