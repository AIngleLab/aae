...
#ifndef aingle_DataFile_hh__
#define aingle_DataFile_hh__
 ...
 *  this object.
 */
class AINGLE_DECL DataFileWriterBase : boost::noncopyable {
...
/**
 *  An AIngle datafile that can store objects of type T.
 */
template<typename T>
...
public:
    ...
    void write(const T &datum) {
        base_->syncIfNeeded();
        aingle::encode(base_->encoder(), datum);
        base_->incr();
    }
 ...
 */
class AINGLE_DECL DataFileReaderBase : boost::noncopyable {
    const std::string filename_;
...
public:
    ...
    bool read(T &datum) {
        if (base_->hasMore()) {
            base_->decr();
            aingle::decode(base_->decoder(), datum);
            return true;
        }
        return false;
    }
