...
#ifndef aingle_Codec_hh__
#define aingle_Codec_hh__
 ...
 * specific types.
 *
 * Primitive AINGLE types BOOLEAN, INT, LONG, FLOAT, DOUBLE, STRING and BYTES
 ...
 * std::vector<T> for arbitrary type T gets encoded as an AIngle array of T.
 * Similarly, std::map<std::string, T> for arbitrary type T gets encoded
 * as an AIngle map with value type T.
 *
 * Users can have their custom types encoded/decoded by specializing
 * aingle::codec_traits class for their types.
 */
namespace aingle {
...
/**
 * Codec_traits tells aingle how to encode and decode an object of given type.
 *
...
/**
 * codec_traits for AIngle boolean.
 */
template<>
...
/**
 * codec_traits for AIngle double.
 */
template<>
...
/**
 * codec_traits for AIngle string.
 */
template<>
...
/**
 * codec_traits for AIngle arrays.
 */
template<typename T>
...
template<>
struct codec_traits<std::conditional<aingle::is_not_defined<bool_codec_traits>::value,
    ...
    static void encode(Encoder &e, const std::map<std::string, T> &b) {
        ...
        if (!b.empty()) {
            ...
            for (typename std::map<std::string, T>::const_iterator
                 ...
                 it != b.end(); ++it) {
                e.startItem();
                aingle::encode(e, it->first);
                aingle::encode(e, it->second);
            }
        }
        e.mapEnd();
    }
...
/**
* codec_traits for AIngle null.
*/
template<>
struct codec_traits<aingle::null> {
    /**
    * Encodes a given value.
    */
    static void encode(Encoder &e, const aingle::null &) {
        e.encodeNull();
    }
    ...
    * Decodes into a given value.
    */
    static void decode(Decoder &d, aingle::null &) {
        d.decodeNull();
    }
};
