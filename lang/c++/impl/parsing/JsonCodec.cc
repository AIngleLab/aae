...
using aingle::json::JsonGenerator;
using aingle::json::JsonNullFormatter;
using aingle::json::JsonParser;
...
int32_t JsonDecoder<P>::decodeInt() {
    ...
    if (result < INT32_MIN || result > INT32_MAX) {
        throw Exception(boost::format("Value out of range for AIngle int: %1%")
                        % result);
    }
...
EncoderPtr jsonEncoder(const ValidSchema &schema) {
    return std::make_shared<parsing::JsonEncoder<
        parsing::SimpleParser<parsing::JsonHandler<aingle::json::JsonNullFormatter>>, aingle::json::JsonNullFormatter>>(schema);
}

EncoderPtr jsonPrettyEncoder(const ValidSchema &schema) {
    return std::make_shared<parsing::JsonEncoder<
        parsing::SimpleParser<parsing::JsonHandler<aingle::json::JsonPrettyFormatter>>, aingle::json::JsonPrettyFormatter>>(schema);
}
