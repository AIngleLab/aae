...
void Validator::setWaitingForCount() {
    ...
    count_ = 0;
    expectedTypesFlag_ = typeToFlag(AINGLE_INT) | typeToFlag(AINGLE_LONG);
    nextType_ = AINGLE_LONG;
}
...
void Validator::doAdvance() {
    ...
    static_assert((sizeof(funcs) / sizeof(AdvanceFunc)) == (AINGLE_NUM_TYPES),
...
void Validator::setupFlag(Type type) {
    ...
    static const flag_t flags[] = {
        typeToFlag(AINGLE_STRING) | typeToFlag(AINGLE_BYTES),
        typeToFlag(AINGLE_STRING) | typeToFlag(AINGLE_BYTES),
        typeToFlag(AINGLE_INT),
        typeToFlag(AINGLE_INT) | typeToFlag(AINGLE_LONG),
        typeToFlag(AINGLE_FLOAT),
        typeToFlag(AINGLE_DOUBLE),
        typeToFlag(AINGLE_BOOL),
        typeToFlag(AINGLE_NULL),
        typeToFlag(AINGLE_RECORD),
        typeToFlag(AINGLE_ENUM),
        typeToFlag(AINGLE_ARRAY),
        typeToFlag(AINGLE_MAP),
        typeToFlag(AINGLE_UNION),
        typeToFlag(AINGLE_FIXED)};
    static_assert((sizeof(flags) / sizeof(flag_t)) == (AINGLE_NUM_TYPES),
                  "Invalid number of aingle type flags");
...
void Validator::setupOperation(const NodePtr &node) {
    ...
    assert(nextType_ < AINGLE_SYMBOLIC);
...
bool Validator::getCurrentRecordName(std::string &name) const {
    ...
    auto idx = static_cast<int>(compoundStack_.size() - ((!compoundStack_.empty() && (isPrimitive(nextType_) || nextType_ == AINGLE_RECORD)) ? 1 : 2));

    if (idx >= 0 && compoundStack_[idx].node->type() == AINGLE_RECORD) {
