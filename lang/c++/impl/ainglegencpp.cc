...
using aingle::compileJsonSchema;
...
static string decorate(const aingle::Name &name) {
...
string CodeGen::cppTypeOf(const NodePtr &n) {
    switch (n->type()) {
        case aingle::AINGLE_STRING:
            return "std::string";
        case aingle::AINGLE_BYTES:
            return "std::vector<uint8_t>";
        case aingle::AINGLE_INT:
            return "int32_t";
        case aingle::AINGLE_LONG:
            return "int64_t";
        case aingle::AINGLE_FLOAT:
            return "float";
        case aingle::AINGLE_DOUBLE:
            return "double";
        case aingle::AINGLE_BOOL:
            return "bool";
        case aingle::AINGLE_RECORD:
        case aingle::AINGLE_ENUM: {
            string nm = decorate(n->name());
            return inNamespace_ ? nm : fullname(nm);
        }
        case aingle::AINGLE_ARRAY:
            return "std::vector<" + cppTypeOf(n->leafAt(0)) + " >";
        case aingle::AINGLE_MAP:
            return "std::map<std::string, " + cppTypeOf(n->leafAt(1)) + " >";
        case aingle::AINGLE_FIXED:
            return "std::array<uint8_t, " + lexical_cast<string>(n->fixedSize()) + ">";
        case aingle::AINGLE_SYMBOLIC:
            return cppTypeOf(resolveSymbol(n));
        case aingle::AINGLE_UNION:
            return fullname(done[n]);
        case aingle::AINGLE_NULL:
            return "aingle::null";
        default:
            return "$Undefined$";
    }
...
string CodeGen::generateRecordType(const NodePtr &n) {
    ...
    if (!noUnion_) {
        for (size_t i = 0; i < c; ++i) {
            if (n->leafAt(i)->type() == aingle::AINGLE_UNION) {
                os_ << "    typedef " << types[i]
                    << ' ' << n->nameAt(i) << "_t;\n";
            }
        }
    ...
    for (size_t i = 0; i < c; ++i) {
        ...
        if (!noUnion_ && n->leafAt(i)->type() == aingle::AINGLE_UNION) {
...
static void generateGetterAndSetter(ostream &os,
    ...
    os << type << sn << "get_" << name << "() const {\n"
       << "    if (idx_ != " << idx << ") {\n"
       << "        throw aingle::Exception(\"Invalid type for "
       << "union\");\n"
       << "    }\n"
...
string CodeGen::generateUnionType(const NodePtr &n) {
    ...
    pendingConstructors.emplace_back(result, types[0],
                                     n->leafAt(0)->type() != aingle::AINGLE_NULL);
    os_ << "};\n\n";
...
string CodeGen::generateType(const NodePtr &n) {
    NodePtr nn = (n->type() == aingle::AINGLE_SYMBOLIC) ? resolveSymbol(n) : n;
...
void CodeGen::generateEnumTraits(const NodePtr &n) {
    ...
    os_ << "template<> struct codec_traits<" << fn << "> {\n"
        ...
        << "            throw aingle::Exception(error.str());\n"
        << "        }\n"
...
void CodeGen::generateRecordTraits(const NodePtr &n) {
    ...
    for (size_t i = 0; i < c; ++i) {
        ...
        // so we need to call decorate on it
        std::string decoratedNameAt = decorate(n->nameAt(i));
        os_ << "        aingle::encode(e, v." << decoratedNameAt << ");\n";
    }

    os_ << "    }\n"
        << "    static void decode(Decoder& d, " << fn << "& v) {\n";
    os_ << "        if (aingle::ResolvingDecoder *rd =\n";
    os_ << "            dynamic_cast<aingle::ResolvingDecoder *>(&d)) {\n";
    ...
    for (size_t i = 0; i < c; ++i) {
        ...
        std::string decoratedNameAt = decorate(n->nameAt(i));
        os_ << "                case " << i << ":\n";
        os_ << "                    aingle::decode(d, v." << decoratedNameAt << ");\n";
        os_ << "                    break;\n";
    }
    os_ << "                default:\n";
    os_ << "                    break;\n";
...
void CodeGen::generateUnionTraits(const NodePtr &n) {
    ...
    for (size_t i = 0; i < c; ++i) {
        ...
        if (nn->type() == aingle::AINGLE_NULL) {
            ...
            os_ << "            aingle::encode(e, v.get_" << cppNameOf(nn)
                << "());\n";
        }
        os_ << "            break;\n";
    }
...
void CodeGen::generateTraits(const NodePtr &n) {
    switch (n->type()) {
        ...
        case aingle::AINGLE_ARRAY:
        case aingle::AINGLE_MAP:
            generateTraits(n->leafAt(n->type() == aingle::AINGLE_ARRAY ? 0 : 1));
            break;
        case aingle::AINGLE_UNION:
...
int main(int argc, char **argv) {
    ...
    desc.add_options()("help,h", "produce help message")("include-prefix,p", po::value<string>()->default_value("aingle"),
                                                         "prefix for include headers, - for none, default: aingle")("no-union-typedef,U", "do not generate typedefs for unions in records")("namespace,n", po::value<string>(), "set namespace for generated code")("input,i", po::value<string>(), "input file")("output,o", po::value<string>(), "output file to generate");
