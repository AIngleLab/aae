...
static NodePtr makePrimitive(const string &t) {
    if (t == "null") {
        return NodePtr(new NodePrimitive(AINGLE_NULL));
    } else if (t == "boolean") {
        return NodePtr(new NodePrimitive(AINGLE_BOOL));
    } else if (t == "int") {
        return NodePtr(new NodePrimitive(AINGLE_INT));
    } else if (t == "long") {
        return NodePtr(new NodePrimitive(AINGLE_LONG));
    } else if (t == "float") {
        return NodePtr(new NodePrimitive(AINGLE_FLOAT));
    } else if (t == "double") {
        return NodePtr(new NodePrimitive(AINGLE_DOUBLE));
    } else if (t == "string") {
        return NodePtr(new NodePrimitive(AINGLE_STRING));
    } else if (t == "bytes") {
        return NodePtr(new NodePrimitive(AINGLE_BYTES));
    } else {
        return NodePtr();
    }
}
...
static NodePtr makeNode(const json::Entity &e, SymbolTable &st, const string &ns) {
    switch (e.type()) {
        ...
        default: throw Exception(boost::format("Invalid AIngle type: %1%") % e.toString());
    }
}
...
AINGLE_DECL ValidSchema compileJsonSchemaFromString(const string &input) {
...
AINGLE_DECL bool compileJsonSchema(std::istream &is, ValidSchema &schema, string &error) {
    try {
