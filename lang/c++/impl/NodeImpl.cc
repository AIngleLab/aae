...
void NodeRecord::printJson(std::ostream &os, size_t depth) const {
    ...
    for (size_t i = 0; i < fields; ++i) {
        ...
        if (!defaultValues.empty()) {
            if (!defaultValues[i].isUnion() && defaultValues[i].type() == AINGLE_NULL) {
                // No "default" field.
            } else {
                os << ",\n"
...
NodeRecord::NodeRecord(const HasName &name,
                       ...
                       std::vector<GenericDatum> dv) : NodeImplRecord(AINGLE_RECORD, name, fields, fieldsNames, NoSize()),
...
NodeMap::NodeMap() : NodeImplMap(AINGLE_MAP) {
