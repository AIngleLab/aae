...
class ResolverFactory : private boost::noncopyable {
    ...
    constructCompound(const NodePtr &writer, const NodePtr &reader, const Layout &offset) {
        ...
        aingle::SchemaResolution match = writer->resolve(*reader);

        if (match == RESOLVE_NO_MATCH) {
            instruction = unique_ptr<Resolver>(new Skipper(*this, writer));
        } else if (writer->type() != AINGLE_UNION && reader->type() == AINGLE_UNION) {
...
public:
    ...
    construct(const NodePtr &writer, const NodePtr &reader, const Layout &offset) {
        ...
        NodePtr currentWriter = (writer->type() == AINGLE_SYMBOLIC) ? resolveSymbol(writer) : writer;

        NodePtr currentReader = (reader->type() == AINGLE_SYMBOLIC) ? resolveSymbol(reader) : reader;
        ...
        static_assert((sizeof(funcs) / sizeof(BuilderFunc)) == (AINGLE_NUM_TYPES),
    ...
    skipper(const NodePtr &writer) {
        ...
        NodePtr currentWriter = (writer->type() == AINGLE_SYMBOLIC) ? writer->leafAt(0) : writer;
