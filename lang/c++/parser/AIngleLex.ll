...
{
    aingle::CompilerContext *c = static_cast<aingle::CompilerContext *>(ctx);
    int ret = c->lexer().yylex();
    if( ret > AINGLE_LEX_OUTPUT_TEXT_BEGIN && ret < AINGLE_LEX_OUTPUT_TEXT_END ) {
...
%%
<READTYPE>int                   return AINGLE_LEX_INT;
<READTYPE>long                  return AINGLE_LEX_LONG;
<READTYPE>null                  return AINGLE_LEX_NULL;
<READTYPE>boolean               return AINGLE_LEX_BOOL;
<READTYPE>float                 return AINGLE_LEX_FLOAT;
<READTYPE>double                return AINGLE_LEX_DOUBLE;
<READTYPE>string                return AINGLE_LEX_STRING;
<READTYPE>bytes                 return AINGLE_LEX_BYTES;
<READTYPE>record                return AINGLE_LEX_RECORD;
<READTYPE>enum                  return AINGLE_LEX_ENUM;
<READTYPE>map                   return AINGLE_LEX_MAP;
<READTYPE>array                 return AINGLE_LEX_ARRAY;
<READTYPE>fixed                 return AINGLE_LEX_FIXED;
<READTYPE>{aingletext}            return AINGLE_LEX_NAMED_TYPE;
<READTYPE>\"                    yy_pop_state();

<READNAME>{aingletext}            return AINGLE_LEX_NAME;
<READNAME>\"                    yy_pop_state();

<READSYMBOL>{aingletext}          return AINGLE_LEX_SYMBOL;
<READSYMBOL>\"                  yy_pop_state();

<READFIELDNAME>{aingletext}       return AINGLE_LEX_FIELD_NAME;
<READFIELDNAME>\"               yy_pop_state();

<READFIELD>\"type\"{delim}      yy_push_state(STARTSCHEMA);
<READFIELD>\"name\"{delim}\"    yy_push_state(READFIELDNAME);
<READFIELD>\}                   yy_pop_state(); return AINGLE_LEX_FIELD_END;
<READFIELD>,                    return yytext[0];
<READFIELD>\"{aingletext}\"+{delim}      yy_push_state(READMETADATA); return AINGLE_LEX_METADATA;
<READFIELD>{ws}                 ;

<READFIELDS>\{                  yy_push_state(READFIELD); return AINGLE_LEX_FIELD;
<READFIELDS>\]                  yy_pop_state(); return AINGLE_LEX_FIELDS_END;
<READFIELDS>,                   return yytext[0];
<READFIELDS>{ws}                ;

<READSYMBOLS>\"                 yy_push_state(READSYMBOL);
<READSYMBOLS>,                  return yytext[0];
<READSYMBOLS>\]                 yy_pop_state(); return AINGLE_LEX_SYMBOLS_END;
<READSYMBOLS>{ws}               ;

<READSIZE>{integer}             yy_pop_state(); return AINGLE_LEX_SIZE;

<INUNION>\"                     yy_push_state(READTYPE); return AINGLE_LEX_SIMPLE_TYPE;
...
<INOBJECT>\"type\"{delim}       yy_push_state(STARTTYPE); return AINGLE_LEX_TYPE;
<INOBJECT>\"name\"{delim}\"     yy_push_state(READNAME);
<INOBJECT>\"size\"{delim}       yy_push_state(READSIZE);
<INOBJECT>\"items\"{delim}      yy_push_state(STARTSCHEMA); return AINGLE_LEX_ITEMS;
<INOBJECT>\"values\"{delim}     yy_push_state(STARTSCHEMA); return AINGLE_LEX_VALUES;
<INOBJECT>\"fields\"{delim}\[   yy_push_state(READFIELDS); return AINGLE_LEX_FIELDS;
<INOBJECT>\"symbols\"{delim}\[  yy_push_state(READSYMBOLS); return AINGLE_LEX_SYMBOLS;
<INOBJECT>,                     return yytext[0];
<INOBJECT>\}                    yy_pop_state(); return yytext[0];
<INOBJECT>\"{aingletext}+\"{delim}       yy_push_state(READMETADATA); return AINGLE_LEX_METADATA;
<INOBJECT>{ws}                  ;
...
<STARTSCHEMA>\"                 yy_pop_state(); yy_push_state(READTYPE); return AINGLE_LEX_SIMPLE_TYPE;
...
\"                              yy_push_state(READTYPE); return AINGLE_LEX_SIMPLE_TYPE;
{ws}                            ;
<<EOF>>                         {
