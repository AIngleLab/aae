...
"""
Read/Write AIngle File Object Containers.

https://apache.aingle.ai/docs/current/spec.html#Object+Container+Files
"""
import io
import json
...
from aingle.utils import TypedDict, randbytes
...
META_SCHEMA: aingle.schema.RecordSchema = aingle.schema.parse(
    json.dumps(
        {
            "type": "record",
            "name": "org.apache.aingle.file.Header",
            "fields": [
...
NULL_CODEC = "null"
VALID_CODECS = aingle.codecs.KNOWN_CODECS.keys()
VALID_ENCODINGS = ["binary"]  # not used yet

CODEC_KEY = "aingle.codec"
SCHEMA_KEY = "aingle.schema"
...
class _DataFileMetadata:
    ...
    All metadata properties that start with "aingle." are reserved.
    The following file metadata properties are currently used:

    - `aingle.schema` contains the schema of objects stored in the file, as JSON data (required).
    - `aingle.codec`, the name of the compression codec used to compress blocks, as a string.
      Implementations are required to support the following codecs: "null" and "deflate".
      If codec is absent, it is assumed to be "null". See aingle.codecs for implementation details.
    """
    ...
    def schema(self) -> str:
        ...
        raise aingle.errors.DataFileException("Missing required schema metadata.")
    ...
    def codec(self, value: str) -> None:
        ...
        if value not in VALID_CODECS:
            raise aingle.errors.DataFileException(f"Unknown codec: {value!r}")
...
class DataFileWriter(_DataFileMetadata):
    ...
    _buffer_encoder: aingle.io.BinaryEncoder
    _buffer_writer: io.BytesIO  # BinaryIO would have better compatibility, but we use getvalue right now.
    _datum_writer: aingle.io.DatumWriter
    _encoder: aingle.io.BinaryEncoder
    ...
    def __init__(
        self, writer: BinaryIO, datum_writer: aingle.io.DatumWriter, writers_schema: Optional[aingle.schema.Schema] = None, codec: str = NULL_CODEC
    ) -> None:
        """If the schema is not present, presume we're appending."""
        self._writer = writer
        self._encoder = aingle.io.BinaryEncoder(writer)
        self._datum_writer = datum_writer
        self._buffer_writer = io.BytesIO()
        self._buffer_encoder = aingle.io.BinaryEncoder(self._buffer_writer)
        self.block_count = 0
        self._header_written = False

        if writers_schema is None:
            # open writer for reading to collect metadata
            dfr = DataFileReader(writer, aingle.io.DatumReader())
            ...
            self.datum_writer.writers_schema = aingle.schema.parse(schema_from_file)
    ...
    @property
    def encoder(self) -> aingle.io.BinaryEncoder:
        return self._encoder

    @property
    def datum_writer(self) -> aingle.io.DatumWriter:
    ...
    @property
    def buffer_encoder(self) -> aingle.io.BinaryEncoder:
    ...
    def _write_block(self) -> None:
        ...
        if self.block_count > 0:
            ...
            codec = aingle.codecs.get_codec(self.codec)
...
class DataFileReader(_DataFileMetadata):
    ...
    _datum_decoder: Optional[aingle.io.BinaryDecoder]
    _datum_reader: aingle.io.DatumReader
    _file_length: int
    _raw_decoder: aingle.io.BinaryDecoder
    _reader: BinaryIO
    block_count: int
    sync_marker: bytes
    ...
    def __init__(self, reader, datum_reader):
        self._reader = reader
        self._raw_decoder = aingle.io.BinaryDecoder(reader)
        ...
        # get ready to read
        self.block_count = 0
        self.datum_reader.writers_schema = aingle.schema.parse(self.schema)
    ...
    @property
    def raw_decoder(self) -> aingle.io.BinaryDecoder:
        return self._raw_decoder

    @property
    def datum_decoder(self) -> Optional[aingle.io.BinaryDecoder]:
        return self._datum_decoder

    @property
    def datum_reader(self) -> aingle.io.DatumReader:
    ...
    def _read_header(self) -> None:
        ...
        if header.get("magic") != MAGIC:
            raise aingle.errors.AIngleException(f"Not an AIngle data file: {header.get('magic')!r} doesn't match {MAGIC!r}.")
    ...
    def __next__(self) -> object:
        ...
        if self.datum_decoder is None:
            raise aingle.errors.DataFileException("DataFile is not ready to read because it has no decoder")
