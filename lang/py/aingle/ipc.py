...
HANDSHAKE_REQUEST_SCHEMA_JSON = _load("HandshakeRequest.ain")
HANDSHAKE_RESPONSE_SCHEMA_JSON = _load("HandshakeResponse.ain")
HANDSHAKE_REQUEST_SCHEMA = aingle.schema.parse(HANDSHAKE_REQUEST_SCHEMA_JSON)
HANDSHAKE_RESPONSE_SCHEMA = aingle.schema.parse(HANDSHAKE_RESPONSE_SCHEMA_JSON)

HANDSHAKE_REQUESTOR_WRITER = aingle.io.DatumWriter(HANDSHAKE_REQUEST_SCHEMA)
HANDSHAKE_REQUESTOR_READER = aingle.io.DatumReader(HANDSHAKE_RESPONSE_SCHEMA)
HANDSHAKE_RESPONDER_WRITER = aingle.io.DatumWriter(HANDSHAKE_RESPONSE_SCHEMA)
HANDSHAKE_RESPONDER_READER = aingle.io.DatumReader(HANDSHAKE_REQUEST_SCHEMA)

META_SCHEMA = aingle.schema.parse('{"type": "map", "values": "bytes"}')
META_WRITER = aingle.io.DatumWriter(META_SCHEMA)
META_READER = aingle.io.DatumReader(META_SCHEMA)
...
class BaseRequestor:
    ...
    def request(self, message_name, request_datum):
        ...
        buffer_encoder = aingle.io.BinaryEncoder(buffer_writer)
    ...
    def write_call_request(self, message_name, request_datum, encoder):
        ...
        if message is None:
            raise aingle.errors.AIngleException(f"Unknown message: {message_name}")
    ...
    def write_request(self, request_schema, request_datum, encoder):
        datum_writer = aingle.io.DatumWriter(request_schema)
    ...
    def read_handshake_response(self, decoder):
        ...
        if match == "BOTH":
            ...
            if self.send_protocol:
                raise aingle.errors.AIngleException("Handshake failure.")
            self.remote_protocol = aingle.protocol.parse(handshake_response.get("serverProtocol"))
            ...
            raise aingle.errors.AIngleException(f"Unexpected match: {match}")
    ...
    def read_call_response(self, message_name, decoder):
        ...
        if remote_message_schema is None:
            raise aingle.errors.AIngleException(f"Unknown remote message: {message_name}")

        # local response schema
        local_message_schema = self.local_protocol.messages.get(message_name)
        if local_message_schema is None:
            raise aingle.errors.AIngleException(f"Unknown local message: {message_name}")
        ...
        if not decoder.read_boolean():
            ...
            raise aingle.errors.AIngleRemoteException(datum_reader.read(decoder))
...
class Requestor(BaseRequestor):
    def issue_request(self, call_request, message_name, request_datum):
        ...
        buffer_decoder = aingle.io.BinaryDecoder(io.BytesIO(call_response))
...
class Responder:
    ...
    def respond(self, call_request):
        ...
        buffer_decoder = aingle.io.BinaryDecoder(buffer_reader)
        buffer_writer = io.BytesIO()
        buffer_encoder = aingle.io.BinaryEncoder(buffer_writer)
        error = None
        response_metadata = {}
        ...
        try:
            ...
            except aingle.errors.AIngleRemoteException as e:
                error = e
            except Exception as e:
                error = aingle.errors.AIngleRemoteException(str(e))
        ...
        except schema.AIngleException as e:
            error = aingle.errors.AIngleRemoteException(str(e))
            buffer_encoder = aingle.io.BinaryEncoder(io.BytesIO())
            META_WRITER.write(response_metadata, buffer_encoder)
    ...
    def read_framed_message(self):
        ...
        while True:
            ...
            while buffer.tell() < buffer_length:
                ...
                if chunk == "":
                    raise aingle.errors.ConnectionClosedException("Reader read 0 bytes.")
                buffer.write(chunk)
...
class HTTPTransceiver:
    ...
    def write_framed_message(self, message):
        req_method = "POST"
        req_headers = {"Content-Type": "aingle/binary"}
