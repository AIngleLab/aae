...
inputProtocol = aingle.protocol.parse(prototxt)

# use a named tuple to represent the tasktype enumeration
assert inputProtocol.types_dict is not None
taskschema = cast(aingle.schema.EnumSchema, inputProtocol.types_dict["TaskType"])
...
outputProtocol = aingle.protocol.parse(prototxt)
...
class Collector:
    ...
    def __init__(self, scheme, outputClient):
        ...
        if not isinstance(scheme, aingle.schema.Schema):
            scheme = aingle.schema.parse(scheme)

        self.scheme = scheme

        self.datum_writer = aingle.io.DatumWriter(writers_schema=self.scheme)
    ...
    def collect(self, record, partition=None):
        ...
        with io.BytesIO() as buff:
            self.encoder = aingle.io.BinaryEncoder(buff)
...
class HTTPRequestor:
    ...
    def request(self, *args, **param):
        transciever = aingle.ipc.HTTPTransceiver(self.server, self.port)
        requestor = aingle.ipc.Requestor(self.protocol, transciever)
...
class TetherTask(abc.ABC):
    ...
    def __init__(self, inschema, midschema, outschema):
        ...
        inscheme='{"type":"record", "name":"Pair","namespace":"org.apache.aingle.mapred","fields":[
        ...
        the differences (see https://apache.aingle.ai/docs/current/spec.html#Schema+Resolution))

        """
        # make sure we can parse the schemas
        # Should we call fail if we can't parse the schemas?
        self.inschema = aingle.schema.parse(inschema)
        self.midschema = aingle.schema.parse(midschema)
        self.outschema = aingle.schema.parse(outschema)
    ...
    def open(self, inputport, clientPort=None):
        ...
        clientPort - The port on which the server in the parent process is listening
                    - If this is None we look for the environment variable AINGLE_TETHER_OUTPUT_PORT
        ...
        # variable AINGLE_TETHER_OUTPUT_PORT
        # open output client, connecting to parent
        clientPort = int(clientPort or os.getenv("AINGLE_TETHER_OUTPUT_PORT", 0))
        if clientPort == 0:
            raise aingle.errors.UsageError("AINGLE_TETHER_OUTPUT_PORT env var is not set")

        self.log.info("TetherTask.open: Opening connection to parent server on port=%d", clientPort)

        # self.outputClient =  aingle.ipc.Requestor(outputProtocol, self.clientTransceiver)
    ...
    def configure(self, taskType, inSchemaText, outSchemaText):
        ...
        inSchemaText -  string containing the input schema
                     - This is the actual schema with which the data was encoded
                       i.e it is the writer_schema (see https://apache.aingle.ai/docs/current/spec.html#Schema+Resolution)
        ...
        try:
            inSchema = aingle.schema.parse(inSchemaText)
            outSchema = aingle.schema.parse(outSchemaText)

            if taskType == TaskType.MAP:
                self.inReader = aingle.io.DatumReader(writers_schema=inSchema, readers_schema=self.inschema)
                self.midCollector = Collector(outSchemaText, self.outputClient)

            elif taskType == TaskType.REDUCE:
                self.midReader = aingle.io.DatumReader(writers_schema=inSchema, readers_schema=self.midschema)
    ...
    def input(self, data, count):
        ...
        """
        try:
            # to aingle.io.BinaryDecoder
            bdata = io.BytesIO(data)
            decoder = aingle.io.BinaryDecoder(bdata)
