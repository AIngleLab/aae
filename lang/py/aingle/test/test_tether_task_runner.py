...
import aingle.io
import aingle.test.mock_tether_parent
import aingle.test.word_count_task
...
class TestTetherTaskRunner(unittest.TestCase):
    ...
    def test1(self):
        ...
        try:
            ...
            parent_port = aingle.tether.util.find_port()

            pyfile = aingle.test.mock_tether_parent.__file__
            proc = subprocess.Popen([sys.executable, pyfile, "start_server", f"{parent_port}"])
            input_port = aingle.tether.util.find_port()
            ...
            runner = aingle.tether.tether_task_runner.TaskRunner(aingle.test.word_count_task.WordCountTask())
            ...
            requestor = aingle.tether.tether_task.HTTPRequestor(
                "localhost",
                runner.server.server_address[1],
                aingle.tether.tether_task.inputProtocol,
            )
            ...
            requestor.request(
                "configure",
                {
                    "taskType": aingle.tether.tether_task.TaskType.MAP,
            ...
            datum_writer = aingle.io.DatumWriter(runner.task.inschema)
            ...
            requestor.request(
                "configure",
                {
                    "taskType": aingle.tether.tether_task.TaskType.REDUCE,
            ...
            writer = io.BytesIO()
            encoder = aingle.io.BinaryEncoder(writer)
            datum_writer = aingle.io.DatumWriter(runner.task.midschema)
            datum_writer.write(datum, encoder)
    ...
    def test2(self):
        ...
        try:
            ...
            runnerproc = subprocess.Popen(
                [
                    sys.executable,
                    aingle.tether.tether_task_runner.__file__,
                    "aingle.test.word_count_task.WordCountTask",
                ],
                env={"AINGLE_TETHER_OUTPUT_PORT": f"{parent_port}", "PYTHONPATH": ":".join(sys.path)},
            )
