...
class TestSchema:
    ...
    def parse(self):
        return aingle.schema.parse(str(self))
...
PRIMITIVE_EXAMPLES.extend([ValidTestSchema(f'"{t}"') for t in aingle.schema.PRIMITIVE_TYPES])
PRIMITIVE_EXAMPLES.extend([ValidTestSchema({"type": t}) for t in aingle.schema.PRIMITIVE_TYPES])
...
FIXED_EXAMPLES = [
    ...
    ValidTestSchema(
        {
            ...
            "name": "MyFixed",
            "size": 1,
            "namespace": "org.apache.hadoop.aingle",
        }
    ),
    ...
    InvalidTestSchema({"type": "fixed", "size": 314, "name": "dr. spaceman"}, comment="AINGLE-621"),
]

ENUM_EXAMPLES = [
    ValidTestSchema({"type": "enum", "name": "Test", "symbols": ["A", "B"]}),
    ValidTestSchema({"type": "enum", "name": "AINGLE2174", "symbols": ["nowhitespace"]}),
    ...
    InvalidTestSchema({"type": "enum", "name": "AINGLE2174", "symbols": ["white space"]}),
]
...
IGNORED_LOGICAL_TYPE = [
    ValidTestSchema(
        ...
        warnings=[aingle.errors.IgnoredLogicalType("Unknown unknown-logical-type, using string.")],
    ),
    ValidTestSchema(
        {"type": "bytes", "logicalType": "decimal", "scale": 0},
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal precision None. Must be a positive integer.")],
    ),
    ValidTestSchema(
        {"type": "bytes", "logicalType": "decimal", "precision": 2.4, "scale": 0},
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal precision 2.4. Must be a positive integer.")],
    ),
    ValidTestSchema(
        {"type": "bytes", "logicalType": "decimal", "precision": 2, "scale": -2},
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal scale -2. Must be a positive integer.")],
    ),
    ValidTestSchema(
        {"type": "bytes", "logicalType": "decimal", "precision": -2, "scale": 2},
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal precision -2. Must be a positive integer.")],
    ),
    ValidTestSchema(
        {"type": "bytes", "logicalType": "decimal", "precision": 2, "scale": 3},
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal scale 3. Cannot be greater than precision 2.")],
    ),
    ValidTestSchema(
        {
            "type": "fixed",
        ...
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal precision -10. Must be a positive integer.")],
    ),
    ValidTestSchema(
        {
            "type": "fixed",
    ...
    ValidTestSchema(
        ...
        warnings=[aingle.errors.IgnoredLogicalType("Invalid decimal precision 311. Max is 310.")],
    ),
    ValidTestSchema(
        {"type": "float", "logicalType": "decimal", "precision": 2, "scale": 0},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type decimal requires literal type bytes/fixed, not float.")],
    ),
    ValidTestSchema(
        {"type": "int", "logicalType": "date1"},
        warnings=[aingle.errors.IgnoredLogicalType("Unknown date1, using int.")],
    ),
    ValidTestSchema(
        {"type": "long", "logicalType": "date"},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type date requires literal type int, not long.")],
    ),
    ValidTestSchema(
        {"type": "int", "logicalType": "time-milis"},
        warnings=[aingle.errors.IgnoredLogicalType("Unknown time-milis, using int.")],
    ),
    ValidTestSchema(
        {"type": "long", "logicalType": "time-millis"},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type time-millis requires literal type int, not long.")],
    ),
    ValidTestSchema(
        {"type": "long", "logicalType": "time-micro"},
        warnings=[aingle.errors.IgnoredLogicalType("Unknown time-micro, using long.")],
    ),
    ValidTestSchema(
        {"type": "int", "logicalType": "time-micros"},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type time-micros requires literal type long, not int.")],
    ),
    ValidTestSchema(
        {"type": "long", "logicalType": "timestamp-milis"},
        warnings=[aingle.errors.IgnoredLogicalType("Unknown timestamp-milis, using long.")],
    ),
    ValidTestSchema(
        {"type": "int", "logicalType": "timestamp-millis"},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type timestamp-millis requires literal type long, not int.")],
    ),
    ValidTestSchema(
        {"type": "long", "logicalType": "timestamp-micro"},
        warnings=[aingle.errors.IgnoredLogicalType("Unknown timestamp-micro, using long.")],
    ),
    ValidTestSchema(
        {"type": "int", "logicalType": "timestamp-micros"},
        warnings=[aingle.errors.IgnoredLogicalType("Logical type timestamp-micros requires literal type long, not int.")],
    ),
]
...
class TestMisc(unittest.TestCase):
    ...
    def test_correct_recursive_extraction(self):
        ...
        t = aingle.schema.parse(str(s.fields[0].type))
    ...
    def test_name_is_none(self):
        ...
        self.assertIsNone(aingle.schema.Name(None, None, None).fullname)
        self.assertIsNone(aingle.schema.Name(None, None, None).space)

    def test_name_not_empty_string(self):
        """A name cannot be the empty string."""
        self.assertRaises(aingle.errors.SchemaParseException, aingle.schema.Name, "", None, None)

    def test_name_space_specified(self):
        """Space combines with a name to become the fullname."""
        # name and namespace specified
        fullname = aingle.schema.Name("a", "o.a.h", None).fullname
    ...
    def test_fullname_space_specified(self):
        ...
        fullname = aingle.schema.Name("a.b.c.d", "o.a.h", None).fullname
    ...
    def test_name_default_specified(self):
        ...
        fullname = aingle.schema.Name("a", None, "b.c.d").fullname
    ...
    def test_equal_names(self):
        ...
        self.assertEqual(
            aingle.schema.Name("a.b.c.d", None, None),
            aingle.schema.Name("d", "a.b.c", None),
        )
        self.assertNotEqual(aingle.schema.Name("C.d", None, None), aingle.schema.Name("c.d", None, None))

    def test_invalid_name(self):
        """The name portion of a fullname, record field names, and enum symbols must:
        start with [A-Za-z_] and subsequently contain only [A-Za-z0-9_]"""
        self.assertRaises(
            aingle.errors.InvalidName,
            aingle.schema.Name,
    ...
    def test_null_namespace(self):
        ...
        name = aingle.schema.Name("name", "", None)
    ...
    def test_exception_is_not_swallowed_on_parse_error(self):
        ...
        self.assertRaisesRegexp(
            aingle.errors.SchemaParseException,
    ...
    def test_fixed_decimal_valid_max_precision(self):
        ...
        fixed_decimal = fixed_decimal_schema.parse()
        self.assertIsInstance(fixed_decimal, aingle.schema.FixedSchema)
        self.assertIsInstance(fixed_decimal, aingle.schema.DecimalLogicalSchema)
    ...
    def test_fixed_decimal_invalid_max_precision(self):
        ...
        fixed_decimal = fixed_decimal_schema.parse()
        self.assertIsInstance(fixed_decimal, aingle.schema.FixedSchema)
        self.assertNotIsInstance(fixed_decimal, aingle.schema.DecimalLogicalSchema)

    def test_parse_invalid_symbol(self):
        """Disabling enumschema symbol validation should allow invalid symbols to pass."""
        test_schema_string = json.dumps({"type": "enum", "name": "AINGLE2174", "symbols": ["white space"]})
        with self.assertRaises(aingle.errors.InvalidName, msg="When enum symbol validation is enabled, an invalid symbol should raise InvalidName."):
            aingle.schema.parse(test_schema_string, validate_enum_symbols=True)
        try:
            aingle.schema.parse(test_schema_string, validate_enum_symbols=False)
        except aingle.errors.InvalidName:  # pragma: no coverage
...
class SchemaParseTestCase(unittest.TestCase):
    ...
    def parse_valid(self) -> None:
        ...
        try:
            warnings.filterwarnings(action="error", category=aingle.errors.IgnoredLogicalType)
            self.test_schema.parse()
        except (aingle.errors.IgnoredLogicalType) as e:
            self.assertIn(type(e), (type(w) for w in test_warnings))
            self.assertIn(str(e), (str(w) for w in test_warnings))
        except (aingle.errors.AIngleException, aingle.errors.SchemaParseException):  # pragma: no coverage
            self.fail(f"Valid schema failed to parse: {self.test_schema!s}")
        else:
            self.assertEqual([], test_warnings)
        finally:
            warnings.filterwarnings(action="default", category=aingle.errors.IgnoredLogicalType)

    def parse_invalid(self):
        """Parsing an invalid schema should error."""
        with self.assertRaises(
            (aingle.errors.AIngleException, aingle.errors.SchemaParseException), msg=f"Invalid schema should not have parsed: {self.test_schema!s}"
        ):
            self.test_schema.parse()
...
class RoundTripParseTestCase(unittest.TestCase):
    ...
    def parse_round_trip(self):
        ...
        round_trip = aingle.schema.parse(str(parsed))
        self.assertEqual(
            parsed,
            round_trip,
            {
...
class OtherAttributesTestCase(unittest.TestCase):
    ...
    def check_attributes(self):
        ...
        round_trip = aingle.schema.parse(str(sch))
        self.assertEqual(
            sch,
            round_trip,
...
class CanonicalFormTestCase(unittest.TestCase):
    ...
    Assuming an input schema (in JSON form) that's already UTF-8 text for a valid AIngle schema (including all
    ...
    def test_primitive_int(self):
        ...
        """
        s = aingle.schema.parse(json.dumps("int"))
        self.assertEqual(s.canonical_form, '"int"')

        s = aingle.schema.parse(json.dumps({"type": "int"}))
        self.assertEqual(s.canonical_form, '"int"')

    def test_primitive_float(self):
        s = aingle.schema.parse(json.dumps("float"))
        self.assertEqual(s.canonical_form, '"float"')

        s = aingle.schema.parse(json.dumps({"type": "float"}))
        self.assertEqual(s.canonical_form, '"float"')

    def test_primitive_double(self):
        s = aingle.schema.parse(json.dumps("double"))
        self.assertEqual(s.canonical_form, '"double"')

        s = aingle.schema.parse(json.dumps({"type": "double"}))
        self.assertEqual(s.canonical_form, '"double"')

    def test_primitive_null(self):
        s = aingle.schema.parse(json.dumps("null"))
        self.assertEqual(s.canonical_form, '"null"')

        s = aingle.schema.parse(json.dumps({"type": "null"}))
        self.assertEqual(s.canonical_form, '"null"')

    def test_primitive_bytes(self):
        s = aingle.schema.parse(json.dumps("bytes"))
        self.assertEqual(s.canonical_form, '"bytes"')

        s = aingle.schema.parse(json.dumps({"type": "bytes"}))
        self.assertEqual(s.canonical_form, '"bytes"')

    def test_primitive_long(self):
        s = aingle.schema.parse(json.dumps("long"))
        self.assertEqual(s.canonical_form, '"long"')

        s = aingle.schema.parse(json.dumps({"type": "long"}))
        self.assertEqual(s.canonical_form, '"long"')

    def test_primitive_boolean(self):
        s = aingle.schema.parse(json.dumps("boolean"))
        self.assertEqual(s.canonical_form, '"boolean"')

        s = aingle.schema.parse(json.dumps({"type": "boolean"}))
        self.assertEqual(s.canonical_form, '"boolean"')

    def test_primitive_string(self):
        s = aingle.schema.parse(json.dumps("string"))
        self.assertEqual(s.canonical_form, '"string"')

        s = aingle.schema.parse(json.dumps({"type": "string"}))
        self.assertEqual(s.canonical_form, '"string"')
    ...
    def test_integer_canonical_form(self):
        ...
        """
        s = aingle.schema.parse('{"name":"md5","type":"fixed","size":16}')
        self.assertEqual(
            s.canonical_form,
    ...
    def test_string_with_escaped_characters(self):
        ...
        """
        s = aingle.schema.parse('{"name":"\u0041","type":"fixed","size":16}')
        self.assertEqual(
            s.canonical_form,
    ...
    def test_fullname(self):
        ...
        self.assertEqual(
            s.canonical_form,
            self.compact_json_string({"name": "aingle.example", "type": "enum", "symbols": ["a", "b"]}),
        )
    ...
    def test_order(self):
        ...
        """
        s = aingle.schema.parse(json.dumps({"symbols": ["a", "b"], "type": "enum", "name": "example"}))
        self.assertEqual(
            s.canonical_form,
    ...
    def test_array(self):
        ...
        """
        s = aingle.schema.parse(json.dumps({"items": "long", "type": "array"}))
        self.assertEqual(
            s.canonical_form,
    ...
    def test_map(self):
        ...
        """
        s = aingle.schema.parse(json.dumps({"values": "long", "type": "map"}))
        self.assertEqual(
            s.canonical_form,
    ...
    def test_union(self):
        ...
        """
        s = aingle.schema.parse(json.dumps(["string", "null", "long"]))
    ...
    def test_large_record_handshake_request(self):
        ...
        self.assertEqual(
            s.canonical_form,
            (
                '{"name":"org.apache.aingle.ipc.HandshakeRequest","type":"record",'
                '"fields":[{"name":"clientHash","type":{"name":"org.apache.aingle.ipc.MD5",'
                '"type":"fixed","size":16}},{"name":"clientProtocol","type":["null","string"]},'
                '{"name":"serverHash","type":{"name":"org.apache.aingle.ipc.MD5","type":"fixed","size":16}},'
                '{"name":"meta","type":["null",{"type":"map","values":"bytes"}]}]}'
            ),
        )
    ...
    def test_large_record_handshake_response(self):
        ...
        self.assertEqual(
            s.canonical_form,
            (
                '{"name":"org.apache.aingle.ipc.HandshakeResponse","type":"rec'
                ...
                'ache.aingle.ipc.MD5","type":"fixed","size":16}]},{"name":"met'
    ...
    def test_large_record_interop(self):
        ...
        self.assertEqual(
            s.canonical_form,
            (
                '{"name":"org.apache.aingle.Interop","type":"record","fields":[{"na'
                ...
                '":"map","values":{"name":"org.apache.aingle.Foo","type":"record","'
                'fields":[{"name":"label","type":"string"}]}}},{"name":"unionFiel'
                'd","type":["boolean","double",{"type":"array","items":"bytes"}]}'
                ',{"name":"enumField","type":{"name":"org.apache.aingle.Kind","type'
                '":"enum","symbols":["A","B","C"]}},{"name":"fixedField","type":{'
                '"name":"org.apache.aingle.MD5","type":"fixed","size":16}},{"name":'
                '"recordField","type":{"name":"org.apache.aingle.Node","type":"reco'
                'rd","fields":[{"name":"label","type":"string"},{"name":"children'
                '","type":{"type":"array","items":"org.apache.aingle.Node"}}]}}]}'
            ),
        )
