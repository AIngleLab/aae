...
SCRIPT = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "scripts", "aingle")
...
def gen_aingle(filename):
    schema = aingle.schema.parse(SCHEMA)
    fo = open(filename, "wb")
    writer = aingle.datafile.DataFileWriter(fo, aingle.io.DatumWriter(), schema)
...
class TestCat(unittest.TestCase):
    def setUp(self):
        self.aingle_file = _tempfile()
        gen_aingle(self.aingle_file)

    def tearDown(self):
        if os.path.isfile(self.aingle_file):
            os.unlink(self.aingle_file)

    def _run(self, *args, **kw):
        out = subprocess.check_output([sys.executable, SCRIPT, "cat", self.aingle_file] + list(args)).decode()
        if kw.get("raw"):
            return out
    ...
    def test_print_schema(self):
        ...
        assert json.loads(out)["namespace"] == "test.aingle"
    ...
    def test_files(self):
        out = self._run(self.aingle_file)
...
class TestWrite(unittest.TestCase):
    ...
    def test_outfile(self):
        ...
        assert len(self.load_aingle(tmp)) == NUM_RECORDS
        os.unlink(tmp)
    ...
    def test_multi_file(self):
        ...
        assert len(self.load_aingle(tmp)) == 2 * NUM_RECORDS
        os.unlink(tmp)
