...
_INTEROP_DATA_DIR = Path(aingle.__file__).parent / "test" / "interop" / "data"
...
class TestDataFileInterop(unittest.TestCase):
    def test_interop(self) -> None:
        ...
        for filename in _INTEROP_DATA_DIR.iterdir():
            ...
            if len(base_ext) < 2 or base_ext[1] not in aingle.codecs.KNOWN_CODECS:
                print(f"SKIPPING {filename} due to an unsupported codec\n")
                continue
            i = None
            with self.subTest(filename=filename), aingle.datafile.DataFileReader(filename.open("rb"), aingle.io.DatumReader()) as dfr:
                for i, datum in enumerate(cast(aingle.datafile.DataFileReader, dfr), 1):
