...
from aingle.utils import randbytes

TYPES = ("A", "CNAME")
SCHEMA: aingle.schema.RecordSchema = aingle.schema.parse(
    json.dumps(
        {
            "type": "record",
            "name": "Query",
...
READER = aingle.io.DatumReader(SCHEMA)
WRITER = aingle.io.DatumWriter(SCHEMA)
...
def time_writes(path: Path, number: int) -> float:
    with aingle.datafile.DataFileWriter(path.open("wb"), WRITER, SCHEMA) as dw:
...
def time_read(path: Path) -> float:
    ...
    """
    with aingle.datafile.DataFileReader(path.open("rb"), READER) as dr:
        return timeit.timeit("tuple(dr)", number=1, globals={"dr": dr})


def parse_args() -> argparse.Namespace:  # pragma: no cover
    parser = argparse.ArgumentParser(description="Benchmark writing some random aingle.")
    parser.add_argument(
        "--number",
        "-n",
        type=int,
