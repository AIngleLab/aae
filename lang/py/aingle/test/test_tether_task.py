...
class TestTetherTask(unittest.TestCase):
    ...
    def test_tether_task(self) -> None:
        ...
        subprocess
        """
        task = aingle.test.word_count_task.WordCountTask()
        proc = None
        pyfile = aingle.test.mock_tether_parent.__file__
        server_port = aingle.tether.util.find_port()
        ...
        try:
            ...
            # ***************************************************************
            # Test the mapper
            if aingle.tether.tether_task.TaskType is None:
                self.fail()
            task.configure(
                aingle.tether.tether_task.TaskType.MAP,
                str(task.inschema),
                str(task.midschema),
            )
            ...
            datum_writer = aingle.io.DatumWriter(task.inschema)
            ...
            # Test the reducer
            task.configure(
                aingle.tether.tether_task.TaskType.REDUCE,
                str(task.midschema),
                str(task.outschema),
            )
            ...
            writer = io.BytesIO()
            encoder = aingle.io.BinaryEncoder(writer)
            datum_writer = aingle.io.DatumWriter(task.midschema)
