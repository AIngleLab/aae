...
def make_requestor(server_host, server_port, protocol):
    client = aingle.ipc.HTTPTransceiver(SERVER_HOST, SERVER_PORT)
    return aingle.ipc.Requestor(protocol, client)


if __name__ == "__main__":
    if len(sys.argv) not in [4, 5]:
        raise aingle.errors.UsageError("Usage: <to> <from> <body> [<count>]")
