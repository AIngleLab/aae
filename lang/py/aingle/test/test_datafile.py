...
TEST_PAIRS = tuple(
    (aingle.schema.parse(schema), datum)
    for schema, datum in (
        ('"null"', None),
        ('"boolean"', True),
...
def writer(path, schema=None, codec=aingle.datafile.NULL_CODEC, mode="wb"):
    with aingle.datafile.DataFileWriter(open(path, mode), aingle.io.DatumWriter(), schema, codec) as dfw:
        yield dfw


@contextlib.contextmanager
def reader(path, mode="rb"):
    with aingle.datafile.DataFileReader(open(path, mode), aingle.io.DatumReader()) as dfr:
        yield dfr
...
class TestDataFile(unittest.TestCase):
    ...
    def tempfile(self):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".aingle") as f:
            pass
