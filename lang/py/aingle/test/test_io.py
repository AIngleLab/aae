...
SCHEMAS_TO_VALIDATE = tuple(
    ...
    for schema, datum in (
        ...
        (
            ...
            datetime.datetime(1000, 1, 1, 0, 0, 0, 000000, tzinfo=aingle.timezones.utc),
        ),
        (
            {"type": "long", "logicalType": "timestamp-millis"},
            datetime.datetime(9999, 12, 31, 23, 59, 59, 999000, tzinfo=aingle.timezones.utc),
        ),
        (
            {"type": "long", "logicalType": "timestamp-millis"},
            datetime.datetime(2000, 1, 18, 2, 2, 1, 100000, tzinfo=aingle.timezones.tst),
        ),
        (
        ...
        (
            {"type": "long", "logicalType": "timestamp-micros"},
            datetime.datetime(9999, 12, 31, 23, 59, 59, 999999, tzinfo=aingle.timezones.utc),
        ),
        (
            {"type": "long", "logicalType": "timestamp-micros"},
            datetime.datetime(2000, 1, 18, 2, 2, 1, 123499, tzinfo=aingle.timezones.tst),
        ),
        (
...
LONG_RECORD_SCHEMA = aingle.schema.parse(
    json.dumps(
        {
            "type": "record",
            "name": "Test",
...
def aingle_hexlify(reader: BinaryIO) -> bytes:
...
def write_datum(datum: object, writers_schema: aingle.schema.Schema) -> Tuple[io.BytesIO, aingle.io.BinaryEncoder, aingle.io.DatumWriter]:
    writer = io.BytesIO()
    encoder = aingle.io.BinaryEncoder(writer)
    datum_writer = aingle.io.DatumWriter(writers_schema)
    datum_writer.write(datum, encoder)
    return writer, encoder, datum_writer


def read_datum(buffer: io.BytesIO, writers_schema: aingle.schema.Schema, readers_schema: Optional[aingle.schema.Schema] = None) -> object:
    reader = io.BytesIO(buffer.getvalue())
    decoder = aingle.io.BinaryDecoder(reader)
    datum_reader = aingle.io.DatumReader(writers_schema, readers_schema)
...
class IoValidateTestCase(unittest.TestCase):
    def __init__(self, test_schema: str, test_datum: object) -> None:
        ...
        self.test_schema = aingle.schema.parse(test_schema)
    ...
    def io_valid(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as actual_warnings:
            self.assertTrue(
                aingle.io.validate(self.test_schema, self.test_datum),
    ...
    def io_round_trip(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as actual_warnings:
            ...
            if isinstance(round_trip_datum, decimal.Decimal):
                ...
                expected, round_trip, message = (
                    cast(datetime.datetime, self.test_datum).astimezone(tz=aingle.timezones.utc),
                    round_trip_datum,
...
class BinaryEncodingTestCase(unittest.TestCase):
    def __init__(self, skip: bool, test_type: str, test_datum: object, test_hex: bytes) -> None:
        ...
        self.writers_schema = aingle.schema.parse(f'"{test_type}"')
        self.test_datum = test_datum
    ...
    def check_skip_encoding(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as actual_warnings:
            ...
            datum_reader = aingle.io.DatumReader(self.writers_schema)
...
class SchemaPromotionTestCase(unittest.TestCase):
    def __init__(self, write_type: str, read_type: str) -> None:
        ...
        self.writers_schema = aingle.schema.parse(f'"{write_type}"')
        self.readers_schema = aingle.schema.parse(f'"{read_type}"')
        # Never hide repeated warnings when running this test case.
        warnings.simplefilter("always")
...
class DefaultValueTestCase(unittest.TestCase):
    ...
    def check_default_value(self) -> None:
        ...
        with warnings.catch_warnings(record=True) as actual_warnings:
            ...
            readers_schema = aingle.schema.parse(
                json.dumps(
                    {
                        "type": "record",
                        "name": "Test",
...
class TestMisc(unittest.TestCase):
    def test_decimal_bytes_small_scale(self) -> None:
        """AIngle should raise an AIngleTypeException when attempting to write a decimal with a larger exponent than the schema's scale."""
        datum = decimal.Decimal("3.1415")
        _, _, exp = datum.as_tuple()
        scale = -1 * exp - 1
        schema = aingle.schema.parse(
            json.dumps(
                {
                    "type": "bytes",
                    "logicalType": "decimal",
        ...
        self.assertRaises(aingle.errors.AIngleOutOfScaleException, write_datum, datum, schema)
    ...
    def test_unknown_symbol(self) -> None:
        datum_to_write = "FOO"
        writers_schema = aingle.schema.parse(json.dumps({"type": "enum", "name": "Test", "symbols": ["FOO", "BAR"]}))
        readers_schema = aingle.schema.parse(json.dumps({"type": "enum", "name": "Test", "symbols": ["BAR", "BAZ"]}))

        writer, encoder, datum_writer = write_datum(datum_to_write, writers_schema)
        reader = io.BytesIO(writer.getvalue())
        decoder = aingle.io.BinaryDecoder(reader)
        datum_reader = aingle.io.DatumReader(writers_schema, readers_schema)
        self.assertRaises(aingle.errors.SchemaResolutionException, datum_reader.read, decoder)
    ...
    def test_type_exception_int(self) -> None:
        writers_schema = aingle.schema.parse(
            json.dumps(
                {
                    "type": "record",
                    "name": "Test",
        ...
        with self.assertRaises(aingle.errors.AIngleTypeException) as exc:
    ...
    def test_type_exception_long(self) -> None:
        writers_schema = aingle.schema.parse(json.dumps({"type": "record", "name": "Test", "fields": [{"name": "foo", "type": "long"}]}))
    ...
    def test_type_exception_record(self) -> None:
        ...
        with self.assertRaisesRegex(aingle.errors.AIngleTypeException, r"The datum \".*\" provided for \".*\" is not an example of the schema [\s\S]*"):
            write_datum(datum_to_write, writers_schema)
