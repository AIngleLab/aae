...
Nodes
-----
AIngle schemas contain many different schema types. Data about the schema
...
import aingle.constants
import aingle.errors
import aingle.schema
import aingle.timezones
...
def validate(expected_schema: aingle.schema.Schema, datum: object, raise_on_error: bool = False) -> bool:
    ...
    :param expected_schema: An aingle schema type object representing the schema against
                            which the datum will be validated.
    :param datum: The datum to be validated, A python dictionary or some supported type
    :param raise_on_error: True if a AIngleTypeException should be raised immediately when a
                           validation problem is encountered.
    :raises: AIngleTypeException if datum is invalid and raise_on_error is True
    ...
    while nodes:
        ...
        if valid_node is None:
            if raise_on_error:
                raise aingle.errors.AIngleTypeException(current_node.schema, current_node.name, current_node.datum)
...
class BinaryDecoder:
    ...
    def read_timestamp_millis_from_long(self) -> datetime.datetime:
        ...
        unix_epoch_datetime = datetime.datetime(1970, 1, 1, 0, 0, 0, 0, tzinfo=aingle.timezones.utc)
...
class BinaryEncoder:
    ...
    def write_decimal_bytes(self, datum: decimal.Decimal, scale: int) -> None:
        ...
        if (-1 * exp) > scale:
            raise aingle.errors.AIngleOutOfScaleException(scale, datum, exp)
    ...
    def write_timestamp_millis_long(self, datum: datetime.datetime) -> None:
        ...
        """
        datum = datum.astimezone(tz=aingle.timezones.utc)
        timedelta = datum - datetime.datetime(1970, 1, 1, 0, 0, 0, 0, tzinfo=aingle.timezones.utc)
...
#
class DatumReader:
    """Deserialize AIngle-encoded data into a Python data structure."""

    _writers_schema: Optional[aingle.schema.Schema]
    _readers_schema: Optional[aingle.schema.Schema]

    def __init__(self, writers_schema: Optional[aingle.schema.Schema] = None, readers_schema: Optional[aingle.schema.Schema] = None) -> None:
        """
        As defined in the AIngle specification, we call the schema encoded
    ...
    @property
    def writers_schema(self) -> Optional[aingle.schema.Schema]:
        return self._writers_schema

    @writers_schema.setter
    def writers_schema(self, writers_schema: aingle.schema.Schema) -> None:
        self._writers_schema = writers_schema

    @property
    def readers_schema(self) -> Optional[aingle.schema.Schema]:
        return self._readers_schema

    @readers_schema.setter
    def readers_schema(self, readers_schema: aingle.schema.Schema) -> None:
        self._readers_schema = readers_schema

    def read(self, decoder: "BinaryDecoder") -> object:
        if self.writers_schema is None:
            raise aingle.errors.IONotReadyException("Cannot read without a writer's schema.")
        if self.readers_schema is None:
            self.readers_schema = self.writers_schema
        return self.read_data(self.writers_schema, self.readers_schema, decoder)

    def read_data(self, writers_schema: aingle.schema.Schema, readers_schema: aingle.schema.Schema, decoder: "BinaryDecoder") -> object:
        # schema matching
        if not readers_schema.match(writers_schema):
            raise aingle.errors.SchemaResolutionException("Schemas do not match.", writers_schema, readers_schema)

        logical_type = getattr(writers_schema, "logical_type", None)

        # function dispatch for reading data based on type of writer's schema
        if isinstance(writers_schema, aingle.schema.UnionSchema) and isinstance(readers_schema, aingle.schema.UnionSchema):
            return self.read_union(writers_schema, readers_schema, decoder)

        if isinstance(readers_schema, aingle.schema.UnionSchema):
        ...
        if writers_schema.type == "int":
            if logical_type == aingle.constants.DATE:
                return decoder.read_date_from_int()
            if logical_type == aingle.constants.TIME_MILLIS:
                return decoder.read_time_millis_from_int()
            return decoder.read_int()
        if writers_schema.type == "long":
            if logical_type == aingle.constants.TIME_MICROS:
                return decoder.read_time_micros_from_long()
            if logical_type == aingle.constants.TIMESTAMP_MILLIS:
                return decoder.read_timestamp_millis_from_long()
            if logical_type == aingle.constants.TIMESTAMP_MICROS:
        ...
        if writers_schema.type == "bytes":
            if logical_type == "decimal":
                ...
                if not (isinstance(precision, int) and precision > 0):
                    warnings.warn(aingle.errors.IgnoredLogicalType(f"Invalid decimal precision {precision}. Must be a positive integer."))
                    return decoder.read_bytes()
                scale = writers_schema.get_prop("scale")
                if not (isinstance(scale, int) and scale > 0):
                    warnings.warn(aingle.errors.IgnoredLogicalType(f"Invalid decimal scale {scale}. Must be a positive integer."))
                    return decoder.read_bytes()
                return decoder.read_decimal_from_bytes(precision, scale)
            return decoder.read_bytes()
        if isinstance(writers_schema, aingle.schema.FixedSchema) and isinstance(readers_schema, aingle.schema.FixedSchema):
            if logical_type == "decimal":
                precision = writers_schema.get_prop("precision")
                if not (isinstance(precision, int) and precision > 0):
                    warnings.warn(aingle.errors.IgnoredLogicalType(f"Invalid decimal precision {precision}. Must be a positive integer."))
            ...
            return self.read_fixed(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.EnumSchema) and isinstance(readers_schema, aingle.schema.EnumSchema):
            return self.read_enum(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.ArraySchema) and isinstance(readers_schema, aingle.schema.ArraySchema):
            return self.read_array(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.MapSchema) and isinstance(readers_schema, aingle.schema.MapSchema):
            return self.read_map(writers_schema, readers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.RecordSchema) and isinstance(readers_schema, aingle.schema.RecordSchema):
            # .type in ["record", "error", "request"]:
            return self.read_record(writers_schema, readers_schema, decoder)
        raise aingle.errors.AIngleException(f"Cannot read unknown schema type: {writers_schema.type}")

    def skip_data(self, writers_schema: aingle.schema.Schema, decoder: BinaryDecoder) -> None:
        if writers_schema.type == "null":
        ...
        if isinstance(writers_schema, aingle.schema.FixedSchema):
            return self.skip_fixed(writers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.EnumSchema):
            return self.skip_enum(writers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.ArraySchema):
            return self.skip_array(writers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.MapSchema):
            return self.skip_map(writers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.UnionSchema):
            return self.skip_union(writers_schema, decoder)
        if isinstance(writers_schema, aingle.schema.RecordSchema):
            return self.skip_record(writers_schema, decoder)
        raise aingle.errors.AIngleException(f"Unknown schema type: {writers_schema.type}")

    def read_fixed(self, writers_schema: aingle.schema.FixedSchema, readers_schema: aingle.schema.Schema, decoder: BinaryDecoder) -> bytes:
        """
    ...
    def skip_fixed(self, writers_schema: aingle.schema.FixedSchema, decoder: BinaryDecoder) -> None:
        return decoder.skip(writers_schema.size)

    def read_enum(self, writers_schema: aingle.schema.EnumSchema, readers_schema: aingle.schema.EnumSchema, decoder: BinaryDecoder) -> str:
        """
        ...
        if index_of_symbol >= len(writers_schema.symbols):
            raise aingle.errors.SchemaResolutionException(
        ...
        if read_symbol not in readers_schema.symbols:
            raise aingle.errors.SchemaResolutionException(f"Symbol {read_symbol} not present in Reader's Schema", writers_schema, readers_schema)

        return read_symbol

    def skip_enum(self, writers_schema: aingle.schema.EnumSchema, decoder: BinaryDecoder) -> None:
        return decoder.skip_int()

    def read_array(self, writers_schema: aingle.schema.ArraySchema, readers_schema: aingle.schema.ArraySchema, decoder: BinaryDecoder) -> List[object]:
        """
    ...
    def skip_array(self, writers_schema: aingle.schema.ArraySchema, decoder: BinaryDecoder) -> None:
        block_count = decoder.read_long()
        while block_count != 0:
            if block_count < 0:
                block_size = decoder.read_long()
    ...
    def read_map(self, writers_schema: aingle.schema.MapSchema, readers_schema: aingle.schema.MapSchema, decoder: BinaryDecoder) -> Mapping[str, object]:
        """
    ...
    def skip_map(self, writers_schema: aingle.schema.MapSchema, decoder: BinaryDecoder) -> None:
        block_count = decoder.read_long()
        while block_count != 0:
            if block_count < 0:
                block_size = decoder.read_long()
    ...
    def read_union(self, writers_schema: aingle.schema.UnionSchema, readers_schema: aingle.schema.UnionSchema, decoder: BinaryDecoder) -> object:
        """
    ...
    def skip_union(self, writers_schema: aingle.schema.UnionSchema, decoder: BinaryDecoder) -> None:
        index_of_schema = int(decoder.read_long())
        if index_of_schema >= len(writers_schema.schemas):
            raise aingle.errors.SchemaResolutionException(
    ...
    def read_record(
        self, writers_schema: aingle.schema.RecordSchema, readers_schema: aingle.schema.RecordSchema, decoder: BinaryDecoder
    ) -> Mapping[str, object]:
        """
            ...
            for field_name, field in readers_fields_dict.items():
                if field_name not in writers_fields_dict:
                    if not field.has_default:
                        raise aingle.errors.SchemaResolutionException(f"No default value for field {field_name}", writers_schema, readers_schema)
                    field_val = self._read_default_value(field.type, field.default)
                    read_record[field.name] = field_val
        return read_record

    def skip_record(self, writers_schema: aingle.schema.RecordSchema, decoder: BinaryDecoder) -> None:
        for field in writers_schema.fields:
            self.skip_data(field.type, decoder)

    def _read_default_value(self, field_schema: aingle.schema.Schema, default_value: object) -> object:
        """
        ...
        if field_schema.type == "null":
            if default_value is None:
                return None
            raise aingle.errors.InvalidDefaultException(field_schema, default_value)
        ...
        if field_schema.type in ("enum", "string"):
            if isinstance(default_value, str):
                return default_value
            raise aingle.errors.InvalidDefaultException(field_schema, default_value)
        if isinstance(field_schema, aingle.schema.ArraySchema):
        ...
        if isinstance(field_schema, aingle.schema.RecordSchema):
            ...
            for field in field_schema.fields:
                ...
                read_record[field.name] = field_val
            return read_record
        raise aingle.errors.AIngleException(f"Unknown type: {field_schema.type}")
...
class DatumWriter:
    ...
    def __init__(self, writers_schema: Optional[aingle.schema.Schema] = None) -> None:
        self._writers_schema = writers_schema
    ...
    def write(self, datum: object, encoder: BinaryEncoder) -> None:
        if self.writers_schema is None:
            raise aingle.errors.IONotReadyException("Cannot write without a writer's schema.")
        validate(self.writers_schema, datum, raise_on_error=True)
        self.write_data(self.writers_schema, datum, encoder)

    def write_data(self, writers_schema: aingle.schema.Schema, datum: object, encoder: BinaryEncoder) -> None:
        # function dispatch to write datum
        logical_type = getattr(writers_schema, "logical_type", None)
        if writers_schema.type == "null":
            if datum is None:
                return encoder.write_null(datum)
            raise aingle.errors.AIngleTypeException(writers_schema, datum)
        if writers_schema.type == "boolean":
        ...
        if writers_schema.type == "int":
            if logical_type == aingle.constants.DATE:
                ...
                warnings.warn(aingle.errors.IgnoredLogicalType(f"{datum} is not a date type"))
            elif logical_type == aingle.constants.TIME_MILLIS:
                if isinstance(datum, datetime.time):
                    return encoder.write_time_millis_int(datum)
                warnings.warn(aingle.errors.IgnoredLogicalType(f"{datum} is not a time type"))
        ...
        if writers_schema.type == "long":
            if logical_type == aingle.constants.TIME_MICROS:
                ...
                warnings.warn(aingle.errors.IgnoredLogicalType(f"{datum} is not a time type"))
            elif logical_type == aingle.constants.TIMESTAMP_MILLIS:
                if isinstance(datum, datetime.datetime):
                    return encoder.write_timestamp_millis_long(datum)
                warnings.warn(aingle.errors.IgnoredLogicalType(f"{datum} is not a datetime type"))
            elif logical_type == aingle.constants.TIMESTAMP_MICROS:
                if isinstance(datum, datetime.datetime):
        ...
        if writers_schema.type == "bytes":
            if logical_type == "decimal":
                ...
                if not (isinstance(scale, int) and scale > 0):
                    ...
                    warnings.warn(aingle.errors.IgnoredLogicalType(f"{datum} is not a decimal type"))
                else:
        ...
        if isinstance(writers_schema, aingle.schema.RecordSchema):
            ...
            raise aingle.errors.AIngleTypeException(writers_schema, datum)
        raise aingle.errors.AIngleException(f"Unknown type: {writers_schema.type}")

    def write_fixed(self, writers_schema: aingle.schema.FixedSchema, datum: bytes, encoder: BinaryEncoder) -> None:
        """
        Fixed instances are encoded using the number of bytes declared
        in the schema.
        """
    ...
    def write_enum(self, writers_schema: aingle.schema.EnumSchema, datum: str, encoder: BinaryEncoder) -> None:
        """
        An enum is encoded by a int, representing the zero-based position
        of the symbol in the schema.
        """
    ...
    def write_array(self, writers_schema: aingle.schema.ArraySchema, datum: Sequence[object], encoder: BinaryEncoder) -> None:
        """
        Arrays are encoded as a series of blocks.
    ...
    def write_map(self, writers_schema: aingle.schema.MapSchema, datum: Mapping[str, object], encoder: BinaryEncoder) -> None:
        """
        Maps are encoded as a series of blocks.
    ...
    def write_union(self, writers_schema: aingle.schema.UnionSchema, datum: object, encoder: BinaryEncoder) -> None:
        """
        A union is encoded by first writing an int value indicating
        the zero-based position within the union of the schema of its value.
        The value is then encoded per the indicated schema within the union.
    ...
    def write_record(self, writers_schema: aingle.schema.RecordSchema, datum: Mapping[str, object], encoder: BinaryEncoder) -> None:
        """
        A record is encoded by encoding the values of its fields
        in the order that they are declared. In other words, a record
        is encoded as just the concatenation of the encodings of its fields.
