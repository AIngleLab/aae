...
"""
Contains Codecs for Python AIngle.

Note that the word "codecs" means "compression/decompression algorithms" in the
AIngle world (https://apache.aingle.ai/docs/current/spec.html#Object+Container+Files),
...
def _check_crc32(bytes_: bytes, checksum: bytes) -> None:
    if binascii.crc32(bytes_) & 0xFFFFFFFF != STRUCT_CRC32.unpack(checksum)[0]:
        raise aingle.errors.AIngleException("Checksum failure")
...
class Codec(abc.ABC):
    """Abstract base class for all AIngle codec classes."""
    ...
    @staticmethod
    @abc.abstractmethod
    def decompress(readers_decoder: aingle.io.BinaryDecoder) -> aingle.io.BinaryDecoder:
        ...
        :type readers_decoder: aingle.io.BinaryDecoder

        :rtype: aingle.io.BinaryDecoder
...
class DeflateCodec(Codec):
    ...
    def decompress(readers_decoder: aingle.io.BinaryDecoder) -> aingle.io.BinaryDecoder:
        ...
        return aingle.io.BinaryDecoder(io.BytesIO(uncompressed))
...
def get_codec(codec_name: str) -> Type[Codec]:
    try:
        ...
        raise aingle.errors.UnsupportedCodec(f"Unsupported codec: {codec_name}. (Is it installed?)")
