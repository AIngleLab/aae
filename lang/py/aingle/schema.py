...
from aingle.constants import NAMED_TYPES, PRIMITIVE_TYPES, VALID_TYPES
from aingle.name import Name, Names, validate_basename
...
class Schema(abc.ABC, CanonicalPropertiesMixin):
    ...
    def __init__(self, type_: str, other_props: Optional[Mapping[str, object]] = None) -> None:
        if not isinstance(type_, str):
            raise aingle.errors.SchemaParseException("Schema type must be a string.")
        if type_ not in VALID_TYPES:
            raise aingle.errors.SchemaParseException(f"{type_} is not a valid type.")
    ...
    def to_json(self, names: Optional[Names] = None) -> object:
        """
        Converts the schema object into its AINGLE specification representation.
    ...
    def to_canonical_json(self, names: Optional[Names] = None) -> object:
        ...
        http://aingle.apache.org/docs/current/spec.html#Parsing+Canonical+Form+for+Schemas
...
class NamedSchema(Schema):
    ...
    def __init__(
        ...
        if not name:
            raise aingle.errors.SchemaParseException("Named Schemas must have a non-empty name.")
        if not isinstance(name, str):
            raise aingle.errors.SchemaParseException("The name property must be a string.")
        if namespace is not None and not isinstance(namespace, str):
            raise aingle.errors.SchemaParseException("The namespace property must be a string.")
...
class DecimalLogicalSchema(LogicalSchema):
    def __init__(self, precision, scale=0, max_precision=0):
        if not isinstance(precision, int) or precision <= 0:
            raise aingle.errors.IgnoredLogicalType(f"Invalid decimal precision {precision}. Must be a positive integer.")

        if precision > max_precision:
            raise aingle.errors.IgnoredLogicalType(f"Invalid decimal precision {precision}. Max is {max_precision}.")

        if not isinstance(scale, int) or scale < 0:
            raise aingle.errors.IgnoredLogicalType(f"Invalid decimal scale {scale}. Must be a positive integer.")

        if scale > precision:
            raise aingle.errors.IgnoredLogicalType(f"Invalid decimal scale {scale}. Cannot be greater than precision {precision}.")
...
class Field(CanonicalPropertiesMixin, EqualByJsonMixin):
    ...
    def __init__(
        ...
        if not name:
            raise aingle.errors.SchemaParseException("Fields must have a non-empty name.")
        if not isinstance(name, str):
            raise aingle.errors.SchemaParseException("The name property must be a string.")
        if order is not None and order not in VALID_FIELD_SORT_ORDERS:
            raise aingle.errors.SchemaParseException(f"The order property {order} is not valid.")
        ...
        if isinstance(type_, str) and names is not None and names.has_name(type_, None):
            ...
            try:
                type_schema = make_ain_object(type_, names)
            except Exception as e:
                raise aingle.errors.SchemaParseException(f'Type property "{type_}" not a valid AIngle schema: {e}')
...
class PrimitiveSchema(EqualByPropsMixin, Schema):
    ...
    def __init__(self, type, other_props=None):
        ...
        if type not in PRIMITIVE_TYPES:
            raise aingle.errors.AIngleException(f"{type} is not a valid primitive type.")
...
class FixedSchema(EqualByPropsMixin, NamedSchema):
    def __init__(self, name, namespace, size, names=None, other_props=None):
        ...
        if not isinstance(size, int) or size < 0:
            ...
            raise aingle.errors.AIngleException(fail_msg)
...
class EnumSchema(EqualByPropsMixin, NamedSchema):
    def __init__(
        ...
        """
        @arg validate_enum_symbols: If False, will allow enum symbols that are not valid AIngle names.
        """
        if validate_enum_symbols:
            for symbol in symbols:
                try:
                    validate_basename(symbol)
                except aingle.errors.InvalidName:
                    raise aingle.errors.InvalidName("An enum symbol must be a valid schema name.")

        if len(set(symbols)) < len(symbols):
            raise aingle.errors.AIngleException(f"Duplicate symbol: {symbols}")
...
class ArraySchema(EqualByJsonMixin, Schema):
    def __init__(self, items, names=None, other_props=None):
        ...
        if isinstance(items, str) and names.has_name(items, None):
            ...
            try:
                items_schema = make_ain_object(items, names)
            except aingle.errors.SchemaParseException as e:
                fail_msg = f"Items schema ({items}) not a valid AIngle schema: {e} (known names: {names.names.keys()})"
                raise aingle.errors.SchemaParseException(fail_msg)
...
class MapSchema(EqualByJsonMixin, Schema):
    def __init__(self, values, names=None, other_props=None):
        ...
        if isinstance(values, str) and names.has_name(values, None):
            ...
            try:
                values_schema = make_ain_object(values, names)
            except aingle.errors.SchemaParseException:
                raise
            except Exception:
                raise aingle.errors.SchemaParseException("Values schema is not a valid AIngle schema.")
    ...
    def __init__(self, schemas, names=None):
        ...
        for schema in schemas:
            if isinstance(schema, str) and names.has_name(schema, None):
                ...
                try:
                    new_schema = make_ain_object(schema, names)
                except Exception as e:
                    raise aingle.errors.SchemaParseException(f"Union item must be a valid AIngle schema: {e}")
            ...
            if (
                ...
                raise aingle.errors.SchemaParseException(f"{new_schema.type} type already in Union")
            elif new_schema.type == "union":
                raise aingle.errors.SchemaParseException("Unions cannot contain other unions.")
            else:
...
class RecordSchema(EqualByJsonMixin, NamedSchema):
    @staticmethod
    def make_field_objects(field_data: Sequence[Mapping[str, object]], names: aingle.name.Names) -> Sequence[Field]:
        """We're going to need to make message parameters too."""
        field_objects = []
        field_names = []
        for field in field_data:
            if not callable(getattr(field, "get", None)):
                raise aingle.errors.SchemaParseException(f"Not a valid field: {field}")
            type = field.get("type")
            name = field.get("name")
    ...
    def __init__(
        ...
        if fields is None:
            ...
            fail_msg = "Fields property must be a list of AIngle schemas."
            raise aingle.errors.SchemaParseException(fail_msg)
...
class DateSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.DATE)
...
class TimeMillisSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.TIME_MILLIS)
        PrimitiveSchema.__init__(self, "int", other_props)
...
class TimeMicrosSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.TIME_MICROS)
...
class TimestampMillisSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.TIMESTAMP_MILLIS)
        PrimitiveSchema.__init__(self, "long", other_props)
...
class TimestampMicrosSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.TIMESTAMP_MICROS)
        PrimitiveSchema.__init__(self, "long", other_props)
...
class UUIDSchema(LogicalSchema, PrimitiveSchema):
    def __init__(self, other_props=None):
        LogicalSchema.__init__(self, aingle.constants.UUID)
...
def make_logical_schema(logical_type, type_, other_props):
    ...
    logical_types = {
        (aingle.constants.DATE, "int"): DateSchema,
        (aingle.constants.DECIMAL, "bytes"): make_bytes_decimal_schema,
        # The fixed decimal schema is handled later by returning None now.
        (aingle.constants.DECIMAL, "fixed"): lambda x: None,
        (aingle.constants.TIMESTAMP_MICROS, "long"): TimestampMicrosSchema,
        (aingle.constants.TIMESTAMP_MILLIS, "long"): TimestampMillisSchema,
        (aingle.constants.TIME_MICROS, "long"): TimeMicrosSchema,
        (aingle.constants.TIME_MILLIS, "int"): TimeMillisSchema,
        (aingle.constants.UUID, "string"): UUIDSchema,
    }
    try:
        ...
        if expected_types:
            warnings.warn(
                aingle.errors.IgnoredLogicalType(f"Logical type {logical_type} requires literal type {'/'.join(expected_types)}, not {type_}.")
            )
        else:
            warnings.warn(aingle.errors.IgnoredLogicalType(f"Unknown {logical_type}, using {type_}."))
    except aingle.errors.IgnoredLogicalType as warning:
        warnings.warn(warning)
    return None


def make_ain_object(json_data: object, names: Optional[aingle.name.Names] = None, validate_enum_symbols: bool = True) -> Schema:
    """
    Build AIngle Schema from data parsed out of JSON string.
    ...
    if callable(getattr(json_data, "get", None)):
        ...
        if type_ in NAMED_TYPES:
            ...
            if not isinstance(name, str):
                raise aingle.errors.SchemaParseException(f"Name {name} must be a string, but it is {type(name)}.")
            ...
            if type_ == "fixed":
                ...
                if not isinstance(symbols, Sequence):
                    raise aingle.errors.SchemaParseException(f"Enum symbols must be a sequence of strings, but it is {type(symbols)}")
                for symbol in symbols:
                    if not isinstance(symbol, str):
                        raise aingle.errors.SchemaParseException(f"Enum symbols must be a sequence of strings, but one symbol is a {type(symbol)}")
            ...
            raise aingle.errors.SchemaParseException(f"Unknown Named Type: {type_}")
        ...
        if type_ in VALID_TYPES:
            if type_ == "array":
                ...
                raise aingle.errors.SchemaParseException(f"Unknown Valid Type: {type_}")
        elif type_ is None:
            raise aingle.errors.SchemaParseException(f'No "type" property: {json_data}')
        else:
            raise aingle.errors.SchemaParseException(f"Undefined type: {type_}")
    # JSON array (union)
    ...
    # not for us!
    fail_msg = f"Could not make an AIngle Schema object from {json_data}"
    raise aingle.errors.SchemaParseException(fail_msg)
...
def parse(json_string, validate_enum_symbols=True):
    ...
    try:
        ...
        new_exception = aingle.errors.SchemaParseException(msg)
    ...
    # construct the AIngle Schema object
    return make_ain_object(json_data, names, validate_enum_symbols)
