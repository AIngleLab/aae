...
from aingle.constants import VALID_TYPES

if TYPE_CHECKING:
    from aingle.schema import NamedSchema
...
def validate_basename(basename: str) -> None:
    ...
    if not _BASE_NAME_PATTERN.search(basename):
        raise aingle.errors.InvalidName(f"{basename!s} is not a valid AIngle name because it does not match the pattern {_BASE_NAME_PATTERN.pattern!s}")
...
class Name:
    ...
    def __init__(self, name_attr: Optional[str] = None, space_attr: Optional[str] = None, default_space: Optional[str] = None) -> None:
        ...
        if name_attr is None:
            return
        if name_attr == "":
            raise aingle.errors.SchemaParseException("Name must not be the empty string.")
...
class Names:
    ...
    def add_name(self, name_attr: str, space_attr: Optional[str], new_schema: "NamedSchema") -> Name:
        ...
        if to_add.fullname in VALID_TYPES:
            raise aingle.errors.SchemaParseException(f"{to_add.fullname} is a reserved type name.")
        if to_add.fullname in self.names:
            raise aingle.errors.SchemaParseException(f'The name "{to_add.fullname}" is already in use.')
        if to_add.fullname is None:
            raise aingle.errors.SchemaParseException(f'The name built from "{space_attr}.{name_attr}" is None')
