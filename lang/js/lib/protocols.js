...
/**
 * This module implements AIngle's IPC/RPC logic.
 *
...
var HANDSHAKE_REQUEST_TYPE = schemas.createType({
  ...
  fields: [
    ...
    {name: 'serverHash', type: 'org.apache.aingle.ipc.MD5'},
    {
      name: 'meta',
...
Protocol.prototype.emit = function (name, req, emitter, cb) {
  ...
  if (
    ...
    asyncAIngleCb(this, cb, 'invalid emitter');
    return;
  }

  var message = this._messages[name];
  if (!message) {
    asyncAIngleCb(this, cb, f('unknown message: %s', name));
    return;
  }
...
MessageEmitter.prototype._finalizeHandshake = function (tap, handshakeReq) {
  ...
  if (res.serverHash && res.serverProtocol) {
    ...
    hashString = res.serverHash['org.apache.aingle.ipc.MD5'].toString('binary');
...
StatelessEmitter.prototype._emit = function (message, req, cb) {
  ...
  this._pending[id] = cb;
  if (this._destroyed) {
    asyncAIngleCb(undefined, done, 'emitter destroyed');
    return;
  }
  emit(false);
  ...
  function emit(retry) {
    ...
    try {
      ...
      asyncAIngleCb(undefined, done, err);
      return;
    }
...
StatefulEmitter.prototype._emit = function (message, req, cb) {
  if (this._destroyed) {
    asyncAIngleCb(this._ptcl, cb, 'emitter destroyed');
    return;
  }
  ...
  try {
    ...
    asyncAIngleCb(this._ptcl, cb, err);
    return;
  }
...
MessageListener.prototype._validateHandshake = function (reqTap, resTap) {
  ...
  var handshakeRes = new HandshakeResponse(
    ...
    serverMatch ? null : {'org.apache.aingle.ipc.MD5': getHash(this._ptcl)},
...
MessageListener.prototype._encodeSystemError = function (tap, err) {
  ...
  safeWrite(tap, SYSTEM_ERROR_TYPE, aingleError(err));
};
...
/**
 * Default ID generator, using AIngle messages' metadata field.
 *
 ...
 * @param type {Type} AIngle type.
 * @param val {...} Corresponding AIngle value.
 *
 */
 ...
 * instance or a string, it will be converted into valid format for AIngle.
 * @param res {...} Response. Passed as second argument to `cb`.
 *
 */
function asyncAIngleCb(ctx, cb, err, res) {
  process.nextTick(function () { cb.call(ctx, aingleError(err), res); });
}
