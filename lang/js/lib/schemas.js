 ...
 * or `aingle.js` (browserify) which each add convenience functionality.
 *
 */
...
/**
 * "Abstract" base AIngle type class.
 *
...
Type.prototype.createResolver = function (type, opts) {
  ...
  if (this instanceof RecordType && type instanceof RecordType) {
    key = this._name + ':' + type._name; // ':' is illegal in AIngle type names.
 ...
 * (similarly to AIngle's python implementation). It was removed to keep all
...
RecordType.prototype._createConstructor = function (isError) {
  ...
  // because it is an invalid property name in AIngle but not in JavaScript.
...
function Field(attrs, opts) {
  ...
  if (value !== undefined) {
    ...
    // allowed instead).
    // http://apache-aingle.679487.n3.nabble.com/field-union-default-in-Java-td1175327.html
    var type = this._type;
