...
var BLOCK_TYPE = schemas.createType({
  type: 'record',
  name: 'org.apache.aingle.file.Block',
  fields : [
...
// First 4 bytes of an AIngle object container file.
...
BlockDecoder.prototype._decodeHeader = function () {
  ...
  var codec = (header.meta['aingle.codec'] || 'null').toString();
  ...
  try {
    var schema = JSON.parse(header.meta['aingle.schema'].toString());
...
BlockEncoder.prototype._write = function (val, encoding, cb) {
  ...
  if (!this._omitHeader) {
    var meta = {
      'aingle.schema': new Buffer(this._schema || this._type.getSchema()),
      'aingle.codec': new Buffer(this._codec)
    };
...
function extractFileHeader(path, opts) {
  ...
  if (MAGIC_BYTES.equals(buf.slice(0, 4))) {
    ...
    if (decode !== false) {
      ...
      meta['aingle.schema'] = JSON.parse(meta['aingle.schema'].toString());
      if (meta['aingle.codec'] !== undefined) {
        meta['aingle.codec'] = meta['aingle.codec'].toString();
      }
    }
  }
  fs.closeSync(fd);
...
/**
 * Readable stream of records from a local AIngle file.
 *
 */
...
/**
 * Writable stream of records to a local AIngle file.
 *
 */
