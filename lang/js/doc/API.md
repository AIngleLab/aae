...
+ `schema` {Object|String} An AIngle schema, represented by one of:
...
+ `opts` {Object} Parsing options. The following keys are currently supported:
  ...
  + `assertLogicalTypes` {Boolean} The AIngle specification mandates that we fall
...
```javascript
var longType = new aingle.types.LongType();
...
All the classes below are available in the `aingle.types` namespace:
...
"Abstract" base AIngle type class; all implementations inherit from it.
...
Returns a `Buffer` containing the AIngle serialization of `val`.
...
+ `opts` {Object} Options:
  ...
  + `fieldHook(field, any, type)` {Function} Function called when each record
    ...
    parent type.
  + `wrapUnions` {Boolean} AIngle's JSON representation expects all union values
...
AIngle [resolution rules][schema-resolution]. If the schemas are incompatible,
...
+ `fromJSON(any)`
  ...
  + `any` {Number|...} Parsed value. To ensure that the `fromString` method
    works correctly on data JSON-serialized according to the AIngle spec, this
  ...
  numbers without loss of precision (at the cost of violating the AIngle spec).
...
Get the underlying AIngle type. This can be useful when a logical type can
...
+ `path` {String} Path to AIngle container file.
+ `opts` {Object} Decoding options, passed to
  [`BlockDecoder`](Api#class-blockdecoderopts).

Returns a readable stream of decoded objects from an AIngle container file.
...
Returns a writable stream of objects. These will end up serialized into an AIngle
container file.
...
Extract header from an AIngle container file synchronously. If no header is
present (i.e. the path doesn't point to a valid AIngle container file), `null` is
returned.


For more specific use-cases, the following stream classes are available in the
`aingle.streams` namespace:
...
A duplex stream which decodes bytes coming from on AIngle object container file.
...
A duplex stream which can be used to decode a stream of serialized AIngle objects
...
A duplex stream to create AIngle container object files.
...
AIngle also defines a way of executing remote procedure calls. We expose this via
...
[canonical-schema]: https://apache.aingle.ai/docs/current/spec.html#Parsing+Canonical+Form+for+Schemas
[schema-resolution]: https://apache.aingle.ai/docs/current/spec.html#Schema+Resolution
[sort-order]: https://apache.aingle.ai/docs/current/spec.html#order
[fingerprint]: https://apache.aingle.ai/docs/current/spec.html#Schema+Fingerprints
[custom-long]: Advanced-usage#custom-long-types
[logical-types]: Advanced-usage#logical-types
[framing-messages]: https://apache.aingle.ai/docs/current/spec.html#Message+Framing
