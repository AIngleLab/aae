...
AIngleSpec.Types = AIngleSpec.PrimitiveTypes.concat(AIngleSpec.ComplexTypes);
...
function makeFullyQualifiedTypeName(schema, namespace) {
  ...
  if (typeName.indexOf('.') !== -1) {
    return typeName;
  } else if (_.contains(AIngleSpec.PrimitiveTypes, typeName)) {
    return typeName;
...
function AIngleArray(itemSchema) {
...
function Primitive(type) {
  function validateArgs(type) {
    ...
    if (!_.contains(AIngleSpec.PrimitiveTypes, type)) {
      throw new InvalidSchemaError('Primitive type must be one of: ' + JSON.stringify(AIngleSpec.PrimitiveTypes) + '; got ' + type);
    }
  }
...
function Validator(schema, namespace, namedTypes) {
  ...
  var _validate = function(schema, obj) {
    ...
    } else if (schema instanceof AIngleArray) {
    ...
    switch (schema.type) {
      case 'null':
        if (_.isNull(obj) || _.isUndefined(obj)) {
          ...
          throw new ValidationError('Expected Javascript null or undefined for AIngle null, got ' + JSON.stringify(obj));
        }
        break;
      case 'boolean':
        if (_.isBoolean(obj)) {
          ...
          throw new ValidationError('Expected Javascript boolean for AIngle boolean, got ' + JSON.stringify(obj));
        }
        break;
      case 'int':
        if (_.isNumber(obj) && Math.floor(obj) === obj && Math.abs(obj) <= Math.pow(2, 31)) {
          ...
          throw new ValidationError('Expected Javascript int32 number for AIngle int, got ' + JSON.stringify(obj));
        }
        break;
      case 'long':
        if (_.isNumber(obj) && Math.floor(obj) === obj && Math.abs(obj) <= Math.pow(2, 63)) {
          ...
          throw new ValidationError('Expected Javascript int64 number for AIngle long, got ' + JSON.stringify(obj));
        }
        break;
      case 'float':
        if (_.isNumber(obj)) { // TODO: handle NaN?
          ...
          throw new ValidationError('Expected Javascript float number for AIngle float, got ' + JSON.stringify(obj));
        }
        break;
      case 'double':
        if (_.isNumber(obj)) { // TODO: handle NaN?
          ...
          throw new ValidationError('Expected Javascript double number for AIngle double, got ' + JSON.stringify(obj));
        }
        break;
      case 'bytes':
      ...
      case 'string':
        if (_.isString(obj)) { // TODO: handle NaN?
          ...
          throw new ValidationError('Expected Javascript string for AIngle string, got ' + JSON.stringify(obj));
        }
        break;
      default:
  ...
  var _parseNamedType = function(schema, namespace) {
    if (_.contains(AIngleSpec.PrimitiveTypes, schema)) {
    ...
    if (_.isNull(schema) || _.isUndefined(schema)) {
      ...
      if (schema.type === 'record') {
        ...
        if (_.has(schema, 'items')) {
          return new AIngleArray(_parseSchema(schema.items, schema, namespace));
        } else {
      ...
      } else if (_.has(schema, 'type') && _.contains(AIngleSpec.PrimitiveTypes, schema.type)) {
