...
suite('files', function () {
  ...
  suite('parse', function () {
    ...
    test('file', function () {
      ...
      var t2 = parse(path.join(__dirname, 'dat', 'Id.ain'));
  ...
  suite('BlockDecoder', function () {
    ...
    test('invalid sync marker', function (cb) {
      ...
      var header = new Header(
        MAGIC_BYTES,
        {
          'aingle.schema': new Buffer('"int"'),
          'aingle.codec': new Buffer('null')
        },
        SYNC
      );
  ...
  test('createFileDecoder', function (cb) {
    var n = 0;
    var type = loadSchema(path.join(DPATH, 'Person.ain'));
    files.createFileDecoder(path.join(DPATH, 'person-10.aingle'))
  ...
  test('extractFileHeader', function () {
    var header;
    var fpath = path.join(DPATH, 'person-10.aingle');
    header = files.extractFileHeader(fpath);
    assert(header !== null);
    assert.equal(typeof header.meta['aingle.schema'], 'object');
    header = files.extractFileHeader(fpath, {decode: false});
    assert(Buffer.isBuffer(header.meta['aingle.schema']));
    header = files.extractFileHeader(fpath, {size: 2});
    assert.equal(typeof header.meta['aingle.schema'], 'object');
    header = files.extractFileHeader(path.join(DPATH, 'person-10.aingle.raw'));
    assert(header === null);
    header = files.extractFileHeader(
      path.join(DPATH, 'person-10.no-codec.aingle')
    );
    assert(header !== null);
  });
