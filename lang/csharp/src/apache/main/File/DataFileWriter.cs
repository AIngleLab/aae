...
namespace AIngle.File
{
    ...
    public class DataFileWriter<T> : IFileWriter<T>
    {
        ...
        public static IFileWriter<T> OpenAppendWriter(DatumWriter<T> writer, Stream inStream, Stream outStream)
        {
            if (!inStream.CanRead)
            {
                throw new AIngleRuntimeException($"{nameof(inStream)} must have Read access");
            }

            if (!outStream.CanWrite)
            {
                throw new AIngleRuntimeException($"{nameof(outStream)} must have Write access");
            }
        ...
        public void SetMeta(string key, byte[] value)
        {
            ...
            {
                throw new AIngleRuntimeException("Cannot set reserved meta key: " + key);
            }
        ...
        public void SetMeta(string key, long value)
        {
            ...
            {
                throw new AIngleRuntimeException(e.Message, e);
            }
        }
        ...
        public void SetSyncInterval(int syncInterval)
        {
            ...
            {
                throw new AIngleRuntimeException("Invalid sync interval value: " + syncInterval);
            }
        ...
        public void Append(T datum)
        {
            ...
            catch (Exception e)
            {
                ...
                throw new AIngleRuntimeException("Error appending datum to writer", e);
            }
            _blockCount++;
            WriteIfBlockFull();
        }
        ...
        {
            if (!_isOpen) throw new AIngleRuntimeException("Cannot complete operation: aingle file/stream not open");
        }
        ...
        private void WriteMetaData()
        {
            ...
            GenerateSyncData();
            //SetMetaInternal(DataFileConstants.MetaDataSync, _syncData); - AIngle 1.5.4 C
