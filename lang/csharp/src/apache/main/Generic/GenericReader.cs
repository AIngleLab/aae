...
{
    /// <summary>
    /// A function that can read the AIngle type from the stream.
    /// </summary>
    ...
    /// <summary>
    /// A general purpose reader of data from aingle streams. This can optionally resolve if the reader's and writer's
    ...
    /// <summary>
    /// The default implementation for the generic reader. It constructs new .NET objects for aingle objects on the
    /// stream and returns the .NET object. Users can directly use this class or, if they want to customize the
    /// object types for differnt AIngle schema types, can derive from this class. There are enough hooks in this
    /// class to allow customization.
    /// </summary>
    /// <remarks>
    /// <list type="table">
    /// <listheader><term>AIngle Type</term><description>.NET Type</description></listheader>
    ...
    public class DefaultReader
    {
        ...
        /// reader's and writer's schemas are different this class performs the resolution.
        /// This default implemenation maps AIngle types to .NET types as follows:
        /// </summary>
        /// <param name="writerSchema">The schema used while generating the data</param>
        /// <param name="readerSchema">The schema desired by the reader</param>
        ...
        public T Read<T>(T reuse, Decoder decoder)
        {
            if (!ReaderSchema.CanRead(WriterSchema))
                throw new AIngleException("Schema mismatch. Reader: " + ReaderSchema + ", writer: " + WriterSchema);
        ...
        public object Read(object reuse, Schema writerSchema, Schema readerSchema, Decoder d)
        {
            ...
            */
            switch (writerSchema.Tag)
            {
                ...
                default:
                    throw new AIngleException("Unknown schema type: " + writerSchema);
            }
        }
        ...
        /// <param name="tag">The AIngle type tag for the object on the stream</param>
        /// <param name="readerSchema">A schema compatible to the AIngle type</param>
        /// <param name="reader">A function that can read the aingle type from the stream</param>
        ...
        /// <returns>An object suitable to deserialize an aingle array</returns>
        ...
        /// <summary>
        /// Deserialized an aingle map. The default implemenation creats a new map using CreateMap() and then
        ...
        protected virtual object ReadFixed(object reuse, FixedSchema writerSchema, Schema readerSchema, Decoder d)
        {
            ...
            {
                throw new AIngleException("Size mismatch between reader and writer fixed schemas. Writer: " + writerSchema +
                    ", reader: " + readerSchema);
            }
        ...
        protected static Schema findBranch(UnionSchema us, Schema s)
        {
            ...
            throw new AIngleException("No matching schema for " + s + " in " + us);
        }
