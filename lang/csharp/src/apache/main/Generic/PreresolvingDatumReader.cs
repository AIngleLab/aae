...
{
    /// <summary>
    /// A general purpose reader of data from aingle streams. This reader analyzes and resolves the reader and writer schemas
            ...
            return (r, d) =>
                        {
                            ...
                            if (readerOrdinal == -1)
                            {
                                throw new AIngleException("No such symbol: " + writerSchema[writerOrdinal]);
                            }
        ...
        private ReadItem ResolveUnion(UnionSchema writerSchema, Schema readerSchema)
        {
            ...
            for (int i = 0; i < writerSchema.Count; i++)
            {
                ...
                if (readerSchema is UnionSchema)
                {
                    ...
                    if (readerBranch == -1)
                    {
                        lookup[i] = (r, d) => { throw new AIngleException( "No matching schema for " + writerBranch + " in " + unionReader ); };
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        lookup[i] = (r, d) => { throw new AIngleException( "Schema mismatch Reader: " + ReaderSchema + ", writer: " + WriterSchema ); };
                    }
                    else
                    {
