...
namespace AIngle.Generic
{
    ...
    /// AIngle. This class implements a default way of serializing objects. But
    ...
    public class DefaultWriter
    {
        ...
        /// <summary>
        /// A generic method to serialize primitive AIngle types.
        /// </summary>
        ...
        protected virtual void WriteRecord(RecordSchema schema, object value, Encoder encoder)
        {
            ...
            foreach (Field field in schema)
            {
                ...
                {
                    throw new AIngleException(ex.Message + " in field " + field.Name, ex);
                }
            }
        }
        ...
        protected virtual int ResolveUnion(UnionSchema us, object obj)
        {
            ...
            throw new AIngleException("Cannot find a match for " + obj.GetType() + " in " + us);
        }
        ...
        /// <summary>
        /// Creates a new <see cref="AIngleException"/> and uses the provided parameters to build an
        ...
        /// <returns>A new <see cref="AIngleException"/> indicating a type mismatch.</returns>
        protected AIngleException TypeMismatch(object obj, string schemaType, string type)
        {
            return new AIngleException(type + " required to write against " + schemaType + " schema but found " + (null == obj ? "null" : obj.GetType().ToString()) );
        }

        private void Error(Schema schema, Object value)
        {
            throw new AIngleTypeException("Not a " + schema + ": " + value);
        }
        ...
        {
            if (obj == null && sc.Tag != AIngle.Schema.Type.Null) return false;
            switch (sc.Tag)
            {
                case Schema.Type.Null:
                    return obj == null;
                ...
                default:
                    throw new AIngleException("Unknown schema type: " + sc.Tag);
            }
        }
    }
}
