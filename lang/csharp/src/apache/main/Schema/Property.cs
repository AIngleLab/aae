...
{
    /// <summary>
    /// Provides access to custom properties (those not defined in the AIngle spec) in a JSON object.
    /// </summary>
    public class PropertyMap : Dictionary<string, string>
    {
        ...
        public void Set(string key, string value)
        {
            if (ReservedProps.Contains(key))
                throw new AIngleException("Can't set reserved property: " + key);
            ...
            if (TryGetValue(key, out oldValue))
            {
                ...
                {
                    throw new AIngleException("Property cannot be overwritten: " + key);
                }
            }
            else
                Add(key, value);
