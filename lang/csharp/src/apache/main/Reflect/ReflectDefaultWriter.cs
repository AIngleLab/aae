...
namespace AIngle.Reflect
{
    ...
    public class ReflectDefaultWriter : SpecificDefaultWriter
    {
        ...
        protected override void WriteFixed(FixedSchema schema, object value, Encoder encoder)
        {
            ...
            if (fixedrec == null)
            {
                throw new AIngleTypeException("Fixed object is not derived from byte[]");
            }

            if (fixedrec.Length != schema.Size)
            {
                throw new AIngleTypeException($"Fixed object length is not the same as schema length {schema.Size}");
            }
        ...
        protected override void WriteArray(ArraySchema schema, object value, Encoder encoder)
        {
            ...
            if (arr == null)
            {
                throw new AIngleTypeException("Array does not implement have registered ReflectArray derived type");
            }
        ...
        protected override void WriteMap(MapSchema schema, object value, Encoder encoder)
        {
            if (value == null)
            {
                throw new AIngleTypeException("Map is null - use a union for nullable types");
            }
