...
namespace AIngle.Test
{
    ...
    [TestFixture]
    public class TestFromAIngleProject
    {
        private const string _aingleTestSchemaV1 = @"{
        ""protocol"" : ""MyProtocol"",
        ""namespace"" : ""com.foo"",
        ""types"" :
        [
        ...
        public Z SerializeDeserialize(Z z)
        {
            try
            {
                Protocol protocol = Protocol.Parse(_aingleTestSchemaV1);
                Schema schema = null;
                ...
                var aingleWriter = new ReflectWriter<Z>(schema);
                var aingleReader = new ReflectReader<Z>(schema, schema);

                byte[] serialized;

                using (var stream = new MemoryStream(256))
                {
                    aingleWriter.Write(z, new BinaryEncoder(stream));
                ...
                {
                    deserialized = aingleReader.Read(default(Z), new BinaryDecoder(stream));
                }
