...
namespace AIngle.Test.File
{
    [TestFixture]
    public class FileTests
    {
        const string specificSchema  = "{\"type\":\"record\",\"name\":\"Foo\",\"namespace\":\"AIngle.Test.File\",\"fields\":"
        ...
        public void OpenAppendWriter_IncorrectInStream_Throws()
        {
            ...
            var ex = Assert.Throws(typeof(AIngleRuntimeException), action);
        }
        ...
        public void OpenAppendWriter_IncorrectOutStream_Throws()
        {
            ...
            Assert.Throws(typeof(AIngleRuntimeException), action);
        }
        ...
        public void TestNonSeekableStream(string schemaStr, object[] value, Codec.Type codecType)
        {
            foreach (var rwFactory in GenericOptions<GenericRecord>())
            {
                ...
                using (IFileReader<GenericRecord> reader = rwFactory.CreateReader(dataFileInputStream, null))
                {
                    ...
                    Assert.Throws<AIngleRuntimeException>(() => reader.Seek(0));
                    Assert.Throws<AIngleRuntimeException>(() => reader.PreviousSync());
                }
    ...
    public class Foo : ISpecificRecord
    {
        ...
        public Schema Schema
        {
            get
            {
                return Schema.Parse("{\"type\":\"record\",\"name\":\"Foo\",\"namespace\":\"AIngle.Test.File\"," +
