...
//! Logic handling reading from AIngle format at user level.
use crate::{decode::decode, schema::Schema, types::Value, util, AIngleResult, Codec, Error};
...
impl<R: Read> Block<R> {
    fn new(reader: R) -> AIngleResult<Block<R>> {
        let mut block = Block {
            reader,
            codec: Codec::Null,
    ...
    /// its content.
    fn read_header(&mut self) -> AIngleResult<()> {
        ...
        if let Value::Map(meta) = decode(&meta_schema, &mut self.reader)? {
            ...
            let json = meta
                ...
                .ok_or(Error::GetAIngleSchemaFromMap)?;
    ...
    fn fill_buf(&mut self, n: usize) -> AIngleResult<()> {
    ...
    fn read_block_next(&mut self) -> AIngleResult<()> {
    ...
    fn read_next(&mut self, read_schema: Option<&Schema>) -> AIngleResult<Option<Value>> {
        if self.is_empty() {
        ...
        let item = from_aingle_datum(&self.writer_schema, &mut block_bytes, read_schema)?;
...
/// Main interface for reading AIngle formatted values.
///
...
impl<'a, R: Read> Reader<'a, R> {
    ...
    ///
    /// **NOTE** The aingle header is going to be read automatically upon creation of the `Reader`.
    pub fn new(reader: R) -> AIngleResult<Reader<'a, R>> {
    ...
    ///
    /// **NOTE** The aingle header is going to be read automatically upon creation of the `Reader`.
    pub fn with_schema(schema: &'a Schema, reader: R) -> AIngleResult<Reader<'a, R>> {
        let block = Block::new(reader)?;
    ...
    #[inline]
    fn read_next(&mut self) -> AIngleResult<Option<Value>> {
...
/// Decode a `Value` encoded in AIngle format given its `Schema` and anything implementing `io::Read`
/// to read from.
///
...
pub fn from_aingle_datum<R: Read>(
    writer_schema: &Schema,
    reader: &mut R,
...
mod tests {
    ...
    fn test_from_aingle_datum() {
        ...
        assert_eq!(
            from_aingle_datum(&schema, &mut encoded, None).unwrap(),
            expected
        );
    }
