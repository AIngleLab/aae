...
//! Logic handling writing in AIngle format at user level.
use crate::{
    encode::{encode, encode_ref, encode_to_vec},
    schema::Schema,
    ser::Serializer,
    types::Value,
    AIngleResult, Codec, Error,
};
use rand::random;
use serde::Serialize;
...
const AINGLE_OBJECT_HEADER: &[u8] = b"Obj\x01";

/// Main interface for writing AIngle formatted values.
...
impl<'a, W: Write> Writer<'a, W> {
    ...
    /// Append a compatible value (implementing the `ToAIngle` trait) to a `Writer`, also performing
    /// schema validation.
    ///
    ...
    pub fn append<T: Into<Value>>(&mut self, value: T) -> AIngleResult<usize> {
        ...
        let aingle = value.into();
        write_value_ref(self.schema, &aingle, &mut self.buffer)?;
    ...
    /// internal buffering for performance reasons. If you want to be sure the value has been
    /// written, then call [`flush`](struct.Writer.html#method.flush).
    pub fn append_value_ref(&mut self, value: &Value) -> AIngleResult<usize> {
        let n = if !self.has_header {
            let header = self.header()?;
            let n = self.append_bytes(header.as_ref())?;
            self.has_header = true;
    ...
    /// internal buffering for performance reasons. If you want to be sure the value has been
    /// written, then call [`flush`](struct.Writer.html#method.flush).
    pub fn append_ser<S: Serialize>(&mut self, value: S) -> AIngleResult<usize> {
        let aingle_value = value.serialize(&mut self.serializer)?;
        self.append(aingle_value)
    }

    /// Extend a `Writer` with an `Iterator` of compatible values (implementing the `ToAIngle`
    ...
    pub fn extend<I, T: Into<Value>>(&mut self, values: I) -> AIngleResult<usize>
    where
    ...
    /// **NOTE** This function forces the written data to be flushed (an implicit
    /// call to [`flush`](struct.Writer.html#method.flush) is performed).
    pub fn extend_ser<I, T: Serialize>(&mut self, values: I) -> AIngleResult<usize>
    where
        I: IntoIterator<Item = T>,
    {
        /*
    ...
    /// **NOTE** This function forces the written data to be flushed (an implicit
    /// call to [`flush`](struct.Writer.html#method.flush) is performed).
    pub fn extend_from_slice(&mut self, values: &[Value]) -> AIngleResult<usize> {
        let mut num_bytes = 0;
        for value in values {
    ...
    ///
    /// Return the number of bytes written.
    pub fn flush(&mut self) -> AIngleResult<usize> {
    ...
    /// **NOTE** This function forces the written data to be flushed (an implicit
    /// call to [`flush`](struct.Writer.html#method.flush) is performed).
    pub fn into_inner(mut self) -> AIngleResult<W> {
        self.flush()?;
        Ok(self.writer)
    }

    /// Generate and append synchronization marker to the payload.
    fn append_marker(&mut self) -> AIngleResult<usize> {
    ...
    /// Append a raw AIngle Value to the payload avoiding to encode it again.
    fn append_raw(&mut self, value: &Value, schema: &Schema) -> AIngleResult<usize> {
        self.append_bytes(encode_to_vec(&value, schema).as_ref())
    }

    /// Append pure bytes to the payload.
    fn append_bytes(&mut self, bytes: &[u8]) -> AIngleResult<usize> {
        self.writer.write(bytes).map_err(Error::WriteBytes)
    }

    /// Create an AIngle header based on schema, codec and sync marker.
    fn header(&self) -> Result<Vec<u8>, Error> {
        ...
        metadata.insert("aingle.schema", Value::Bytes(schema_bytes));
        metadata.insert("aingle.codec", self.codec.into());

        let mut header = Vec::new();
        header.extend_from_slice(AINGLE_OBJECT_HEADER);
        encode(
            &metadata.into(),
...
/// Encode a compatible value (implementing the `ToAIngle` trait) into AIngle format, also performing
/// schema validation.
///
/// This is an internal function which gets the bytes buffer where to write as parameter instead of
/// creating a new one like `to_aingle_datum`.
fn write_aingle_datum<T: Into<Value>>(
    schema: &Schema,
    value: T,
    buffer: &mut Vec<u8>,
) -> Result<(), Error> {
    let aingle = value.into();
    if !aingle.validate(schema) {
        return Err(Error::Validation);
    }
    encode(&aingle, schema, buffer);
    Ok(())
}

fn write_value_ref(schema: &Schema, value: &Value, buffer: &mut Vec<u8>) -> AIngleResult<()> {
...
/// Encode a compatible value (implementing the `ToAIngle` trait) into AIngle format, also
/// performing schema validation.
///
/// **NOTE** This function has a quite small niche of usage and does NOT generate headers and sync
/// markers; use [`Writer`](struct.Writer.html) to be fully AIngle-compatible if you don't know what
/// you are doing, instead.
pub fn to_aingle_datum<T: Into<Value>>(schema: &Schema, value: T) -> AIngleResult<Vec<u8>> {
    let mut buffer = Vec::new();
    write_aingle_datum(schema, value, &mut buffer)?;
    Ok(buffer)
}
...
mod tests {
    ...
    const AINGLE_OBJECT_HEADER_LEN: usize = AINGLE_OBJECT_HEADER.len();
    ...
    fn test_to_aingle_datum() {
        ...
        assert_eq!(to_aingle_datum(&schema, record).unwrap(), expected);
    }
    ...
    fn test_union_not_null() {
        ...
        assert_eq!(to_aingle_datum(&schema, union).unwrap(), expected);
    }
    ...
    fn logical_type_test<T: Into<Value> + Clone>(
        ...
        let ser = to_aingle_datum(&schema, value.clone())?;
        let raw_ser = to_aingle_datum(&raw_schema, raw_value)?;
        assert_eq!(ser, raw_ser);

        // Should deserialize from the schema into the logical type.
        let mut r = ser.as_slice();
        let de = crate::from_aingle_datum(&schema, &mut r, None).unwrap();
        assert_eq!(de, value);
        Ok(())
    }
    ...
    fn test_writer_append() {
        ...
        // starts with magic
        assert_eq!(&result[..AINGLE_OBJECT_HEADER_LEN], AINGLE_OBJECT_HEADER);
