...
//! Logic for all supported compression codecs in AIngle.
use crate::{types::Value, AIngleResult, Error};
...
impl Codec {
    ...
    pub fn compress(self, stream: &mut Vec<u8>) -> AIngleResult<()> {
        match self {
            Codec::Null => (),
            Codec::Deflate => {
    ...
    pub fn decompress(self, stream: &mut Vec<u8>) -> AIngleResult<()> {
        *stream = match self {
