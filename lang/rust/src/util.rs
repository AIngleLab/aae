...
pub fn read_long<R: Read>(reader: &mut R) -> AIngleResult<i64> {
    zag_i64(reader)
}
...
pub fn zag_i32<R: Read>(reader: &mut R) -> AIngleResult<i32> {
    let i = zag_i64(reader)?;
    i32::try_from(i).map_err(|e| Error::ZagI32(e, i))
}

pub fn zag_i64<R: Read>(reader: &mut R) -> AIngleResult<i64> {
...
fn decode_variable<R: Read>(reader: &mut R) -> AIngleResult<u64> {
    let mut i = 0u64;
    let mut buf = [0u8; 1];
...
pub fn safe_len(len: usize) -> AIngleResult<usize> {
