...
pub use reader::{from_aingle_datum, Reader};
pub use schema::Schema;
pub use ser::to_value;
pub use util::max_allocation_bytes;
pub use writer::{to_aingle_datum, Writer};

/// A convenience type alias for `Result`s with `Error`s.
pub type AIngleResult<T> = Result<T, Error>;
...
mod tests {
    ...
    fn test_illformed_length() {
        ...
        let value = from_aingle_datum(&schema, &mut &illformed[..], None);
        assert!(value.is_err());
    }
}
