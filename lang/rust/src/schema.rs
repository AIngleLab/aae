...
//! Logic for parsing and interacting with schemas in AIngle format.
use crate::{error::Error, types, util::MapHelper, AIngleResult};
use digest::Digest;
...
/// Represents an AIngle schema fingerprint
/// More information about AIngle schema fingerprints can be found in the
/// [AIngle Schema Fingerprint documentation](https://apache.aingle.ai/docs/current/spec.html#schema_fingerprints)
...
/// Represents any valid AIngle schema
/// More information about AIngle schemas can be found in the
...
pub enum Schema {
    /// A `null` AIngle schema.
    Null,
    /// A `boolean` AIngle schema.
    Boolean,
    /// An `int` AIngle schema.
    Int,
    /// A `long` AIngle schema.
    Long,
    /// A `float` AIngle schema.
    Float,
    /// A `double` AIngle schema.
    Double,
    /// A `bytes` AIngle schema.
    /// `Bytes` represents a sequence of 8-bit unsigned bytes.
    Bytes,
    /// A `string` AIngle schema.
    /// `String` represents a unicode character sequence.
    String,
    /// A `array` AIngle schema. AIngle arrays are required to have the same type for each element.
    ...
    Map(Box<Schema>),
    /// A `union` AIngle schema.
    Union(UnionSchema),
    /// A `record` AIngle schema.
    ///
    ...
    /// An `enum` AIngle schema.
    Enum {
        name: Name,
        doc: Documentation,
        symbols: Vec<String>,
    },
    /// A `fixed` AIngle schema.
...
impl PartialEq for Schema {
    ...
    /// https://apache.aingle.ai/docs/1.8.2/spec.html#Parsing+Canonical+Form+for+Schemas
...
/// Represents names for `record`, `enum` and `fixed` AIngle schemas.
///
...
/// [AIngle specification](https://apache.aingle.ai/docs/current/spec.html#names)
...
/// Represents documentation for complex AIngle schemas.
...
impl Name {
    ...
    fn parse(complex: &Map<String, Value>) -> AIngleResult<Self> {
...
/// Represents a `field` in a `record` AIngle schema.
...
pub struct RecordField {
    ...
    /// This value will be used when reading AIngle datum if schema resolution
    /// is enabled.
...
/// Represents any valid order for a `field` in a `record` AIngle schema.
...
impl RecordField {
    ...
    fn parse(field: &Map<String, Value>, position: usize, parser: &mut Parser) -> AIngleResult<Self> {
...
impl UnionSchema {
    pub(crate) fn new(schemas: Vec<Schema>) -> AIngleResult<Self> {
...
impl Schema {
    ...
    /// https://apache.aingle.ai/docs/1.8.2/spec.html#Parsing+Canonical+Form+for+Schemas
    /// [fingerprint]:
    /// https://apache.aingle.ai/docs/current/spec.html#schema_fingerprints
    ...
    /// Create a `Schema` from a string representing a JSON AIngle schema.
    pub fn parse_str(input: &str) -> Result<Schema, Error> {
        let mut parser = Parser::default();
        parser.parse_str(input)
    }

    /// Create a array of `Schema`'s from a list of named JSON AIngle schemas (Record, Enum, and
    /// Fixed).
    ///
    ...
    pub fn parse(value: &Value) -> AIngleResult<Schema> {
        let mut parser = Parser::default();
        parser.parse(value)
    }
}
...
impl Parser {
    ...
    /// Create an array of `Schema`'s from an iterator of JSON AIngle schemas. It is allowed that
    ...
    /// Create a `Schema` from a `serde_json::Value` representing a JSON AIngle
    /// schema.
    fn parse(&mut self, value: &Value) -> AIngleResult<Schema> {
        match *value {
    ...
    /// Parse a `serde_json::Value` representing an AIngle type whose Schema is known into a
    ...
    fn parse_known_schema(&mut self, name: &str) -> AIngleResult<Schema> {
        match name {
            "null" => Ok(Schema::Null),
    ...
    fn fetch_schema(&mut self, name: &str) -> AIngleResult<Schema> {
    ...
    /// Parse a `serde_json::Value` representing a complex AIngle type into a
    /// `Schema`.
    ///
    /// AIngle supports "recursive" definition of types.
    /// e.g: {"type": {"type": "string"}}
    fn parse_complex(&mut self, complex: &Map<String, Value>) -> AIngleResult<Schema> {
    ...
    /// Parse a `serde_json::Value` representing a AIngle record type into a
    /// `Schema`.
    fn parse_record(&mut self, complex: &Map<String, Value>) -> AIngleResult<Schema> {
    ...
    /// Parse a `serde_json::Value` representing a AIngle enum type into a
    /// `Schema`.
    fn parse_enum(complex: &Map<String, Value>) -> AIngleResult<Schema> {
        let name = Name::parse(complex)?;
    ...
    /// Parse a `serde_json::Value` representing a AIngle array type into a
    /// `Schema`.
    fn parse_array(&mut self, complex: &Map<String, Value>) -> AIngleResult<Schema> {
        complex
            .get("items")
    ...
    /// Parse a `serde_json::Value` representing a AIngle map type into a
    /// `Schema`.
    fn parse_map(&mut self, complex: &Map<String, Value>) -> AIngleResult<Schema> {
        complex
            .get("values")
    ...
    /// Parse a `serde_json::Value` representing a AIngle union type into a
    /// `Schema`.
    fn parse_union(&mut self, items: &[Value]) -> AIngleResult<Schema> {
        items
            .iter()
            .map(|v| self.parse(v))
    ...
    /// Parse a `serde_json::Value` representing a AIngle fixed type into a
    /// `Schema`.
    fn parse_fixed(complex: &Map<String, Value>) -> AIngleResult<Schema> {
        let name = Name::parse(complex)?;
        ...
        S: Serializer,
    {
        match *self {
            ...
            Schema::Duration => {
                ...
                // the AIngle doesn't indicate what the name of the underlying fixed type of a
...
/// Parses a **valid** aingle schema into the Parsing Canonical Form.
/// https://apache.aingle.ai/docs/1.8.2/spec.html#Parsing+Canonical+Form+for+Schemas
