...
//! Logic handling the intermediate representation of AIngle values.
use crate::{
    decimal::Decimal,
    duration::Duration,
...
/// A valid AIngle value.
///
/// More information about AIngle values can be found in the [AIngle
/// Specification](https://apache.aingle.ai/docs/current/spec.html#schemas)
#[derive(Clone, Debug, PartialEq, strum_macros::EnumDiscriminants)]
#[strum_discriminants(name(ValueKind))]
pub enum Value {
    /// A `null` AIngle value.
    Null,
    /// A `boolean` AIngle value.
    Boolean(bool),
    /// A `int` AIngle value.
    Int(i32),
    /// A `long` AIngle value.
    Long(i64),
    /// A `float` AIngle value.
    Float(f32),
    /// A `double` AIngle value.
    Double(f64),
    /// A `bytes` AIngle value.
    Bytes(Vec<u8>),
    /// A `string` AIngle value.
    String(String),
    /// A `fixed` AIngle value.
    /// The size of the fixed value is represented as a `usize`.
    Fixed(usize, Vec<u8>),
    /// An `enum` AIngle value.
    ///
    ...
    /// reading values.
    Enum(i32, String),
    /// An `union` AIngle value.
    Union(Box<Value>),
    /// An `array` AIngle value.
    Array(Vec<Value>),
    /// A `map` AIngle value.
    Map(HashMap<String, Value>),
    /// A `record` AIngle value.
    ///
    ...
    /// schema.
    Date(i32),
    /// An AIngle Decimal value. Bytes are in big-endian order, per the AIngle spec.
    Decimal(Decimal),
    ...
    TimestampMicros(i64),
    /// AIngle Duration. An amount of time defined by months, days and milliseconds.
    Duration(Duration),
    /// Universally unique identifier.
    /// Universally unique identifier.
    Uuid(Uuid),
}
/// Any structure implementing the [ToAIngle](trait.ToAIngle.html) trait will be usable
...
pub trait ToAIngle {
    /// Transforms this value into an AIngle-compatible [Value](enum.Value.html).
    fn aingle(self) -> Value;
}

#[allow(deprecated)]
impl<T: Into<Value>> ToAIngle for T {
    fn aingle(self) -> Value {
        self.into()
    }
}
...
impl<'a> Record<'a> {
    ...
    /// Put a compatible value (implementing the `ToAIngle` trait) in the
...
/// Convert AIngle values to Json values
impl std::convert::TryFrom<Value> for JsonValue {
    type Error = crate::error::Error;
    fn try_from(value: Value) -> AIngleResult<Self> {
        match value {
...
impl Value {
    ...
    ///
    /// See the [AIngle specification](https://apache.aingle.ai/docs/current/spec.html)
    ...
    ///
    /// See [Schema Resolution](https://apache.aingle.ai/docs/current/spec.html#Schema+Resolution)
    /// in the AIngle specification for the full set of rules of schema
    /// resolution.
    pub fn resolve(mut self, schema: &Schema) -> AIngleResult<Self> {
    ...
    fn try_u8(self) -> AIngleResult<u8> {
