...
#[inline]
fn decode_long<R: Read>(reader: &mut R) -> AIngleResult<Value> {
    zag_i64(reader).map(Value::Long)
}

#[inline]
fn decode_int<R: Read>(reader: &mut R) -> AIngleResult<Value> {
    zag_i32(reader).map(Value::Int)
}

#[inline]
fn decode_len<R: Read>(reader: &mut R) -> AIngleResult<usize> {
...
///
/// Maps and arrays are 0-terminated, 0i64 is also encoded as 0 in AIngle reading a length of 0 means
/// the end of the map or array.
fn decode_seq_len<R: Read>(reader: &mut R) -> AIngleResult<usize> {
...
/// Decode a `Value` from aingle format given its `Schema`.
pub fn decode<R: Read>(schema: &Schema, reader: &mut R) -> AIngleResult<Value> {
    match *schema {
