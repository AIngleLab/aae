...
[![Latest Version](https://img.shields.io/crates/v/aingle-rs-tool.svg)](https://crates.io/crates/aingle-rs-tool)
[![Continuous Integration](https://github.com/AIngleLab/aingle-rs-tool/workflows/Continuous%20Integration/badge.svg)](https://github.com/AIngleLab/aingle-rs-tool/actions)
[![Latest Documentation](https://docs.rs/aingle-rs-tool/badge.svg)](https://docs.rs/aingle-rs-tool)
[![MIT licensed](https://img.shields.io/badge/license-MIT-blue.svg)](https://github.com/AIngleLab/aingle-rs-tool/blob/main/LICENSE)

A library for working with [Apache AIngle](https://apache.aingle.ai/) in Rust.

Please check our [documentation](https://docs.rs/aingle-rs-tool) for examples, tutorials and API reference.

**[Apache AIngle](https://apache.aingle.ai/)** is a data serialization system which provides rich
data structures and a compact, fast, binary data format.

All data in AIngle is schematized, as in the following example:
...
There are basically two ways of handling AIngle data in Rust:

* **as AIngle-specialized data types** based on an AIngle schema;
* **as generic Rust serde-compatible types** implementing/deriving `Serialize` and
`Deserialize`;

**aingle-rs-tool** provides a way to read and write both these data representations easily and
efficiently.
...
An AIngle data cannot exist without an AIngle schema. Schemas **must** be used while writing and
**can** be used while reading and they carry the information regarding the type of data we are
handling. AIngle schemas are used for both schema validation and resolution of AIngle data.

AIngle schemas are defined in **JSON** format and can just be parsed out of a raw string:

```rust
use aingle_rs::Schema;
...
[AIngle Specification](https://apache.aingle.ai/docs/current/spec.html#schemas).

## Writing data

Once we have defined a schema, we are ready to serialize data in AIngle, validating them against
the provided schema in the process. As mentioned before, there are two ways of handling AIngle
data in Rust.

**NOTE:** The library also provides a low-level interface for encoding a single datum in AIngle
bytecode without generating markers and headers (for advanced use), but we highly recommend the
`Writer` interface to be totally AIngle-compatible. Please read the API reference in case you are
interested.

### The aingle way

Given that the schema we defined above is that of an AIngle *Record*, we are going to use the
associated type provided by the library to specify the data we want to serialize:

```rust
use aingle_rs::types::Record;
use aingle_rs::Writer;
#
...
// this is how to get back the resulting aingle bytecode
...
case we want to directly define an AIngle value, the library offers that capability via the
`Value` interface.

```rust
use aingle_rs::types::Value;
...
Given that the schema we defined above is an AIngle *Record*, we can directly use a Rust struct
...
The vast majority of the times, schemas tend to define a record as a top-level container
encapsulating all the values to convert as fields and providing documentation for them, but in
case we want to directly define an AIngle value, any type implementing `Serialize` should work.
...
AIngle supports three different compression codecs when encoding data:
...
As far as reading AIngle encoded data goes, we can just use the schema encoded with the data to
read them. The library will do it automatically for us, as it already does for the compression
codec:

```rust
use aingle_rs::Reader;
#
// reader creation can fail in case the input to read from is not AIngle-compatible or malformed
...
As usual, there are two ways to handle AIngle data in Rust, as you can see below.

**NOTE:** The library also provides a low-level interface for decoding a single datum in AIngle
bytecode without markers and header (for advanced use), but we highly recommend the `Reader`
interface to leverage all AIngle features. Please read the API reference in case you are
interested.
...
// value is a Result  of an AIngle Value in case the read operation fails
for value in reader {
...
```rust
use aingle_rs::{Codec, Reader, Schema, Writer, from_value, types::Record, Error};
...
`aingle-rs-tool` also supports the logical types listed in the [AIngle specification](https://apache.aingle.ai/docs/current/spec.html#Logical+Types):
...
### Calculate AIngle schema fingerprint
...
```rust
use aingle_rs::rabin::Rabin;
use aingle_rs::{Schema, Error};
use md5::Md5;
use sha2::Sha256;
...
In order to ease decoding, the Binary Encoding specification of AIngle data
...
To shield users from ill-formed data, `aingle-rs-tool` sets a limit (default: 512MB)
...
```rust
use aingle_rs::max_allocation_bytes;
...
https://github.com/AIngleLab/aingle-rs-tool/pull/76).
...
```rust
use aingle_rs::{Schema, schema_compatibility::SchemaCompatibility};
...
## License
This project is licensed under [MIT License](https://github.com/AIngleLab/aingle-rs-tool/blob/main/LICENSE).
Please note that this is not an official project maintained by [Apache AIngle](https://apache.aingle.ai/).
...
All contributions will be licensed under [MIT License](https://github.com/AIngleLab/aingle-rs-tool/blob/main/LICENSE).

Please consider adding documentation, tests and a line for your change under the Unreleased section in the [CHANGELOG](https://github.com/AIngleLab/aingle-rs-tool/blob/main/CHANGELOG.md).
