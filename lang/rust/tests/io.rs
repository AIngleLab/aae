...
//! Port of https://github.com/AIngleLab/aae/blob/release-1.9.1/lang/py/test/test_io.py
use aingle_rs::{from_aingle_datum, to_aingle_datum, types::Value, Error, Schema};
...
fn test_round_trip() {
    for (raw_schema, value) in SCHEMAS_TO_VALIDATE.iter() {
        let schema = Schema::parse_str(raw_schema).unwrap();
        let encoded = to_aingle_datum(&schema, value.clone()).unwrap();
        let decoded = from_aingle_datum(&schema, &mut Cursor::new(encoded), None).unwrap();
...
fn test_binary_int_encoding() {
    for (number, hex_encoding) in BINARY_ENCODINGS.iter() {
        let encoded = to_aingle_datum(&Schema::Int, Value::Int(*number as i32)).unwrap();
...
fn test_binary_long_encoding() {
    for (number, hex_encoding) in BINARY_ENCODINGS.iter() {
        let encoded = to_aingle_datum(&Schema::Long, Value::Long(*number as i64)).unwrap();
        assert_eq!(&encoded, hex_encoding);
    }
}
...
fn test_schema_promotion() {
    ...
    for (i, writer_raw_schema) in promotable_schemas.iter().enumerate() {
        ...
        for (j, reader_raw_schema) in promotable_schemas.iter().enumerate().skip(i + 1) {
            ...
            let encoded = to_aingle_datum(&writer_schema, original_value.clone()).unwrap();
            let decoded = from_aingle_datum(
                &writer_schema,
...
fn test_unknown_symbol() {
    ...
    let encoded = to_aingle_datum(&writer_schema, original_value).unwrap();
    let decoded = from_aingle_datum(
        &writer_schema,
        &mut Cursor::new(encoded),
        Some(&reader_schema),
...
fn test_default_value() {
    for (field_type, default_json, default_datum) in DEFAULT_VALUE_EXAMPLES.iter() {
        ...
        let encoded = to_aingle_datum(&LONG_RECORD_SCHEMA, LONG_RECORD_DATUM.clone()).unwrap();
        let datum_read = from_aingle_datum(
            &LONG_RECORD_SCHEMA,
            &mut Cursor::new(encoded),
            Some(&reader_schema),
        )
...
fn test_type_exception() -> Result<(), String> {
    ...
    let encoded = to_aingle_datum(&writer_schema, datum_to_write);
    match encoded {
