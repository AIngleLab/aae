...
class DataFileTest extends TestCase
{
    ...
    public function test_write_read_nothing_round_trip()
    {
        foreach (AIngleDataIO::validCodecs() as $codec) {
            if (
                ($codec === AIngleDataIO::SNAPPY_CODEC && !extension_loaded('snappy'))
                || ($codec === AIngleDataIO::ZSTANDARD_CODEC && !extension_loaded('zstd'))
            ) {
                continue;
            }
            $data_file = $this->add_data_file(sprintf('data-wr-nothing-null-%s.avr', $codec));
            $writers_schema = '"null"';
            $dw = AIngleDataIO::openFile($data_file, 'w', $writers_schema, $codec);
            $dw->close();

            $dr = AIngleDataIO::openFile($data_file);
            $data = $dr->data();
            $dr->close();
...
JSON;
            ...
            $dw = AIngleDataIO::openFile($data_file, 'w', $writer_schema, $codec);
            foreach ($data as $datum) {
                $dw->append($datum);
            }
            $dw->close();
...
JSON;
            $dr = AIngleDataIO::openFile($data_file, 'r', $reader_schema);
...
JSON;
            ...
            foreach (array(
                ...
                $dr = AIngleDataIO::openFile($data_file, 'r', json_encode($readers_schema));
