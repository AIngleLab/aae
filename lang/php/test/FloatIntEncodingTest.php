...
_RUBY;
  ...
  function assert_encode_values($type, $val, $bits)
  {
    ...
    {
      $decoder = array(AIngleIOBinaryDecoder::class, 'intBitsToFloat');
      $encoder = array(AIngleIOBinaryEncoder::class, 'floatToIntBits');
    }
    else
    {
      $decoder = array(AIngleIOBinaryDecoder::class, 'longBitsToDouble');
      $encoder = array(AIngleIOBinaryEncoder::class, 'doubleToLongBits');
    }
    ...
    $this->assertEquals($bits, $encoded_val_bits,
                        sprintf("%s\n expected: '%s'\n    given: '%s'",
                                'ENCODED VAL',
                                AIngleDebug::hexString($bits),
                                AIngleDebug::hexString($encoded_val_bits)));
