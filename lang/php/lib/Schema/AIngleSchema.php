 ...
 *  - Should AIngleField really be an AIngleSchema object? AIngle Fields have a name
...
class AIngleSchema
{
    ...
    /**
     * Unions of error schemas are used by AIngle messages
     ...
     *           a class which extends AIngleSchema
     */
    ...
    {
        $schemata = new AIngleNamedSchemata();
        return self::realParse(json_decode($json, true), null, $schemata);
    }

    /**
     * @param mixed $aingle JSON-decoded schema
     * @param string $default_namespace namespace of enclosing schema
     * @param AIngleNamedSchemata &$schemata reference to named schemas
     * @returns AIngleSchema
     * @throws AIngleSchemaParseException
     */
    public static function realParse($aingle, $default_namespace = null, &$schemata = null)
    {
        if (is_null($schemata)) {
            $schemata = new AIngleNamedSchemata();
        }
        ...
        if (is_array($aingle)) {
            $type = $aingle[self::TYPE_ATTR] ?? null;

            if (self::isPrimitiveType($type)) {
                return new AInglePrimitiveSchema($type);
            }

            if (self::isNamedType($type)) {
                $name = $aingle[self::NAME_ATTR] ?? null;
                $namespace = $aingle[self::NAMESPACE_ATTR] ?? null;
                $new_name = new AIngleName($name, $namespace, $default_namespace);
                $doc = $aingle[self::DOC_ATTR] ?? null;
                $aliases = $aingle[self::ALIASES_ATTR] ?? null;
                switch ($type) {
                    case self::FIXED_SCHEMA:
                        $size = $aingle[self::SIZE_ATTR] ?? null;
                        return new AIngleFixedSchema(
                            $new_name,
                            $doc,
                            $size,
                            $schemata,
                    ...
                    case self::ENUM_SCHEMA:
                        $symbols = $aingle[self::SYMBOLS_ATTR] ?? null;
                        return new AIngleEnumSchema(
                            $new_name,
                            $doc,
                            $symbols,
                            $schemata,
                    ...
                    case self::ERROR_SCHEMA:
                        $fields = $aingle[self::FIELDS_ATTR] ?? null;
                        return new AIngleRecordSchema(
                            $new_name,
                            $doc,
                            $fields,
                            $schemata,
                    ...
                    default:
                        throw new AIngleSchemaParseException(sprintf('Unknown named type: %s', $type));
                }
            } elseif (self::isValidType($type)) {
                switch ($type) {
                    case self::ARRAY_SCHEMA:
                        return new AIngleArraySchema(
                            $aingle[self::ITEMS_ATTR],
                            $default_namespace,
                            $schemata
                        );
                    case self::MAP_SCHEMA:
                        return new AIngleMapSchema(
                            $aingle[self::VALUES_ATTR],
                            $default_namespace,
                            $schemata
                        );
                    default:
                ...
                !array_key_exists(self::TYPE_ATTR, $aingle)
                && AIngleUtil::isList($aingle)
            ) {
                return new AIngleUnionSchema($aingle, $default_namespace, $schemata);
            } else {
                throw new AIngleSchemaParseException(sprintf(
                    'Undefined type: %s',
                    $type
                ));
            }
        } elseif (self::isPrimitiveType($aingle)) {
            return new AInglePrimitiveSchema($aingle);
        } else {
            throw new AIngleSchemaParseException(
                sprintf(
    ...
    public static function isValidDatum($expected_schema, $datum)
    {
        switch ($expected_schema->type) {
            ...
            case self::REQUEST_SCHEMA:
                ...
                return false;
            default:
                throw new AIngleSchemaParseException(sprintf('%s is not allowed.', $expected_schema));
        }
    }
     ...
     * @returns AIngleSchema
     * @throws AIngleSchemaParseException
     * @uses AIngleSchema::realParse()
     */
    protected static function subparse($aingle, $default_namespace, &$schemata = null)
    {
        try {
            return self::realParse($aingle, $default_namespace, $schemata);
        } catch (AIngleSchemaParseException $e) {
            throw $e;
        } catch (\Exception $e) {
            throw new AIngleSchemaParseException(
                sprintf(
                    'Sub-schema is not a valid AIngle schema. Bad schema: %s',
                    print_r($aingle, true)
                )
            );
        }
    ...
    /**
     * @returns string the JSON-encoded representation of this AIngle schema.
     */
    public function __toString()
    {
        return (string) json_encode($this->toAIngle());
    }
