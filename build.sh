...
# The entrypoint when running the aingle docker from this script.
...
do
  ...
  case "$target" in
    ...
    test)
      ...
      (cd lang/java/aingle; mvn -B -P interop-data-generate generate-resources)
    ...
    dist)
      ...
      SRC_DIR=aingle-src-$VERSION
      DOC_DIR=aingle-doc-$VERSION
      ...
      cp "lang/perl/AIngle-$VERSION.tar.gz" dist/perl/
      ...
      (cd build; tar czf "../dist/aingle-doc-$VERSION.tar.gz" "$DOC_DIR")
    ...
    docker)
      ...
      DOCKER_IMAGE_NAME=${DOCKER_IMAGE_NAME:-"aingle-build-$USER_NAME:latest"}
      {
        cat share/docker/Dockerfile
        grep -vF 'FROM aingle-build-ci' share/docker/DockerfileLocal
      ...
      docker run --rm -t -i \
        ...
        --volume "${PWD}:/home/${USER_NAME}/aingle${DOCKER_MOUNT_FLAG}" \
        --workdir "/home/${USER_NAME}/aingle" \
        ${DOCKER_RUN_XTRA_ARGS} "$DOCKER_IMAGE_NAME" ${DOCKER_RUN_ENTRYPOINT}
      ;;

    rat)
      mvn test -Dmaven.main.skip=true -Dmaven.test.skip=true -DskipTests=true -P rat -pl :aingle-toplevel
      ;;

    githooks)
      echo "Installing AINGLE git hooks."
    ...
    docker-test)
      tar -cf- share/docker/Dockerfile lang/ruby/Gemfile |
        docker build -t aingle-test -f share/docker/Dockerfile -
      docker run --rm -v "${PWD}:/aingle/" --env "JAVA=${JAVA:-8}" aingle-test /aingle/share/docker/run-tests.sh
      ;;
