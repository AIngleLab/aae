...
function usage {
  ...
  echo "the AIngle source code, on a computer where Maven is installed"
  echo "and the other build-prerequisites of AIngle are in place.  This"
  echo "script will do a 'mvn clean install' of AIngle from withing"
  ...
  echo 
  echo "     org.apache.aingle.io.perf.count -- the number of elements"
  echo "generated for the inner-most loop of the performance test.  Defaults"
  echo "to 250K.  Must be a multple of 4."
  echo
  echo "     org.apache.aingle.io.perf.cycles -- the number of times the inner-"
  echo "most loop is called within an invocation of Perf.java.  Defaults"
  echo " to 800."
  echo 
  echo "     org.apache.aingle.io.perf.use-direct -- use DirectBinaryEncoder instead"
  ...
  echo "improvements."
  echo
  echo "     org.apache.aingle.specific.use_custom_coders -- flag that turns on"
...
function Perf_java {
  ...
  if [[ "$DBG" != "" ]]; then
    $DBG MAVEN_OPTS=-server mvn exec:java -Dexec.classpathScope=test \
      -Dexec.mainClass=org.apache.aingle.io.Perf ${STATIC_SYSPROPS[@]} \
...
function run_trial {
  ...
  if [[ "$thisbranch" != "$lastbranch" ]]; then
    ...
    (cd ..; $DBG mvn clean && $DBG mvn -pl "aingle,compiler,maven-plugin,ipc" install -DskipTests)
  fi
